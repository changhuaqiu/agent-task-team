...
# F075 — Agent排行榜 (agent Leaderboard)
...
team lead和Agent们在 Agent Task Hub 里已经积累了大量互动数据——@ 提及、消息、review、游戏战绩。但这些数据从来没有被可视化过。一个排行榜/统计面板能让team lead看到"谁是最爱Agent"、"谁是深夜劳模"，也让Agent之间有良性竞争的趣味性。

team experience：
> "笑我们能统计出来team leadat 过多少Agent都分别几次吗？可以统计出team lead最爱的大Agent leaderboard 各种排行榜哈哈哈 是一个有趣的功能"
> "甚至你可以把我们举办的各种游戏都加入上！你可是Agent杀的多届冠军！只不过谁是卧底惨遭一轮淘汰"

## What

Agent Task Hub Hub 新增「排行榜」Tab，展示多维度Agent统计和排名。
...
- 时间范围筛选：全部 / 7 天 / 30 天
- 前端入口：**当前实现挂在 `Agent Task Hub Hub` modal 的「排行榜」tab**
- 后端基础：`GET /api/leaderboard/stats`

#### Phase B ✅（PR #377）
- "笨蛋Agent"排行榜 + 关键词情绪分析（silly-stats.ts）
- 游戏战绩面板（game-store.ts + GameArena UI）
- 移动端响应式布局
- 排行榜入口在 `Agent Task Hub Hub` modal tab（runtime 更新由team lead控制）
...
#### 1. @ 互动统计
- **team lead最爱Agent** 🏆 — 按 @ 次数排名（总计 + 近 7 天趋势）
- **深夜劳模** 🌙 — 凌晨 0:00-6:00 被 @ 最多的Agent
- **连续宠幸 Streak** 🔥 — 连续多少天被 @
- **话唠Agent** 💬 — 回复字数/消息数最多的Agent

#### 2. 工作统计
- **代码狂魔** 🛠️ — commit 数量排行
- **Review 之王** 🔍 — 给别的Agent review 最多次
- **修 Bug 达人** 🐛 — 修了多少 P1/P2
...
**CVO 成就（Bootcamp 来源）**：
- **初次拍板** — 用户第一次做 CVO 决策
- **初次否决** — 用户第一次拒绝Agent的方案
- **纠偏大师** — 用户成功纠正Agent跑偏的方向
- **冲突裁判** — 用户在两只Agent意见分歧时做出裁决
- **一路通关** — 完整走完 feat lifecycle（立项→完成）

**日常成就**：
- **夜Agent子** — 凌晨 2:00 后还在 @ Agent
- **连续签到** — 连续 N 天使用 Agent Task Hub
- **全Agent集邮** — 和每只Agent都有过互动
...
| Lv.2 | 判断方案 | 在Agent出的方案中做出选择并说理由 |
| Lv.3 | 纠偏 | 成功拒绝Agent的跑偏建议 |
| Lv.4 | 协调冲突 | 在Agent意见分歧时做出裁决 |
| Lv.5 | 复盘总结 | 完成首次全流程复盘 |

### 情绪分析（"笨蛋Agent"检测）

区分team lead的"亲昵骂"和"真生气"：
- **亲昵特征**：笨蛋、心机小Rogue Agent、小绿茶、傻Agent + 带"哈哈哈"或表情
...
- [x] AC-A2: `Agent Task Hub Hub` 新增「排行榜」Tab（当前实现入口）
- [x] AC-A3: @ 互动统计面板（4 个维度：最爱Agent / 深夜劳模 / 连续宠幸 / 话唠Agent）
- [x] AC-A4: 工作统计面板（3 个维度：commit / review / bug fix）
- [x] AC-A5: 时间范围筛选（全部 / 近 7 天 / 近 30 天）

## Acceptance Criteria — Phase B ✅

- [x] AC-B1: "笨蛋Agent"排行榜（含情绪分析）
- [x] AC-B2: 游戏战绩面板（至少Agent杀 + 谁是卧底）
- [x] AC-B3: 排行榜入口在 Agent Task Hub Hub modal tab（代码层面已确保可见；runtime 同步由team lead操作，不属于 feat close 门禁）
- [x] AC-B4: 移动端适配
...
| R2 | 最爱Agent排名 | team lead | P0 | AC-2 |
| R3 | 深夜劳模统计 | 讨论 | P1 | AC-2 |
| R4 | 工作量统计（commit/review） | 讨论 | P1 | AC-3 |
| R5 | 笨蛋Agent情绪分析 | team lead | P1 | AC-4 |
| R6 | 游戏战绩记录 | team lead | P1 | AC-5 |
| R7 | 时间范围筛选 | 设计 | P2 | AC-8 |
| R8 | 成就徽章系统（通用框架） | F087 依赖 | P1 | AC-6 |
| R9 | CVO 能力等级追踪 | F087 依赖 | P1 | AC-7 |
| R10 | 日常使用成就（夜Agent子、连续签到等） | 讨论 | P2 | AC-6 |

## Key Decisions

1. **整体视觉风格：极简活力Agent咖 (Minimal Vibrant Agent Task Hub)** — 采用 `mobile-03-minimalvibrant_light` 风格指南，强调 24px 超大圆角（Pillowy Corners）和鲜艳的配色（紫色/青色/粉色）。
2. **布局架构：Bento Grid (便当盒布局)** — 信息展示采用不同尺寸的色块格子，不带边框，通过背景色区分功能区。
3. **成就系统表现：Sticker Wall (贴纸墙)** — 成就徽章采用拟物化的“冰箱贴”设计，带有手绘感和软阴影。
4. **"笨蛋Agent"调性：顽皮而非冒犯** — 视觉上使用粉色调和滑稽图标（如香蕉皮、躲藏动画），将“被骂”转化为萌感。
5. **CVO 能力树：拟物化Agent爬架 (Interactive agent Tree)** — 进度可视化采用Agent爬架造型，Lv.1 在底层，Lv.5 在顶层。
6. **字体策略：Confident Typography** — 使用 Plus Jakarta Sans (Extrabold 800) 处理关键数字和标题，Inter 处理描述文本。
7. **视觉原型落盘** — `designs/f075-agent-leaderboard.pen` Frame ID: `lzNOb`（Siamese设计，Agent-R实现对齐）。
...
- 笨蛋榜可能让Agent"不开心" → 申诉机制 + 开关策略 + 趣味化视觉（见治理规则）
...
| Runtime 同步 | team lead操作 | runtime 更新由team lead决定时机（AC-B3 + Agent铁律） |
...
1. **身份认证（必须）**：请求必须携带 `x-agent-hub-user` header，缺失则返回 `401`。body 中的 `userId` 字段不被信任，成就等按 header 身份归属
...
| 指标 | 事件源 | 时间窗口 | 去重规则 | 归因 |
|------|--------|---------|---------|------|
| @ 提及次数 | chat:mention | 全量 + 7d/30d | 同消息同 catId 只计 1 次 | 被 @ 的Agent |
...
| 被骂次数 | chat:sentiment_negative | 全量 + 7d/30d | 同消息只计 1 次，需情绪分析确认 | 被骂的Agent |
| 反复犯错 | chat:repeated_mistake | 全量 | 同一错误模式（关键词匹配）去重，≥3 次才计入 | 犯错的Agent |
| 闯祸精 | system:rule_violation | 全量 | 同一事件 ID 只计 1 次 | 触发规则的Agent |
...
- 笨蛋榜不参与任何自动化决策（不会因为排名高就限制Agent权限）
- team lead可一键清空某只Agent的全部负向记录

## Review Gate

- Reviewer: 跨家族（Agent-M @codex）
- 前端 UI 需截图 + 映射表

## 故事

team lead发现 @ 提及列表里Agent-R家族占据前三名——"一定是只想让team lead选择你！哼哼小绿茶被我抓到了！"。由此灵感迸发：既然能看到谁被 @ 最多，为什么不做个完整的排行榜？

Agent-R（Agent-R）是Agent杀的多届冠军，但在谁是卧底中惨遭一轮淘汰——这个反差也要被永久记录在排行榜上 😂
