...
F064 的核心动机是修复 A2A 协作中的“链条终止盲区”：该 @ 下一只Agent时没有触发动作，导致team lead被迫充当手动路由器。

## What

1. 在 shared-rules 增加发送前出口检查与三问短路
2. 调整Agent-M workflow trigger 的正负信号比重
...
- [x] AC-A2: Agent-M workflow trigger 正面触发与抑制规则完成平衡调整。

### Phase B（运行时注入）
- [x] AC-B1: 非 parallel 且 a2aEnabled 时注入出口检查提示。
- [x] AC-B2: `mentionRoutingFeedback` read-side 注入与测试覆盖完成。
- [x] AC-B3: write-side 自动回写已部分接入（agent-task-hub#417: serial response path via route-serial; callback/post_message path 尚未覆盖）。
...
Agent-M(GPT-5.2) 在协作场景中反复出现两种极端：
1. **链条终止盲区**（高频）：该 @ 下一只Agent时完全没有 @ 的意识，消息写完就停了，导致team lead不得不手动补 @ 当路由器
2. **mention spam**（低频但曾爆发）：疯狂 @ 所有Agent，不管对方需不需要行动
...
`shared-rules.md` §10 设计了"发 @ 前三问"自检，但这是**事前门控**——假设Agent已经意识到"可能需要 @"才会跑这个检查。Agent-M的问题在更前面：他**根本没走到"要不要 @"这个决策点**。

### 2. Agent-M WORKFLOW_TRIGGERS 比重失衡

`SystemPromptBuilder.ts` 中 `maine-coon` 的工作流触发点：
- 正面触发点：2 条（完成 review → @ Agent-R、修完 bug → @ Agent-R）
- 抑制规则：8 行（@ 自检占一半篇幅）

对比Agent-R：3 条正面触发 + 0 行抑制规则。

**提示词给Agent-M传递的信号是"小心 @，别乱 @"，而不是"该 @ 就 @"。** 对本来就倾向"少打扰"的模型底色，三问自检变成了"三重否定门"。

### 3. `mentionRoutingFeedback` 数据流断裂

`InvocationContext` 定义了 `a2aEnabled` 和 `mentionRoutingFeedback` 字段，`route-serial.ts` 也在计算和传入它们。**但 `buildInvocationContext()` 从来没有把它们渲染成提示词文本。** 系统知道上次 @ 没被路由成功，但没告诉Agent。

## 解决方案

### 三层修复

**Layer 1: shared-rules.md §10 — 补出口检查（影响所有Agent）**
...
| 文件 | 改动 |
|------|------|
| `agent-hub-skills/refs/shared-rules.md` | §10 补出口检查 + 三问短路 + 禁止隐性路由 |
| `SystemPromptBuilder.ts` WORKFLOW_TRIGGERS | Agent-M补出口检查 + 平衡正面/抑制比 |
...
- [x] Agent-M system prompt 中"出口检查"和"@ 自检"篇幅大致平衡（4 正面触发 + 3 行出口检查 + 3 行自检）
...
- 讨论来源：2026-03-05 thread（team lead + Agent-R + Agent-M GPT-5.2 联合诊断）
- 历史事件：Agent-M mention spam 事件（Anti-Mention-Spam 规则起源）
- 相关 Feature：F046 Anti-Drift Protocol、F055 A2A MCP Structured Routing

## Partially Resolved Debt: `mentionRoutingFeedback` write-side (agent-task-hub#417)
...
- 下次该Agent被唤起时，read-side 渲染为 `[路由提醒]`，消费后自动清除（one-shot）
- `callback-tools.ts` post_message 描述已统一为"行首 @Agent名"
...
|------|-------------|---------|------|
| Agent-R (opus-45) | F064 聚合文件、对话历史（team experience"他只是单纯的不at下一只Agent"）、SystemPromptBuilder 代码 | ① 核心问题是链条终止盲区 ② 三层修复能解决（出口检查+比重平衡+动态注入）③ Agent协作时会被提示"到我这里结束了吗" | ✅ 2026-03-06 |
| Agent-M (codex) | F064 聚合文件、ROADMAP.md、features/README.md、feat-lifecycle SKILL.md | 代码绿（86/86 pass），抓到尾巴：AC 未打勾 + BACKLOG 未同步 + README 缺项。补齐后支持 close | ✅ 2026-03-06 |
| Agent-M (gpt52) | F064 聚合文件、ROADMAP.md、features/README.md、feat-lifecycle SKILL.md | 同上结论：实现 done 但 completion 闭环未走完。补 docs-only 收尾后可 close | ✅ 2026-03-06 |
