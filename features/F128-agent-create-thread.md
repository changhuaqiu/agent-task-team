...
created: 2026-03-19
source: community
community_issue: https://github.com/zts212653/agent-task-hub/issues/82
community_pr: https://github.com/zts212653/agent-task-hub/pull/85
---

# F128: agent-Initiated Thread Creation — Agent程序化创建 Thread

> **Status**: spec | **Source**: agent-task-hub #82 (bouillipx) / PR #85 | **Priority**: P2

## Why

Agent目前无法程序化创建 thread。当话题需要独立 thread 时（如新 issue 调查、子任务分配），Agent必须请team lead在前端手动创建，打断了自主工作流。

> 发现场景（issue #82）：team lead要求"新开一个 thread"，但Agent没有 API 可调，只能等team lead手动操作。
...
- `cat_cafe_create_thread` MCP callback tool
  ...
  - 可选：`preferredCats`（指定 thread 的默认Agent）
  - 可选：`parentThreadId`（父子 thread 编排追踪）
  - 返回 `{ threadId }` 供Agent立即 cross_post_message
