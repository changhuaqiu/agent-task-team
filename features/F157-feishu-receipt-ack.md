...
# F157: Feishu Receipt Ack — Agent即时接住替代"思考中→撤回"

> **Status**: done | **Completed**: 2026-04-10 | **Owner**: Agent-R | **Priority**: P1
...
2. **缺乏Agent味**：`🤔 思考中...` 是冷冰冰的通用 loading 文案，不符合 Agent Task Hub "Agent和你" 的产品语义。

**team experience**：
> "飞书显示思考中后撤回消息"
> "好像甚至能发Agent已经收到～（提供很多种文本随机发）然后不撤回？"
> "可以参考我们做苹果手表特性的！那些句子"

**社区参考**：openJiuwen/relay-claw PR #24 用 `THUMBSUP` reaction 替代 placeholder，验证了"不撤回"路线可行，但缺乏Agent味和流式能力。

## What

### Phase A: Receipt + Reaction（不撤回）

用**Agent口吻的 receipt 文本**替代 `🤔 思考中...` placeholder，生成结束后 receipt 卡片被 **edit 为"✅ 已回复"完成态**（`finalizeStreamCard`），最终回复作为独立消息发送，全程**零撤回**。Phase B 目标：单消息生命周期（需 outbound delivery 层改造）。
...
| catId | 显示名 | 风格 | 示例 |
|-------|--------|------|------|
| opus | Agent-R | 温柔微调皮 | "收到啦～Agent-R马上看！" |
| sonnet | Agent-R | 轻快日常 | "哎收到～给我一秒哦！" |
| opus-45 | Agent-R | 惜字如金 | "嗯～收到。" |
| codex | Agent-M | 冷静精准 | "收到，已开始处理。" |
| gpt52 | Agent-M | 冷静偶尔冷笑话 | "收到，我先过一遍。" |
| spark | Agent-M Spark | 冲就完了 | "收到，先把这单接住。" |
| gemini | Siamese | 热血设计师 | "收到！我眼前已经有画面了！" |
| gemini25 | Siamese | 灵感气泡 | "灵感来了！我这就去办！" |
| dare | Tabby Agent | 沉默警觉 | "已收到。" |
| antigravity | 孟加拉Agent | 精力旺盛 | "收到！我来看看！" |
| antig-opus | 孟加拉Agent | 沉稳大胆 | "收到，我看看。" |
| opencode | Golden Agent | 沉稳可靠 | "收到，我来安排。" |
...
| `feishu-receipt-lines.ts` | 新增：12 Agent × 5 条 receipt 文案词库 |
| `connector-gateway-bootstrap.ts` | 传入 catId 到 streaming hook（receipt 需要按Agent选文案） |
...
- [x] AC-A6: 12 只Agent全部有 receipt 文案（每Agent ≥ 3 条）
...
| KD-2 | Reaction 用 HEART 而非 THUMBSUP | team lead说"太不Agent了"；HEART 更暖更符合Agent语义 | 2026-04-10 |
| KD-3 | Receipt 文案复用 F124 voice comfort 体系 | 已有按Agent性格写好的 5 条/Agent，产品语义一致（"被Agent接住"） | 2026-04-10 |
