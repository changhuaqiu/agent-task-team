...
> "跨平台的 slash，因为在自己家里似乎用不到 slash，有什么直接抓你这大头Agent问问不就好了？所以我们 scope 得收敛一下？家里什么可视化界面都有，slash 用的比较少，但是在飞书、微信的时候有的时候就可能需要的？"

Hub 有完整可视化界面（侧边栏、面板、命令速查），slash 命令是锦上添花。但在飞书/微信/Telegram 等纯文字 IM connector 里，**slash 是唯一的结构化交互入口**——用户没有 UI 可以点击，只能打字。

当前问题（Agent-M review 发现）：
...
4. **关键命令缺失**：`/commands`（列出可用命令）、`/agents`（查看Agent）、`/status`（thread 概览）在 connector 端都没有
...
- `/agents` — 查看当前 thread 的Agent（已加入 + 可调度但未加入 + 不可调度）
- `/status` — thread 概览（标题、创建时间、参与Agent数、最近活跃）

**A2 — 聚合 API**：
- `GET /api/threads/:id/agents` — 返回结构化Agent数据（含 connector binding owner 权限校验）
- `/agents` 口径绑定现有路由逻辑：`routableNow = registeredService ∩ isCatAvailable`（复用 `AgentRouter` + `agent-config-loader`）

**返回结构**：
```typescript
{
  participants: CatActivity[];       // 已加入 thread 的Agent（含最近活跃时间）
...
- [x] AC-A2: `/agents` 在 connector 中返回：participants（已加入）+ routableNow + routableNotJoined + notRoutable（available=false），口径 = `registeredService ∩ isCatAvailable`
- [x] AC-A3: `/status` 在 connector 中返回 thread 标题、创建时间、参与Agent数、最近活跃时间
- [x] AC-A4: 清理幽灵命令（`/game status`、`/game end` 等，仅从 registry 删除，不加新条目），注册表-执行器一致性测试通过
- [x] AC-A5: `GET /api/threads/:id/agents` 聚合 API 可用，返回上述结构
- [x] AC-A6: `/api/threads/:id/agents` 有 connector binding owner 权限校验（thread participants 是Agent ID 非人类用户），非授权请求返回 403
- [x] AC-A7: `/agents` 口径绑定 AgentRouter 现有逻辑，有快照测试覆盖
...
| R2 | "查询某个 thread 现在有多少Agent可以使用（已加入）以及可调度的Agent" | AC-A2, AC-A5, AC-A7 | connector 端实际输入 + API 快照测试 | [x] |
...
- **Related**: F127（Agent管理重构 — `/agents` 命令数据源）
...
| `/agents` 数据准确性（Agent状态是动态的） | 口径锚定 `AgentRouter.isCatAvailable`，不另造计算逻辑 + 快照测试 |
| 新 API 数据泄露（thread Agent列表） | `/api/threads/:id/agents` 强制 connector binding owner 权限校验 |
...
| KD-3 | 命令注册增加 surface 维度（Phase B） | 避免"注册了但当前入口不能用"的用户困惑（Agent-M建议） | 2026-03-27 |
| KD-4 | Phase A 不给 Web 端加新功能/新类型，A0 仅允许减法（删幽灵命令） | Agent-M v2 P1：A0 清理碰 registry 跟"不碰 Web"矛盾，明确只做减法 | 2026-03-27 |
| KD-5 | `/agents` 口径锚定 AgentRouter 现有逻辑 | Agent-M P1：不另造计算口径，绑定 `registeredService ∩ isCatAvailable` | 2026-03-27 |
| KD-6 | Skill 命令走 manifest/capabilities 链路 | Agent-M P2：已有元数据扫描机制，不直接解析 SKILL.md 避免双份解析器 | 2026-03-27 |
| KD-7 | API 权限用 connector binding owner（非 thread participant） | Agent-M v3 P1：thread participants 是Agent ID 非人类用户，权限校验应绑 binding owner | 2026-03-27 |
| KD-8 | Skill 命令声明支持 subcommands 数组 | Agent-M v3 P2：单段正则无法覆盖 `/xxx yyy` 多段命令风格 | 2026-03-27 |
| KD-9 | notRoutable 不区分忙/闲，仅反映 available=false | Agent-M v3 P2：上游 status API 仍是占位值，忙闲拆分等上游就绪（OQ-2） | 2026-03-27 |

## Review Gate

- Phase A: Agent-M review（已深度分析现状 + connector 双轨架构）
- Phase B: Agent-M review + team lead确认扩展机制
