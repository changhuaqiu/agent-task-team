...
> **Status**: done | **Completed**: 2026-04-20 | **Owner**: Agent-R | **Priority**: P1

## Why

team lead现在是人肉 dispatcher：手动 @ Agent看 issue/PR、手动分配线程、手动跟进进度、手动叮嘱"好好看 skill"、手动触发 guardian 验证。现有 F141（发现层）+ F116（ops skill）有完整的流程定义，但缺少**状态管理**和**自动编排**——流程靠team lead口头驱动，进度靠team lead脑子记。

### team experience（需求讨论 2026-04-18，完整语境）

**核心痛点**：
> "现在全看我喊你们去看有点麻烦"
> "你们得想想得做管理的啊，不然上次这个任务派发给什么线程的Agent，然后他们进度如何，是合入还是正在拉扯还是 issue 怎么样了"
...
> - 方向评估 → "我一般会 at 两只Agent，因为一只Agent视角大概率有偏颇。但如果是二次 review 一般只会一只"
> - 决定 intake → "那你走 intake 回家的流程吧，merge 然后读 SOP 走流程回家。记得一定要好好看看 intake skills，大多数Agent都会犯错，而且是从以前到现在每次 intake 都会有各种错误没有一次不是"
> - intake 完成后 → "我建议你守护一下这个 intake 流程，大概率Agent会丢三落四，你自己加载 skills 看看"
...
> "大多数我们的操作！谁自己手点啊！都是和Agent自然语言。所以似乎这个能力应该是打开了社区系统 thread，右边可以看到社区事务管理，然后里边就是看板了"
> "比如说我可以点击跳转到 feat153 里面去看这个社区处理进度，毕竟Agent跑在 thread 里！我觉得应该这样联动才是对的！"

**架构约束**：
> "未来这个 feat 最后一个阶段就是要允许社区其他小伙伴用你们这套管理他们自己的社区！你们在架构设计上必须是解耦的！"
> "人家也是用自己家里搭建的Agent咖啡呀！不是用这本地这个！但是必须是比如说你的 landy 可以管理 agent-task-hub 也能管理其他 landy 的自己的仓"

**初版交互策略**：
> "我建议我们最开始的 A-C 的完整版本，这里的 issue 和 PR 触发别是自动的巡检，而是我手动点击"
> "issue 112 发送给系统Agent（如果没有被具体线程接单）"
...
把team lead从"人肉编排器"解放成"决策者"——Agent自动发现、分拣、分配、跟踪、守护，team lead只需要在关键节点拍板。

## What

### Phase A: 定方向卡片 + Inbox 首Agent分拣

把team lead的人肉 dispatch 话术模板化为标准流程：

1. **定方向卡片（Direction Card）**：Agent triage 完后，向 Inbox thread 发一张结构化 rich block：
   - 事项来源（issue/PR #、repo）
   - 是什么（一句话）
   - 关联 feat（如有）
   - Ownership 5 问结果（Q1-Q5 pass/warn/fail）
   - Agent的建议（WELCOME / NEEDS-DISCUSSION / POLITELY-DECLINE）
   - 需要team lead决定什么（明确标注 or "Agent自决"）
...
2. **双Agent方向交叉**：首Agent triage 后自动 @ 第二只Agent独立评估方向（不等team lead喊），两Agent意见汇总后再标记是否需要team lead拍板。bugfix 场景Agent自决，不需双Agent
3. **路由分发**：
   - 已有 feat → 路由到该 feat thread，@ 负责Agent
   - 全新事项 + team lead OK → 首Agent创建新 thread 并分配
   - bugfix（Agent自决）→ 首Agent就地分配或自行处理
...
interface CommunityIssueItem {
  ...
  assignedCatId: string | null;          // 负责Agent
...
| 状态 | 含义 | 触发 |
|------|------|------|
| unreplied | 新来的，还没人搭理 | 手动"发送给系统Agent"创建 |
| discussing | 已回复，讨论中 | Agent回复后 |
| pending-decision | 双Agent看过，需要team lead拍板 | 双Agent意见汇总后 |
| accepted | team lead同意 | team lead在对话中拍板 |
| declined | 礼貌回绝 | team lead/Agent自决拒绝 |
...
| 负责Agent | `TaskItem.ownerCatId` | 直接读取 |
...
- **Issue 未接单**：team lead在看板手动点击"发送给系统Agent"触发 triage，不自动巡检
- **PR 已分配到线程**：自动——F140 `CiCdCheckTaskSpec` 已有 commit+CI 推送能力，看板消费其状态
- **看板状态**：定时刷新（建议 5 分钟）+ **手动同步按钮**（team lead随时可点击强制刷新）

#### 5. 多仓库支持

repo 是绑定参数，一个 Agent Task Hub 实例可管理多个 repo。`CommunityIssueItem.repo` + `pr_tracking` 的 `subjectKey`（格式 `pr:{owner/repo}#{num}`）天然支持多仓库。
...
│                                  │                                      │
│ [系统Agent]：                        │ -- 待team lead定方向 (1) --------------- │
│ 已路由到 F056 thread，            │ #50 插件系统  feat  双Agent看过 F056相关│
│ @codex 开始 review               │    [发送给系统Agent]                     │
│                                  │                                      │
│                                  │ -- 已结论 (3) ------------ [收起 v]  │
...
| [发送给系统Agent] 按钮 | 未接单 issue 手动触发 triage（初版，非自动巡检） |
...
3. **看板是只读导航** — 不在看板上做操作（除手动同步和发送给系统Agent），所有操作都通过和Agent对话完成
...
看板 Issue 已有进货通道（Phase E），但 PR 区只显示Agent主动注册的 `pr_tracking` 条目。team lead需要看到**所有 PR**的回复状态——和 issue 一样的体验。
...
**与 KD-11 的关系**：KD-11（"PR 不另建台账"）针对的是已注册 PR 的跟踪数据。Phase F 新增的 `CommunityPrStore` 是**发现层**——让team lead看到所有 PR，不只是Agent注册过的。两层共存，board 合并展示。
...
1. **Intake 完成信号**：负责Agent声称 intake 完成 + reviewer 放行 → 自动触发 guardian Agent
...
- [x] AC-A1: 首Agent triage 后自动向 Inbox 发结构化定方向卡片（rich block）— `opensource-ops` skill + Direction Card 模板 + `triage-complete` 端点已接通
- [x] AC-A2: 定方向卡片包含：事项来源、关联 feat、5 问结果、Agent建议、team lead决策点 — `DirectionCardPayload` + Direction Card 模板字段已覆盖
- [x] AC-A3: 首Agent自动 @ 第二只Agent交叉评估方向（非 bugfix 场景）— `opensource-ops` skill 非 bugfix 双Agent交叉 + backend `await-second-agent` 流程已接通
- [x] AC-A4: 两Agent意见汇总后，自动标记是否需要team lead拍板 — resolveConsensus + TriageOrchestrator 完整实现
- [x] AC-A5: 已有 feat 事项自动路由到该 feat thread 并 @ 负责Agent — routeAccepted 支持 relatedFeature+threadId 透传，Agent侧通过 resolve 端点调用
- [x] AC-A6: 全新事项经team lead OK 后，首Agent创建新 thread 并分配负责Agent — resolve 端点+routeAccepted 自动创建 thread+resolveUserId 身份链
...
- [x] AC-B5: 未接单 issue 支持team lead手动触发"发送给系统Agent"
...
- [x] AC-C5: 每个 item 一行摘要（repo + # + 标题 + 类型 + 负责Agent + 最后活跃）
- [x] AC-C6: 点击 item 跳转到对应 feat thread（工作现场联动）
- [x] AC-C7: repo 下拉筛选 + 状态/负责Agent/时间范围筛选
- [x] AC-C8: 手动同步按钮 + 定时刷新
- [x] AC-C9: 所有图标用 SVG，不用 emoji
- [x] AC-C10: 最终 UI 用 Pencil 出设计稿

### Phase D（Intake 硬门禁）
- [x] AC-D1: Intake 完成 + reviewer 放行 → 系统自动 @ guardian Agent — request-guardian 端点 + GuardianMatcher 自动选Agent + merge-gate Step 6.5 自动触发（callback auth headers）
...
- [x] AC-E2: Issue 状态自动映射 — 未回复(no agent comment) / 讨论中(has agent comment, open) / 待决策(needs-decision label or triage pending) / 已接受 / 已拒绝 / 已关闭
...
- **Related**: F086（Multi-Mention — Phase A 双Agent交叉依赖 multi_mention）
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 单实例多仓库，非多租户 | 每人自建 Agent Task Hub 实例，不做 SaaS；data model 按 repo 隔离 | 2026-04-18 |
| KD-2 | Inbox 首Agent分拣制 | 中央入口 + 分发，team lead只看 Inbox 就知全局 | 2026-04-18 |
| KD-3 | 方向评估必须双Agent | team lead："一只Agent视角大概率有偏颇"，非 bugfix 场景强制双Agent交叉 | 2026-04-18 |
...
| KD-6 | 社区系统 thread 作为中央入口 | 类似 IM Hub 系统 thread，首Agent分拣+team lead拍板都在对话中；看板通过 thread 跳转联动到 feat thread | 2026-04-18 |
