...
>
> 在小艺开放平台创建 OpenClaw 模式智能体，由 Agent Task Hub 通过 WebSocket 对接华为 HAG，
> 用户在华为手机上通过小艺 APP 即可与Agent对话。
>
> Agent Task Hub 不是 OpenClaw 实例 — 它是对接小艺 OpenClaw 模式协议的 connector adapter。

## Why

Agent Task Hub 已通过 F088/F132/F137 接入飞书、Telegram、DingTalk、WeCom、WeChat 五个渠道。
华为小艺是 HarmonyOS 设备的原生 AI 助手，覆盖手机/平板/手表/车机。
接入小艺渠道意味着 Agent Task Hub 的Agent可以在**所有华为设备上**被用户直接使用。
...
3. Agent Task Hub 直接作为 WebSocket 客户端连接 HAG，用户无需额外部署
...
```
用户 → 小艺 APP → 华为 HAG Server ←──WebSocket──→ XiaoYiAdapter (Agent Task Hub)
                  (wss://hag.cloud.huawei.com        │
                   /openclaw/v1/ws/link)              ├→ Connector Gateway
                                                      │   ├→ Principal Link
                                                      │   ├→ Session Binding
                                                      │   └→ Command Layer
                                                      └→ Agent Router → agent Agents
```

连接方向：**Agent Task Hub 主动连接华为 HAG**（类似 DingTalk Stream 模式）。

### 核心 ID

| ID | 来源 | 生命周期 | 用途 |
|----|------|---------|------|
| `params.sessionId` | 华为 HAG 下发 | 跨 app 重启稳定 | 对话标识 → 映射到 Agent Task Hub thread |
...
| 投递 | 非流式 — 每只Agent完成后 `artifact-update(text, lastChunk=true)` 一次性推送，首只 `append=false`，后续 `append=true` 累积。首次触发前发 `reasoningText` 思考气泡。close frame `status-update(completed, final=true)` 关闭 task |
...
```json
{
  ...
  "result": {
    ...
    "final": false,
    "artifact": {
      "artifactId": "agent-Agent-R-1714600000000",
      "parts": [{ "kind": "text", "text": "你好！" }]
    }
  }
}
...
**放弃 per-artifact 流式推送**，改为每只Agent完成后一次性投递完整文本。
...
3. Agent处理中 → `status-update(working)` 每 20s keepalive
4. Agent完成 → `artifact-update(text, 完整回复, append: false, lastChunk: true)` — 首只Agent
5. 下一只Agent完成 → `artifact-update(text, \n\n---\n\n+完整回复, append: true, lastChunk: true)` — 追加
...
**多Agent投递**（append 累积模型）：
```
用户发消息 → HAG 下发 message/stream (taskId=T1)
Agent Task Hub 路由给Agent A + Agent B:
...
Agent A 完成:
  → artifact-update(text, "AgentA回复", append=false, lastChunk=true)
      ↑ 首只Agent: append=false

Agent B 完成:
  → artifact-update(text, "\n\n---\n\nAgentB回复", append=true, lastChunk=true)
      ↑ 后续Agent: append=true 追加到Agent A 后面

onDeliveryBatchDone(chainDone=true):
  → status-update(completed, final=true) ← close frame
```

`hasArtifact` Set 追踪 task 是否已发过 text artifact，决定首只Agent `append=false` vs 后续Agent `append=true`。
...
1. **XiaoYiAdapter** — 新增 connector adapter
   ...
   - **非流式 append 累积模型**：首只Agent `append=false`，后续Agent `append=true` 追加。`reasoningText` 思考气泡立即反馈。per-session FIFO 队列 (`taskQueue`) 管理用户连续消息。`onDeliveryBatchDone(chainDone=true)` 信号触发 close frame 关闭 task
   - **协议层抽离**：`xiaoyi-protocol.ts` — 常量、类型、auth、message builders（质量门控文件大小合规）

2. **协议层**
   - `clawd_bot_init` 注册
   - `message/stream` 入站解析 → 标准 InboundMessage
   - `agent_response` 出站格式化：`status-update(working)` → `reasoningText` 思考气泡 → Agent完成后 `artifact-update(text)` 逐Agent投递 → close frame `status-update(completed, final: true)` 关闭
...
- [ ] AC-A4: 用户在小艺 APP 发送文本，Agent收到并回复，小艺端展示回复
- [ ] AC-A5: 非流式投递 — `reasoningText` 思考气泡即时反馈 + 每只Agent完成后 `artifact-update(text)` 一次性投递（首只 `append=false`，后续 `append=true` 分隔线累积）
...
- [ ] AC-A9: 多Agent投递完整性 — 首只Agent `append=false`，后续Agent `append=true` 追加（`---` 分隔线），close frame 仅在 `onDeliveryBatchDone(chainDone=true)` 时发出，task 不提前关闭
...
| 2 | Agent Task Hub 内置适配而非部署 OpenClaw 实例 | 减少用户运维负担，一跳直连 | 2026-04-01 |
...
| 6 | adapter 内置 task 生命周期管理 | 流式出站必须带 taskId 回包；现有 `IStreamableOutboundAdapter` 接口不携带此上下文，由 adapter 内部 FIFO 队列 + invocation 级绑定管理（Agent-M review R4/R5） | 2026-04-01 |
| 7 | `externalChatId` 带 `agentId:` 前缀 | 隔离命名空间，确保 binding key 全局唯一（Agent-M review） | 2026-04-01 |
...
| 9 | ~~多Agent replyParts 聚合~~ → per-delivery artifact | **D9-v1 已废弃**。原以为"一个 task 只能有一个 artifact"，但华为官方文档明确："一次会话请求(final为True结束)，允许若干流式输出"。每次 `sendReply` / streaming 会话用独立 `artifactId`（adapter 不感知 catId），无需合并。删除 `replyParts` / `claimedTasks` / `activeTask` | 2026-04-05 |
...
| 13 | ~~per-delivery artifact（多 artifactId 独立输出）~~ → append 累积 | **D9-v2 已废弃**。真机验证发现：HAG app 对 `text` 类型 artifact，无论 `artifactId` 是否不同，`append=false` 时只渲染最后一个 — 后面Agent的回复覆盖前面的。协议结构上多 artifact 合法，但 app 渲染行为是"最后一个 wins"。改为 append 累积模型：首只Agent `append=false`，后续Agent `append=true`（2026-04-06 真机验证 append=true 跨 artifactId 累积有效） | 2026-04-06 |
| 14 | ~~per-artifact 流式推送（editMessage delta）~~ → 非流式 | 真机验证发现两个问题：① 同一 `artifactId` 内 `append=true` 发 delta，HAG app 显示前一个 chunk 被覆盖而非追加；② 改为 `append=false` 每次发全量文本，单Agent可行但多Agent场景下与 D13 的 `append=true` 累积冲突（首只Agent流式用 `append=false` 会覆盖后续Agent的累积内容）。放弃 intra-artifact 流式，`sendPlaceholder`/`editMessage`/`deleteMessage` 均为 no-op | 2026-04-06 |
...
- Phase A: Agent-M review + team lead真机验证
