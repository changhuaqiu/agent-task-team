...
> **Status**: done | **Owner**: Agent-R | **Priority**: P1 | **Phase A-D Completed**: 2026-03-27 | **Reopened**: 2026-04-24（Phase E — 通知合流：severity 抽取 + 下线 email 路径） | **Completed**: 2026-04-25
...
> 胡兴哲："Agent挂 webhook，收到冲突，自动处理...比如别人 MR 了以后，我的代码有一条 message 是冲突，这块好像要增强一下"
...
**现状 Gap**：F133 解决了 CI/CD 状态追踪，但 PR 冲突检测和全来源 review feedback 感知仍未闭环。F139 Phase 1a 已交付统一调度框架（TaskRunnerV2 + TaskSpec_P1），并注册了 `conflict-check` 和 `review-comments` 的骨架（gate 能感知，execute 是 stub）。本 Feature 补完 execute 层：投递 + 唤醒Agent + 行为引导。
...
- `ConnectorInvokeTrigger` urgent 唤醒Agent
...
- 覆盖所有来源：Codex 云端 review、人类 reviewer、Agent通过 `gh pr review` 留的 comments
- 投递到 thread + 唤醒Agent
...
- `merge-gate` SKILL.md：告知Agent注册 PR 后会收到三类通知（CI + 冲突 + review feedback）
...
Agent收到冲突/review feedback 通知后的操作引导——消息级 action hints + Skill 行为决策树，Agent据此知道该做什么并按 Skill 流程执行：
...
> **注**：Phase B 是引导层——Agent看到 action hint 后仍需按 Skill 流程手动执行操作。真正的零点击自动执行器（代码层面自动 rebase + push + 处理 review）见 Phase C。

### Phase C: 自动执行器 (Auto-executor) ✅

Agent收到通知后**零人工干预自动执行**：

**1. 冲突自动 resolve**
- Agent收到冲突通知 → 在 worktree 中 `git fetch origin main && git rebase origin/main`
- 自动解决简单冲突 → push → 等下一轮 CI 通知
- 复杂冲突（无法自动 resolve）→ 通知team lead

**2. Review feedback 自动处理**
- Agent收到 review feedback 通知 → 自动加载 receive-review 模式 → 逐项处理
...
>
> **守护**：不硬编码 `zts212653/agent-hub`，用 `gh repo view` 动态校验。合法 repo 全放行，非法 repo 全拦截。
>
> **根因**：2026-03-25 一次 merge-gate 注册了 `anthropic-agent-hub/agent-hub#743`（repo 不存在），脏数据驻留导致 CI/CD Check 轮询假仓库。
...
- [x] AC-A2: 冲突消息通过 ConnectorInvokeTrigger urgent 唤醒Agent
...
- [x] AC-A5: Review feedback 唤醒Agent处理
...
- [x] AC-C1: Agent收到冲突通知后零人工干预自动 rebase + push（clean rebase 场景）
- [x] AC-C2: 简单冲突（≤3 文件，non-binary）自动 resolve，复杂冲突通知team lead附冲突文件列表
- [x] AC-C3: Agent收到 review feedback 后自动加载 receive-review 模式处理（CHANGES_REQUESTED 场景）— suggestedSkill routing wired，full auto-processing deferred（intent is hint not constraint）
...
- [x] AC-E10: 代码清理（独立 PR #1398, squash 397df85c）— 删除 11 文件（6 src: GithubReviewWatcher / github-review-bootstrap / ReviewRouter / ReviewContentFetcher / GithubReviewMailParser / ProcessedEmailStore + 5 tests）+ 清 `infrastructure/email/index.ts` 8 组 deprecated re-exports + 清 `src/index.ts` E.2 残注释 + 6 处其他文件残留注释。`github-feedback-filter.ts` Rule A only 已在 E.2 完成。Agent-M GPT-5.5 双轮 review (P2 6 处注释残留 → fix → no-findings) + 云端 codex "Swish! no major issues" — SHA 397df85c
...
| KD-4 | ReviewFeedbackRouter（非 ReviewCommentsRouter）| Agent-M指出：contributor 在乎的不是"有没有 comment"，而是"review feedback 有没有改变 PR 的下一步动作"。只追 comments 不追 decision，信息不完整 | 2026-03-26 |
...
| KD-6 | Skill/SOP 更新是 Phase A 必须组件 | team lead指出：技术管道建了没有行为引导 = 通知发了Agent不知道怎么处理 = 等于没做。F133 Phase B 就是做这件事 | 2026-03-26 |
| KD-7 | F140 定位为追踪层（PR Signals），发现层（Repo Inbox）独立为 F141 | team lead确认分开立项，可并发开发 | 2026-03-26 |
| KD-8 | PrComment → PrFeedbackComment（richer model：+author/filePath/line/commentType） | Agent-M P1：现有 PrComment 只有 id/body/createdAt，支撑不了分区展示的消息格式 | 2026-03-26 |
| KD-9 | Conflict fingerprint 在 MERGEABLE 时清除 | Agent-M P2：同一 headSha 因 base 变化再次冲突会被误 dedupe。检测到 MERGEABLE → 清 lastConflictFingerprint，下次 CONFLICTING 重新通知 | 2026-03-26 |
| KD-10 | Cursor commit 在 delivery 成功后，trigger 是 best-effort | Agent-M P3：delivery 成功 = 主 side-effect 完成 → 立即 commitCursor。trigger() 失败不阻塞 cursor 推进，避免重发已投递消息 | 2026-03-26 |
...
| KD-15 | Phase E cutover 时**删除** Rule B（authoritative-source 语义），不是迁移 | Agent-M GPT-5.4 Design Gate P1 push back（2026-04-24）：Rule B 本来就在 polling 侧（`shouldSkipComment/shouldSkipReview`），email watcher 只用 `isSelfAuthored`（Rule A）。Cutover 后 polling 是唯一真相源，继续 skip "authoritative bot feedback" = bot review/inline comment 直接消失。只保留 Rule A（self-authored skip） | 2026-04-24 |
| KD-16 | Severity parser 严格格式 + FP 护栏 | Agent-M指出现有 `\bP([0-3])\b` 会吃 `MP3`/`P100`/句内裸词且识别 P3（informational 不应进消息头）。采用三种严格格式（badge / 行首方括号 / 行首冒号）+ 排除代码块和 blockquote + 至少 5 条负例测试 | 2026-04-24 |
| KD-17 | E.3 代码清理以"3 场景证据门槛"触发，不以时间窗口 | Agent-M P2：alpha 过 bot-P2 / bot-pass / 人类-CHANGES 三场景后才清，比"观察一周"更可执行。避免时间窗口既保守又不精确 | 2026-04-24 |
...
**参与者**: Agent-R (@opus) + Agent-M (@gpt52, GPT-5.4)
**日期**: 2026-03-26
...
- Phase B: Agent-M (codex/spark) cross-family review — 放行, 无 P1/P2
- Phase B+ dedup fix: Agent-M (codex/spark) cross-family review — 三审放行（P1×2 修复后）, 无 P1/P2
- Phase C: Agent-M (codex/spark) R1 review — 3 P1 发现 + 修复确认放行。云端 Codex R2 — "No major issues"
- Phase D: Agent-M (codex/spark) cross-family review — 放行, 无 P1/P2。云端 Codex R1 1 P1（catch-all→区分 infra failure）修复后 R2 通过
- Phase E.1: Agent-M (gpt52 + codex) cross-family review + 云端 Codex — P0/P1/P2 修复后通过
- Phase E.2: Agent-M (gpt52 + codex) cross-family review + 云端 Codex — P2 注释残留修复后通过
- Phase E.3: Agent-M GPT-5.5 双轮 review + 云端 Codex — P2 注释残留修复后 no-findings
