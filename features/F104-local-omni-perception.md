...
当前Agent的感知管道由三个独立小模型串行组成（ASR 1.7B + LLM后修 4B + TTS 1.7B），总延迟高、能力有限、且Agent完全没有视觉理解能力。team lead的 128GB M4 Max 有充足算力跑 MoE 30B 级模型（3B 激活），升级后Agent从"只会读写的文字 AI"变成"能听能看能说的全感知伙伴"。
...
- **现状**：语音输入 → Qwen3-ASR-1.7B(9876) → Qwen3-4B-Instruct 后修(9878) → 文本 → Agent处理 → Qwen3-TTS-1.7B(9879) → 语音输出
...
4. 保持 clone voice 能力（Admin声线不能丢，关联 F103）
5. 回退机制：Omni 不可用时 fallback 到现有三模型管道

### Phase B: VL 视觉理解 — Agent获得"眼睛"

用 **Qwen3.5-35B-A3B**（MoE 多模态，3B 激活，~18GB@4bit）为Agent增加视觉理解能力：

1. 搭建 VL serving（MLX 量化版）
2. 实现 LocalVisionProvider（图片理解 API）
3. 接入场景：截图理解、设计稿审查、UI 渲染验证
4. 与Agent工作流集成（quality-gate 可调用视觉检查）

### Phase C: 感知层架构统一（如需要）

将 Omni + VL 统一为 Perception Layer API，供所有Agent调用：
...
| R3 | Agent能听能看能说 | AC-A2, AC-A3, AC-B2 | 多模态交互演示 | [ ] |

## Dependencies

- **Evolved from**: F066（语音管道升级，Omni 是下一代）
- **Related**: F103（Per-agent Voice Identity，Omni 需保持声线区分）
