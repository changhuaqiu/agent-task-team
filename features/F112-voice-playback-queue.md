...
> **Status**: done | **Owner**: Golden Agent (OpenCode, claude-opus-4-6) | **Priority**: P1 | **Completed**: 2026-03-18

## Why

现在Agent的语音播放是"发一段播一段"，没有排队、打断、暂停机制。这带来两个问题：

1. **双Agent播客不协调**：F021 Signal Study 的播客功能需要两只Agent交替发言，目前只能靠前端 setTimeout 硬编排，没有真正的播放队列
2. **用户无法打断**：Agent正在说话时，用户想说话或切换话题，没有 interrupt 机制
...
> Scope 收紧（Agent-M GPT-5.4 review 2026-03-17）：只做 queue + interrupt + pause/resume + priority skeleton。replace + intent + podcast orchestration 后置。
...
> **Scope 重定义（2026-03-17）**：原始 Phase B（replace + Intent + 双Agent实时编排）暂无真实场景，降级为未来备选。新 Phase B 聚焦实际需求 — 将 Signal Study 播客播放器迁移到 PlaybackManager，消除重复的 Audio 管理代码。
...
### Phase B-Future: replace + Intent + 双Agent实时编排（未来备选方案，暂无场景）

> **归档原因（2026-03-17）**：route-serial 目前只支持单Agent输出，没有双Agent同时实时语音的场景。replace/intent 行为也暂无使用方。设计保留以备未来语音陪伴模式扩展。

1. **双Agent播客实时编排**（原 Phase B-1）
   - 两只Agent的语音片段按 `queue` 行为交替播放
   - 每段播放完自动切到下一只Agent的片段
   - 播客模式下自动设为 `normal` 优先级
   - ⚠️ 与 Signal Study 播客不同：这里是**实时**双Agent对话编排，Signal Study 是**离线预生成**
...
### Phase C: VAD 打断 — 用户开口Agent停嘴（第三刀）
...
1. **VAD 检测 → PlaybackManager interrupt**
   ...
   - `onSpeechStart` 回调 → `getPlaybackManager().interrupt()` → Agent立刻停嘴
...
### Phase B-Future（replace + Intent + 双Agent实时编排，未来备选）
- [ ] AC-BF1: 双Agent对话稿可按 queue 模式交替播放，无重叠
- [ ] AC-BF2: `replace` 行为只替换同 intent 的语音，不影响其他 intent

### Phase C（VAD 打断 — 用户开口Agent停嘴）✅ Done — merged PR #538
- [x] AC-C1: voiceMode 开启时 VAD 自动检测用户说话并 interrupt 播放
- [x] AC-C2: VAD 检测延迟 < 300ms（从用户开口到Agent停嘴）
