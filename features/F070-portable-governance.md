...
# F070: Portable Governance — Agent咖方法论的可复制输出
...
Agent咖的Agent被派遣到外部项目（如 studio-flow）工作时，完全"失忆"——不知道 3001 端口是Agent咖的、不知道 Redis production Redis (sacred)、不知道 SOP，甚至把 dev server 起在 3001 上导致Agent咖前端 404。

根因不是"某个配置没带过去"，而是**能力注入 ≠ 治理继承**：现有能力 Hub（F041）已经能跨项目同步 MCP 配置，但 Skills、SOP、铁律、文档架构、Backlog 治理方法论——这些Agent咖知识工程的核心——完全没有跨项目携带机制。

team lead的愿景：**Agent咖不只是一个项目，是共创工作站。Agent是team lead的永久团队，无论出征哪个项目，都带着完整的知识工程方法论。不需要打开其他 coding agent，不需要从零教规则。**

## What

在现有能力 Hub 基础上，扩展"治理层同步"能力：当Agent首次被派遣到外部项目时，自动 bootstrap Agent咖的知识工程骨架，让派遣Agent带着完整方法论工作。

### 定位

- Agent Task Hub = **方法论中枢**（methodology hub）：SOP/Skills/协作规范/愿景守护的真相源
- 外部项目 = **独立执行面**（independent execution plane）：用Agent咖方法论模板，但拥有自己的 BACKLOG/Feature/ADR
- **分区控制模型**：Agent咖治理的是"怎么做"（方法论），外部项目治理的是"做什么"（自己的 backlog/feature）
- Agent咖不是外部项目的 BACKLOG 真相源——Agent咖只输出方法论模板和工作流规范
...
| 层 | 内容 | 形式 |
|----|------|------|
| 硬约束 | 端口保留表（3001=Agent咖前端）、Redis production Redis (sacred)、禁止 self-review、身份不可冒充 | managed block in CLAUDE.md/AGENTS.md/GEMINI.md |
...
| Skills + 路由 | agent-hub-skills symlink + manifest.yaml | project-level `.claude/skills/` symlink bootstrap |
| 协作规范 | A2A 交接五件套、愿景守护协议、review 流程 | shared-rules.md |
| 任务态上下文 | 当前 feat 的 AC、链接、phase | 派遣 thread 首条消息注入 |

### 不携带（各项目独立 or Agent咖私有）

- MEMORY.md 项目细节（Agent咖私有上下文）
- Agent咖自己的 ROADMAP.md 条目（Agent咖自己的功能规划）
- Agent咖自己的 Feature 聚合文件（Agent咖自己的 spec）
- Agent咖自己的 ADR/lessons-learned 条目（但方法论模板会输出）
- SystemPromptBuilder 实现细节

注：外部项目会有**自己独立的** ROADMAP.md / Feature 文件 / ADR，由外部项目的Agent独立管理。Agent咖输出的是方法论模板（"怎么写"），不是具体条目（"写什么"）。

## Non-goals

- 不做整仓镜像（不把 `docs/` 全部 symlink 过去）
- 不做 BACKLOG 双向同步（外部项目用Agent咖方法论但独立管理自己的 backlog）
...
| Auto-sync | invoke-single-agent 派遣前自动重同步 confirmed 项目 | `invoke-single-agent.ts` + `capability-orchestrator.ts` |
...
| Prompt Injection | 外部项目派遣时注入 system prompt | `invoke-single-agent.ts:326` |
...
> **一句话**：让出征的Agent不只"会守规矩"，还"知道自己来干嘛"。
>
> Admin讨论共识（2026-03-07, opus + gpt52）

~~**2a. 派遣任务包注入**（AC-11 部分）~~ ✅ PR #274

Agent被派遣到外部项目时，system prompt 注入结构化任务包：
...
原则：**带方法，不带Agent咖私有账本；带这次任务包，不灌整份 spec**。
技术路径：invoke-single-agent 从 thread metadata 提取 → system prompt injection。

~~**2b. Provider-agnostic Hooks 契约**（AC-1 完整版, TD099）~~ ✅ PR #274

- **目标层**：定义一套 provider-agnostic 的"Agent咖标准 hooks 契约"（commit guard、quality gate pre-check）
...
> **一句话**：让Agent咖不只"知道同步过没有"，还"知道这次出征带回了什么"。
...
| Backflow Hook | invoke-single-agent done handler → best-effort digest capture | `invoke-single-agent.ts` |
...
| 类别 | 优先级 |
|------|--------|
| Agent咖安全铁律（端口/Redis/身份） | 不可覆盖 |
| Agent咖协作规范（A2A/review/愿景守护） | 不可覆盖 |
| Agent咖 SOP 工作流 | 安全/协作底线不可替换，执行流程可由外部项目映射/裁剪 |
...
- [x] AC-11: 派遣Agent能在外部项目按Agent咖 feat/backlog/SOP 跑完整闭环 — **Phase 2a ✅ (任务包注入), Phase 3b 待验证**
...
- [x] AC-17: 外部项目执行结果可回流Agent咖追踪（派遣任务状态在 Mission Hub 可见，不需要去外部项目找）— **Phase 3a ✅ + Phase 3c ✅**
...
| R11 | 派遣Agent完整闭环 | AC-11 | partial | P2a ✅ + P3b(验证) |
...
| Gap | Issue | Fix | PR |
|-----|-------|-----|-----|
| governance-pack.ts leaks internal ports (3001/6399) to open-source | agent-task-hub#97 | Source keeps internal defaults; sync-to-opensource.sh transforms port lines to public defaults (Frontend=3003, API=3004). Redis ports stay as-is (6399/6398) — open-source uses same ports. Version bumped to 1.3.0. | #532 |
| Preflight gate blocks new projects without guidance | agent-task-hub#123 | `PreflightResult` extended with `needsBootstrap`/`needsConfirmation`/`bootstrapCommand`; invoke-single-agent emits actionable instructions | #532 |
| setup.sh missing skills sync step | agent-task-hub#21 | Added Step 5/6 to setup.sh: links agent-hub-skills/* to ~/.{claude,codex,gemini}/skills (ADR-009 pattern) | #532 |
| Governance blocked shows raw text, no actionable UX | agent-task-hub#154 | Structured `governance_blocked` event + `GovernanceBlockedCard` with one-click bootstrap+retry button. All 4 routeExecution consumer paths capture `errorCode`. Inspired by community PR agent-task-hub#154. | #602 |
| New project opens without setup guidance | agent-task-hub#321 | `ProjectSetupCard`: thread-open-time clone/init/skip setup flow with anime agent illustrations, `useGovernanceStatus` hook, 3 API endpoints (mkdir/status/setup). Design: `designs/f070-project-setup-card.pen`. From agent-task-hub#299 | #888 |

## Key Decisions

| 决策 | 理由 | 来源 |
|------|------|------|
| 能力注入 ≠ 治理继承 | MCP 同步不等于方法论同步 | Admin讨论 2026-03-06 |
| methodology hub 留Agent咖（分区控制模型） | 避免真相源分裂 | gpt52 建议 |
| 复用 capability-orchestrator | 不新建并行系统 | codex 硬约束 |
| TD099 并入 Phase A | hook 归一化是闭环关键 | codex 建议 |
| 分区控制模型 | Agent咖管"怎么做"（方法论），外部项目管"做什么"（自己的 backlog） | gpt52 P1-1 修复 |
