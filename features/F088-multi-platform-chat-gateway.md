...
> **Status**: done | **Completed**: 2026-04-10 | **Owner**: Agent-R

## Why

Agent Task Hub 目前只能通过 Web UI 和Agent对话。team lead和未来用户希望在**已有的工作聊天工具**中直接与Agent交互，不用切换窗口。

MVP 选型：**飞书**（国内企业）+ **Telegram**（海外开发者）。选型细节见 [平台选型参考](assets/F088/platform-selection.md)。

## What

在 Agent Task Hub 现有 Connector 体系上增加双向聊天能力：
...
| Phase | 内容 | 状态 | PR |
|-------|------|------|-----|
| **1 (MVP)** | 飞书 + Telegram DM-only 双向对话 | ✅ | [#328](https://github.com/zts212653/agent-hub/pull/328) |
| **2** | 多Agent身份 + 分角色展示 + 外部 @路由 | ✅ | [#336](https://github.com/zts212653/agent-hub/pull/336) |
...
| **4** | 消息编辑模拟流式（placeholder → edits → final） | ✅ | [#350](https://github.com/zts212653/agent-hub/pull/350) |
| **C** | 架构归一：命令管道统一 + 跨平台 thread | ✅ | [#353](https://github.com/zts212653/agent-hub/pull/353) |
| **D** | `/use` 模糊匹配：feat号 + title关键词 + 列表序号 | ✅ | [#355](https://github.com/zts212653/agent-hub/pull/355) |
| **5** | 图片/文件收发（双向） | ✅ | [#362](https://github.com/zts212653/agent-hub/pull/362) |
| **6** | 语音消息（STT/TTS） | ✅ | [#362](https://github.com/zts212653/agent-hub/pull/362) |
| **E** | 飞书卡片身份标识：所有回复走 interactive card + Agent名头部，消除多Agent气泡合并 | ✅ | [#389](https://github.com/zts212653/agent-hub/pull/389) |
| **G (8A)** | IM Hub thread：命令隔离（控制面/对话面分离，双绑定）+ Hub thread 可见入口 | 🚧 in-progress | [#570](https://github.com/zts212653/agent-hub/pull/570) |
| **H (8B)** | 模糊意图规则分流：无 binding / 低置信度消息走 Hub，系统卡片选择（无Agent） | 📋 planned | — |
| **I (8C)** | Agent参与 triage：用户点"帮我判断"或连续无法决策时触发 triage Agent（可配置开关） | 📋 planned | — |
...
| **8** | IM Hub 配置向导 — 平台接入引导 UI（飞书/Telegram/钉钉） | ✅ | [#680](https://github.com/zts212653/agent-hub/pull/680) |
| **J1** | file block 全链路 + outbound 投递 + 安全防护（URL 白名单 + path traversal guard + fileName 透传） | ✅ | [#689](https://github.com/zts212653/agent-hub/pull/689) |
| **J2** | Pandoc 文档生成服务 + MCP tool + 自动安装（init-hub.sh / install.sh） | ✅ | [#693](https://github.com/zts212653/agent-hub/pull/693) |
...
- 使用项目 Tailwind 类（`text-hub-black`、`bg-cocreator-light` 等），不使用 .pen 设计稿中的外部样式
...
**背景**：team lead希望Agent能生成 PDF/DOCX/MD 等文档并通过飞书/Telegram 发送给用户。Golden Agent已在飞书测试 thread 中验证过文件生成能力。飞书 API 原生支持 `file_type: pdf/doc/xls/ppt/stream`，上传限制 30MB。

**需求**：
1. Agent生成文档（PDF/DOCX/MD）→ 保存为本地临时文件
...
- 用户从飞书发文件给Agent → 已能接收（`case 'file':` handler）✅
...
- **生成工具：Pandoc**（`pandoc` CLI，非 JS 库）— Agent的输出天然是 Markdown，Pandoc 的 `md → pdf` 和 `md → docx` 是一等公民，无需加 npm 依赖
...
**J2 已完成（PR #693）**：
- [x] 启动脚本自动安装 pandoc（init-hub.sh brew / install.sh apt+dnf）
...
- **ISSUE-2**: Cloudflare Access 与 webhook 路径冲突 — 临时用 `api.agent-task-hub.com`。详见 [架构归一设计](assets/F088/architecture-unification.md#issue-2-cloudflare-access-与-tunnel-ingress-路径冲突)
...
- **ISSUE-5**: 飞书多Agent回复气泡合并无区分度 — 所有Agent共用同一 Feishu Bot，plain text 回复被飞书 UI 合并成连续气泡，不同Agent的回复视觉上混在一起。**Phase E 修复**：统一走 interactive card，每条消息独立卡片 + Agent名头部。
...
- **ISSUE-8**: IM 命令污染对话 thread — `/threads`、`/where` 等元命令的消息存入当前对话 thread，混淆导航和对话内容。**已立项 → Phase G/H/I（三阶段）**：引入 IM Hub thread（控制面/对话面双绑定）。8A 命令隔离（纯控制命令只写 hubThreadId、不触发Agent）→ 8B 模糊意图规则分流（无Agent，系统卡片让用户选）→ 8C Agent参与 triage（可配置开关，兜底才喊Agent）。bindingStore 增加 hubThreadId（懒创建）。**设计修正（team lead 2026-03-19）**：Hub thread **不能隐藏**，必须完全可见——team lead需要在 Web UI 看到所有命令历史，不能有黑盒。Hub thread 需要像Agent训练营一样有专门入口（侧边栏按钮 + 列表页），不是普通 thread 混在对话列表里。**已确认设计（2026-03-20）**：(1) Thread 标记：`connectorHubState?: ConnectorHubStateV1` — 跟 `bootcampState` 同模式（team lead授权技术自决），含 `{ v: 1, connectorId, externalChatId, createdAt }`。(2) 侧边栏入口：🎓 按钮旁加 📡 Hub 按钮 → `HubListModal`（无 IM 面板，新建）。(3) Hub 列表页：按 connector 分组（飞书 Hub / Telegram Hub），显示绑定外部聊天（`lastCommandAt` 命令时间戳为 Phase G+ follow-up，8A 暂不含）。**✅ 8A merged PR #570**：命令隔离 + Hub thread 懒创建 + ConnectorHubStateV1 + 📡 侧边栏入口 + HubListModal + .strict() schema 防护。
- **ISSUE-9**: 多Agent回复只有第一只Agent转发到飞书 — ConnectorInvokeTrigger 在 A2A 链完成后只调一次 deliver()，传第一只Agent的 catId。**✅ PR #545 + #551 修复**：per-agent outbound delivery → per-turn ordered delivery（outboundTurns[] 替代 perCatContent Map），A→B→A ping-pong 正确分发 3 条独立消息。含 richBlocks-only 支持、deliver timeout、实际 speaker catId 归属、turn boundary 检测。
...
- **ISSUE-16**: 外部 IM 创建线程后 spawn 的Agent cwd 错误 — **✅ PR #849 修复**：ConnectorRouter 创建 thread 时传 `findMonorepoRoot()` 作为 `projectPath`（会话 thread + Hub thread），并增加 lazy heal 回填存量 thread。新增 `updateProjectPath()` 到 ThreadStore 接口。3 个回归测试。
- **ISSUE-15**: Agent Task Hub web 发消息 → Agent回复不推送到飞书 — `messages.ts` 的 immediate 路径（`router.routeExecution()`）消费完 agent 事件流后，只做 WebSocket 广播，**没有调用 `OutboundDeliveryHook.deliver()`**。**✅ PR #671 修复**：在 `messages.ts` 注入 `outboundHook` + `streamingHook`，routeExecution 消费循环中收集 turn text + richBlocks，成功时 fire-and-forget 调用 `deliverOutboundFromWeb()`；失败/取消时 `cleanupStreamingOnFailure()` 清理占位卡片。统一 `STREAM_START_TIMEOUT_MS`（5s）常量。18 个回归测试覆盖投递、流式、清理、超时对齐。
