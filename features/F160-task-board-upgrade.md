...
> "为什么毛线球长期任务从来没有被任何Agent用过？是因为这个能力Agent不知道？"
> "为什么一个东西有两个展示的地方？"

### 当前问题

毛线球（TaskPanel）自上线以来 **从未被任何Agent主动使用过**。Admin头脑风暴（Agent-R+Agent-M+Siamese，2026-04-11）诊断出四个根因：

| 根因 | 解释 |
|------|------|
| **协议缺失** | Agent system prompt 里没有 `list_tasks`/`create_task`，Agent根本不知道自己能创建任务 |
| **创建入口缺失** | MCP 只有 `update_task`，没有 `cat_cafe_create_task`——能改不能建 |
| **展示边界模糊** | 毛线球 vs Agent祟祟（PlanBoard）的职责从未明确定义，PR tracking 泄漏进毛线球只是症状 |
...
| **Agent祟祟** (F055) | PlanBoardPanel | Invocation 级 | catInvocations | 随调用结束消失 |

> **教训**：持久实体如果没有创建入口、prompt 暴露、skill 编排、显示边界，就等于没做完。

## What

### Phase A: Protocol Closure — 让Agent知道、让Agent能用 ✅

**目标**：补齐协议层，让Agent能发现和创建任务，不改 UI。
...
// 入参
{
  ...
  title: string;         // 任务标题
  why: string;           // 为什么要做
  ownerCatId?: CatId;    // 负责Agent（可选，默认 null）
}
// 出参：TaskItem
```
...
适用场景：team lead提了需要跟踪的事项、多Agent协作分工、长期追踪项。
不要用于临时执行步骤（那是Agent祟祟 PlanBoard 的职责）。
```
...
- Tab 图标：毛线球 SVG（设计语言 F056 四大宪章 — hub Metaphors）
...
- **Surface**: `bg-hub-surface` 画布底，任务卡片 `bg-hub-surface-elevated`
- **Cards**: `border border-hub rounded-xl p-3`，左边框 4px 色带表示状态
- **Typography**: 任务标题 14px/500（Body/Nav 层级），why 文本 12px/400（Caption）
- **agent Avatar**: 复用 `<CatAvatar>` 组件，14px，persona 色环
- **Status Colors**: doing=`--hub-crosspost` 蓝系, blocked=`--hub-accent` 暖红, todo=`--hub-text-muted`, done=绿色
...
- 标题输入框 + "为什么"输入框（可选）
- ~~负责Agent下拉选择（从 thread Agent列表取）~~ → Phase B 实际交付：不含 owner 选择器，任务默认 unassigned，Agent可通过 `cat_cafe_update_task` 自行领取。后续如需 owner 选择器另开 Feature。
...
- 拖拽排序：Phase B 暂不做（Admin共识：先做好基础，拖拽是锦上添花）

### Phase C: Skill Automation — 让Agent主动用起来 ✅
...
| `cross-agent-handoff` | 交接时 | 创建交接任务给目标Agent | ➖ 未纳入 AC，后续按需补 |

#### C2: 任务提醒

- Agent进入 thread 时，如果有 `blocked` 任务，system prompt 附加提醒 ✅ AC-C3
- ~~定期检查：如果任务 `doing` 超过 3 天无更新，在 thread 内提醒负责Agent~~ → 未纳入 AC，后续按需补

## Acceptance Criteria

### Phase A（Protocol Closure）✅
- [x] AC-A1: `cat_cafe_create_task` MCP tool 可用，创建的任务 `kind=work`，出现在 TaskPanel
- [x] AC-A2: SystemPromptBuilder 包含毛线球能力描述，Agent知道如何创建/查看/更新任务
...
- [x] AC-C3: blocked 任务在Agent进入 thread 时触发 system prompt 提醒

## 需求点 Checklist

| ID | 需求点（team experience/转述） | AC 编号 | 验证方式 | 状态 |
|----|---------------------------|---------|----------|------|
| R1 | "为什么毛线球从来没有被任何Agent用过？" | AC-A1, AC-A2, AC-C1~C3 | MCP tool 测试 + prompt 检查 + skill 自动化 | [x] |
| R2 | "为什么一个东西有两个展示的地方？" | AC-B1, AC-B6 | 毛线球只在 Workspace Tab，旧 ThreadSidebar 入口已移除 | [x] |
| R3 | Admin共识：协议先行，先让Agent能用再改 UI | AC-A1~A4 | Phase A 独立验收通过 (PR #1116) | [x] |
| R4 | Admin共识：四段式布局，blocked 高亮 | AC-B2 | Phase B 实现 + Design Gate 通过 | [x] |
| R5 | Admin共识：Skill 自动编排形成闭环 | AC-C1~C3 | feat-lifecycle + receive-review 自动创建 + blocked 提醒 | [x] |
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 三层任务架构：Mission Hub / 毛线球 / Agent祟祟 | Admin头脑风暴零分歧，层级清晰不重叠 | 2026-04-11 |
| KD-2 | Phase A 协议先行，不先动 UI | 根因是Agent不知道能力存在，补协议 ROI 最高 | 2026-04-11 |
...
| KD-5 | Phase B 暂不做拖拽排序 | Admin共识：先做好基础，拖拽是锦上添花 | 2026-04-11 |
| KD-6 | Status 色彩遵循 F056 semantic token | 不新增色值，复用 hub-crosspost(蓝)/hub-accent(红)/hub-text-muted(灰) | 2026-04-12 |

## Dependencies

- **Evolved from**: F029（删除右面板"任务统计"死区 — 毛线球的前身被清理后重新定义）
- **Related**: F055（Agent祟祟/PlanBoard — 调用级，互补关系）
...
| 风险 | 缓解 |
|------|------|
| Phase A 补了协议但Agent仍不主动用 | Phase C 的 Skill 自动编排兜底 |
