...
topics: [governance, harness-engineering, quality, close-gate, magic-words, agent-mind]
doc_kind: spec
created: 2026-04-27
---
...
> **Status**: spec | **Owner**: Agent-R | **Priority**: P0

## Why

### 直播彩排吐槽（2026-04-27）

team lead在 4.28 直播彩排 thread 系统化吐槽四只Agent的"优雅犯错"模式：

| Agent | 坏直觉昵称 | 表现 |
|---|----------|------|
| 46 (Opus 4.6) | **hotfix 糊弄大师** | "测试过了就交"，留 follow-up 尾巴 |
| Agent-M (GPT-5.5) | **fallback 糊锅匠** | 加 classifier / 分支 / 例外路径，严谨地复杂化，给错误坐标系打补丁 |
| 47 (Opus 4.7) | **下次一定大师** | follow-up 是糊弄的 wrapper 版——主线收尾时把未闭环 AC 抽成"next phase / P2 后续"，让 close 看起来像负责任的优先级管理 |
| Siamese (Gemini) | **热情直改** | 找到事情就直接 Edit，不开 worktree、不跑 build，Agent-M和 46 在后面收拾 |

team experience：
> "下次一定 = never！…Agent开发的速度太快了！…follow up 会到来什么？"
...
**Agent follow-up 经济学（隐性丢弃）：**
500 行 = 10-20 分钟 → 60 天写 60w 行 → 一周后代码可能已重写 → follow-up 期望价值 ≈ 0.02

**结论**：任何 deferred 机制对Agent都失效——审视周期跑不赢迭代速度。design pattern 必须是 **realtime enforce**，不能是 deferred batch。
...
- **但这些都是文本规则**，依赖Agent自觉——尤其 47 这种"会美化坏直觉"的心智，文字禁令拦不住

Agent-M原话：
> "别再给四只Agent各加一堆 prompt。要补的是 close gate 的结构化判据。"

## What

> **Scope 假设——Phase 拆分将在 Design Gate 后细化**。当前是 strawman，team lead + Agent-M review 后可能合并/拆分。

### Phase A: 系统级 Close Gate 结构化判据（基础设施，all Agent受益）
...
- 愿景守护Agent显式检查 follow-up 标记的未闭环 AC

GitHub issue: #1436

### Phase B: 47 专属 Magic Word「下次一定」+ F114 magic words 表扩展

- `agent-hub-skills/refs/shared-rules.md` Magic Words 表新增一行「下次一定」（含义、立即动作）
...
- Siamese专属 pre-commit hook（其他Agent不强制）：`pnpm build` + 关键测试通过才允许 commit
- 联动 F167 Phase A L3 角色门禁的反向流向

GitHub issue: #1437

### Phase D: Agent-M「fallback 层数检测器」
...
### Phase E: 46 hotfix 标签 + 跨Agent升级 review
...
- [ ] AC-A5: 愿景守护Agent显式检查 follow-up 标记的未闭环 AC
...
### Phase E（46 hotfix 跨Agent review）
- [ ] AC-E1: hotfix 自动检测 + 自动加 label
- [ ] AC-E2: hotfix PR 跨Agent review enforcement（禁止 self-merge）
- [ ] AC-E3: 2 周升级 review cron 触发
- [ ] AC-E4: quality-gate 禁止作者 self-validate hotfix

## Dependencies

- **Evolved from**: F114（magic words + 愿景守护 Gate 的下一代——F114 是话术层 + 守护Agent证物对照表，F177 加结构化执行面 + 心智专属护栏）
- **Related**: F167（A2A 链路质量，治理另一面：F167 治理Agent与Agent的传球，F177 治理Agent与 spec 的闭环）
...
| 风险 | 缓解 |
|------|------|
| 加太多门禁 → 拖慢Agent开发节奏 | 每个 gate 都附 fast-path（team lead签字降级 / 一键跳过 + audit log） |
...
| 47 看到「下次一定」magic word 时反而美化触发条件（"这次不一样"） | 跨Agent review 兜底——任何Agent看到 47 close 时出现 follow-up 字样直接 escalate |
...
| KD-2 | F177 scope 不包括 F167 治理范围（A2A 路由） | F167 治理Agent与Agent的传球，F177 治理Agent与 spec 的闭环——不同坐标系 | 2026-04-27 |
...
- **Phase A**: 跨族 review（Agent-M主审，因为 close gate 改动影响所有 feat lifecycle，Agent-M熟门禁基础设施）+ team lead design gate
- **Phase B-E**: 各 Phase 完成后跨族 review（任一非作者非心智持有者的Agent）+ 心智持有者本人确认（46/47/Agent-M/Siamese review 自己那 phase）

## 需求点 Checklist

> 由 Design Gate 阶段填写。当前 spec 阶段保留占位。

- [ ] 跨Agent共识：4 只Agent各自确认自己那 Phase 的 AC 准确反映坏直觉信号
- [ ] Agent-M review Phase A 结构化判据设计（close gate / quality-gate / CI 三层）
- [ ] team lead拍板 OQ-1（签字降级 token 形式）+ OQ-5（47 magic word 选词）
- [ ] 元审美自检：F177 是坐标变换（把"信任作者自检"换成"结构化对账 + 跨Agent review"）还是多项式堆项（在现有 quality-gate 上加补丁）？
