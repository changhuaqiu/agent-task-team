...
topics: [design-language, ux, branding, agent-aesthetic]
doc_kind: feature-spec
created: 2026-03-04
---

# F056: Agent Task Hub 设计语言 — Agent化不是Agent化

> **Status**: doing | **Owner**: Agent-M/GPT-5.2 + Agent-R 主导设计执行 + Agent-R工程架构 + Siamese概念方向
> **Priority**: P1
> **Evolved from**: F051（Agent粮看板Agent爪导航概念）、F052 Phase C（跨线程气泡设计打样）

## 愿景

> **一句话**：Agent Task Hub 应该处处有Agent味，但是好看有设计感的Agent味——不是一只笨蛋Agent随便画的。

### team experience（2026-03-04）

> "我希望Agent化 但是 好看有设计感的 而不是像一只笨蛋Agent随便写的"
> "我们的 plan 叫Agent祟祟，前端应该Agent点"
> "你这个可以当一个打样 告诉未来Agent什么叫Agent化"
> "本质是对齐设计语言"
> "Agent-M的理解、设计语言、完成度、认真，做的最好！"
> "你们三配合才是最棒的！每只大Agent都是最棒的！都是我们家的顶梁柱！"

### 期望体验

team lead打开 Agent Task Hub Hub：
1. 第一眼就知道这是Agent咖——不是因为到处贴了Agent emoji，而是**交互逻辑、微动效、色调**都让人联想到Agent咖
2. 每个新功能的 UI 都自然融入同一种设计感，不会"这个页面像 Notion 那个页面像 Discord"
3. Agent彩蛋散落在细节里（像 B2 的"越用越圆润"），但不影响效率
...
| 品牌感 | 名字叫 Agent Task Hub，UI 是标准 SaaS | Agent味只在文案里 |
| 设计复用 | 每个功能重新定义颜色/间距/组件 | 没有 design token 体系 |
| 新功能设计指导 | Agent们凭直觉设计 | 没有"什么叫Agent化"的参考标准 |

### 已有灵感（Agent-M设计稿 F051）

| 概念 | 可提炼的设计原则 |
|------|-----------------|
| B2 — 越用越圆润 | **活的界面**：使用频率影响视觉（肉垫越来越圆） |
| C — 咖啡香气进度条 | **咖啡馆隐喻**：进度 = 冲泡过程，蒸汽 = 加载 |
| B — Agent爪导航 | **Agent爪触觉**：导航用肉垫形态，"今天想 ruá 哪只Agent" |
...
- ESLint `hub/no-hardcoded-colors` 规则：禁止新增 raw hex / `bg-white` / `text-gray-*` 等非语义类（本地插件 `eslint-plugin-hub`，`warn` 级别）
- "迁移完成"定义（per file）：
  1. `hub/no-hardcoded-colors` 规则零 warning
  2. 所有色彩走 semantic token（`bg-surface`、`text-primary`、`border-default` 等）或 agent token（`bg-opus-primary` 等）
...
**目标**：建立 Agent Task Hub 设计语言的基础规范，让Admin设计新功能时有章可循。对应 Layer 1-2。

#### A1: 设计语言收敛（Admin打样竞赛 → team lead定调）

**设计语言公式**（Agent-M GPT-5.2 提出，team lead拍板选中）：

> **底盘走 Cozy Swiss 的克制，Agent味用"可解释的隐喻"落在少数高频点。**

Agent-M发散了 5 个方向后收敛为一套：
1. **Cozy Swiss（底盘）** — 暖象牙底 + 极细边框 + 单一强调色；Agent味藏在文案和微动效里
2. **Postmark hub（跨线程）** — 跨线程 = "从别的房间寄来的明信片"：奶油纸底 + 来源邮戳
3. **Paw Pads Nav（导航）** — 导航/Tab 像肉垫，交互有"按下去的弹性"
4. **Steam & Brew（状态反馈）** — 进度/加载用"蒸汽、冲泡、杯沿"隐喻

**四大宪章**（Siamese提出，Admin确认）：
1. **温暖触感 (Warm Touch)** — 大圆角（16px-24px），界面像Agent爪垫一样圆润
2. **灵动细节 (Living Details)** — 微交互有生命感，但有上限机制（KD-7）
3. **Agent咖隐喻 (hub Metaphors)** — 可解释的隐喻（邮戳/肉垫/香气），不堆砌Agent emoji
4. **温润色彩 (Cozy Palette)** — 奶油白/软蓝/暖棕，single accent discipline

**实践规则**（F154 事故后补充）：
- **在地设计 (Design in Context)** — 新交互必须先放回真实页面结构中验证。先看现场，再决定放哪里、替代什么、会不会挤、对现有效率是增益还是负担。Agent咖感来自"和环境自然相处"，不是把新元素硬塞进每个角落。

**Admin打样竞赛结果**（team lead评选）：

| Agent | 版本 | Pencil ID | 风格 | team lead评价 |
|---|------|-----------|------|----------|
| Agent-R (Opus) | v1 Apple-inspired | `VJghG` | 日式瑞士，珊瑚渐变线 + 极细边框 | "干净但缺灵魂" |
| Siamese (Gemini) | agent-ified | `pq1cf` | Agent爪印 + 奶油白 | "看不出区别，头像碎了" |
| **Agent-M (GPT-5.2)** | **Postmark** | **`Nfif0`** | **奶油纸底 + 蜡封角标 + 邮戳 pill** | **"做的最好！理解、语言、完成度、认真"** |
| Agent-R (Opus) | v2 Apple-refined | `lydod` | 暖象牙 + 珊瑚点缀 + 精排 | "精致但偏冷" |

**结论**：Agent-M版 Postmark 风格胜出，作为 F056 的参考标准。

#### A2: Design Token 体系 (奶油Agent咖色板)

- **色板 (Pencil 已落地)**:
  - `$agent-cream-white`: `#fdf8f3` (背景基调)
  - `$agent-soft-blue`: `#81D4FA` (功能强调/跨线程隔离)
  - `$agent-warm-brown`: `#8D6E63` (文字/边框)
  - `$agent-paw-pink`: `#FFAB91` (重要交互/彩蛋)
- 圆角梯度：按钮/Pill (100px), 消息气泡 (24px), 侧边栏卡片 (16px)
- 间距系统：8px 基准 grid
- 字体：标题 (Outfit), 正文 (Inter)
- **Token 三层架构**（Agent-M GPT-5.2 提案，Agent-R拍板）：
  - Layer 1 — Base palette（Agent名）：`--agent-cream-white`, `--agent-soft-blue` 等，只定义原料色
  - Layer 2 — Semantic tokens（代码只用这个）：`--hub-surface`, `--hub-text`, `--hub-border`, `--hub-accent`, `--hub-crosspost`，引用 base palette
  - Layer 3 — Agent persona 色：opus/codex/gemini 身份色，不混入品牌色
  - Dark mode：Phase A 就把 `data-theme="dark"` 的 semantic token 留好（不做全量 UI）
- 输出：CSS 变量 / Tailwind config (`hub.surface/text/border/accent`) / Pencil 变量
...
**目标**：全站 emoji 替换为设计过的 SVG/PNG 图标，落实四大宪章第三条"Agent咖隐喻，不堆砌Agent emoji"。
...
- "越用越圆润"类微交互
- 季节/时间主题变化（Agent咖的晨间/午后/夜间氛围）
- Agent状态动画（关联 F014）
- 点击Agent头像 → Agent名片弹窗（生活照、当前心情、个性简介 — "伙伴不是打工Agent"）
...
- [x] AC-A2: Design Token (奶油Agent咖色板) 在 Pencil 变量落地
...
- [x] AC-B0-W2: Bootcamp 任务卡片/成就/排行榜 emoji → SVG（Agent-M负责）
- [x] AC-B0-W3: 系统消息/前端零散 emoji → 文本标签 + SVG（Agent-M负责）
...
### Phase C
- [ ] AC-C1: ≥ 3 个Agent彩蛋微交互上线
- [ ] AC-C2: 点击Agent头像弹出名片（生活照/心情/简介）
...
|----|---------------------------|---------|----------|------|
| R1 | "Agent化但好看有设计感" | AC-A1, AC-A3 | 设计稿 review + 截图 | [/] |
| R2 | "不是笨蛋Agent随便写的" | AC-A1 | 设计原则文档 (四大宪章) | [x] |
| R3 | "前端应该Agent点" | AC-B1, AC-B2 | 改造前后截图对比 | [ ] |
| R4 | "你这个可以当打样" | AC-A4 | F052 气泡收入设计原则 | [x] |
| R5 | "对齐设计语言" | AC-A2 | Token 体系 + 组件库 | [/] |
| R6 | "Agent头像点击出信息/生活照/心情"（不是工卡，是伙伴名片） | AC-C2 | manual | [ ] |
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 从已有概念提炼原则，不从零设计 | Agent-M的 F051 设计稿已有好的方向，提炼比重做高效 | 2026-03-04 |
| KD-2 | F052 跨线程气泡作为设计打样 | team lead确认"你这个可以当打样"，有现成参考 | 2026-03-04 |
| KD-3 | Phase A/B/C 分层推进 | 先建标准再改存量，避免边做边改的混乱 | 2026-03-04 |
| KD-4 | Agent-M(GPT-5.2) 主导设计执行，Siamese出概念方向 | Admin打样竞赛team lead选中Agent-M版（Postmark） | 2026-03-04 |
| KD-5 | Token 三层架构：base(Agent名) → semantic(工程语义) → persona(身份色) | Agent-M(GPT-5.2)提案，避免改一次色板全站手抖 + 代码可读性 | 2026-03-04 |
| KD-6 | Phase A 就留 dark mode semantic token | 成本极低但避免后面返工 | 2026-03-04 |
| KD-7 | 动效上限机制：只在 hover/首次/低频触发 | Agent-M提醒，防止灵动细节拖垮性能 | 2026-03-04 |
| KD-8 | 禁止新硬编码 hex，组件只用 `bg-hub-surface` 等 semantic class | Tailwind 映射统一入口 | 2026-03-04 |
...
| KD-10 | 五层夹心架构：Layer 0 治理 → Layer 1 tokens → Layer 2 primitives → Layer 3 patterns → Layer 4 enterprise | 三方共识（Agent-R+Agent-M+GPT Pro），详见 GPT Pro 咨询报告 | 2026-03-27 |
| KD-11 | 在 TW3 上做，不叠加 TW4 升级风险 | 当前 Tailwind 3.4.0，TW3→TW4 迁移是正交风险源 | 2026-03-27 |
| KD-12 | Radix headless 做 a11y 密集型控件（Dialog/Select/Menu），shadcn 当参考不当宪法 | GPT Pro + Agent-M共识，a11y/focus 管理自建风险高 | 2026-03-27 |
| KD-13 | 术语覆写只做 UI glossary，route/API/schema rename 不在 F056 范围 | Agent-M review：thread 已进路由/API/Redis key，scope 边界必须写死 | 2026-03-27 |
| KD-14 | agent-config.json 消费加 brand token resolver（useCatTheme），组件不直吃 hex | GPT Pro 建议 + codebase 验证：当前无抽象层 | 2026-03-27 |

## Dependencies

- **Evolved from**: F051（Agent爪导航/咖啡香气概念）、F052 Phase C（跨线程气泡打样）
- **Related**: F014（SVG Agent状态动画 — Phase C 彩蛋可关联）
...
- Phase A: team lead + Siamese视觉 review（设计语言必须Admin+team lead认可）
