...
**Agent-R × Agent-M共识**：事件驱动场景（GitHub webhook/邮件/消息唤醒Agent）我们已经比龙虾 heartbeat polling 做得更好——秒级响应 vs 30 分钟轮询。但"没人找你但该主动检查"的自省能力（定时巡检、文档过期检查、stale issue 清理）还没有统一抽象。F102 的 TaskRunner 是 setInterval MVP，硬编码、不可配置、gate 返回 boolean 造成二次扫描。
...
将现有 TaskRunner 升级为六维度 TaskSpec 模型（ADR-022）。**纯后端交付**——Phase 1a 落地后社区和其他Agent可以直接基于 TaskSpec_P1 注册新 consumer（不需要等前端）。前端展示（Workspace 调度 Tab）在 Phase 2 统一做，届时把所有已注册任务 + run ledger 展示出来：
...
- **具体 consumer（team lead要求统一，不再加独立 setInterval）**：
  ...
  - `review-comments` — 新增 PR comments 检测（人类 + Agent的 GitHub comments）

### Phase 1b: Actor + agent Wake
...
- **自然语言配置**：用户说"每天早上 9 点检查 stale issue"→ Agent翻译成 TaskSpec
...
### Phase 1b（Actor + agent Wake）✅
- [x] AC-B1: actor.role resolver 从 agent-config.json 匹配Agent
- [x] AC-B2: MCP dispatch + receipt tracking 端到端
- [x] AC-B3: costTier hint 影响选Agent策略
...
- [x] AC-F1: 删除 NL 输入框，替换为"对话入口 CTA"——引导用户在 thread 里和Agent对话注册任务
...
- [x] AC-G1: Agent在对话中识别"调度注册意图"（如"每天九点发 anthropic 新闻"），命中受支持的任务模板
...
- [x] AC-H2b: JS 重站点 browser 路径接线——`web-digest` 在 `needs-browser` 分支存真实 trigger message 后通过 `invokeTrigger` 唤醒Agent，并带 `suggestedSkill: browser-automation`（PR #826）
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 六维度 TaskSpec 模型（Trigger/Admission/Context/Run/State/Outcome + Actor + Governance） | Admin调研 + GPT Pro 审阅 + Agent-M review 收敛 | 2026-03-25 |
| KD-2 | typed signal gate 替代 boolean | 消除 F102 二次扫描 | 2026-03-25 |
| KD-3 | subjectKey 统一锚点 | 防主键分裂，Agent-M review P1 | 2026-03-25 |
| KD-4 | actor.role = 能力命名空间 | Agent-M review open question 收敛 | 2026-03-25 |
...
| KD-8 | 任务声明 display metadata（label/category/description/subjectKind），前端不再猜 | team lead + Agent-M(gpt52) 讨论收敛：静态展示归 TaskSpec，动态对象展示归 API subjectPreview，前端纯渲染 | 2026-03-26 |
| KD-9 | category 采用对象导向分类（pr/repo/thread/system/external），弃 Custom | Agent-M提出：Custom 是废桶，对象导向更稳，未来"叼邮箱"归 external | 2026-03-26 |
| KD-10 | 任务注册 = 对话驱动，不是 NL 输入框；面板只管展示/管理 | team lead明确指出：W1 Agent是 Agent 不是 API，用户在 thread 里和Agent说话注册任务。NL 输入框违背愿景 | 2026-03-27 |
| KD-11 | 对话式注册先做模板化（受支持模板），不做任意 NL→TaskSpec 生成 | Agent-M(gpt52) 建议：任意生成无审计线、无确认边界、重启丢失。模板化 = Draft/Confirm/Persist/Load 四步 | 2026-03-27 |
...
| KD-14 | 生命周期通知 sender 使用服务端权威身份（`system`），不用客户端传入的 `createdBy` | Agent-M(gpt52) P1 发现：`createdBy` 是客户端传入，用作 notification sender 会引入身份冒充路径 | 2026-04-10 |
...
- Phase A: Agent-M review（跨 family 优先）
- Phase B: Agent-M review + MCP dispatch 集成测试
- Phase C (Phase 2): Agent-M review + **设计→代码保真度对照**（UX V2 设计稿 vs 实现截图，team lead明确要求）
