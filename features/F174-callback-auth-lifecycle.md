...
> **Status**: done | **Owner**: Agent-R（Opus-47）+ Agent-M（GPT-5.4，跨家族独立 review） | **Priority**: P1 | **Completed**: 2026-04-26 (close attempt #3 — D2b-2 rev3 interaction semantics 闭环 + alpha #4 implicit OK)
>
...
**现象**：Agent-M（Codex）反复出现 MCP 工具失败 — `register_pr_tracking` 401、post_message 超时、认证过期。team lead自己也撞到："干了半小时活要发语音，MCP 说 token 过期。"

**team experience（2026-04-23 14:26 / 14:34 / 14:48）**：

> "我发现Agent-M经常有mcp 我们家的 pr trcking 挂不上 auth 过期等等问题，我们mcp的设计是不是又问题？"
>
> "#509：callback auth 基础设施统一/加固 —— 开源社区我们原本 intake进来过这个，这个是什么呢？我希望你站在架构层面完整的优化实现最佳方案 不当补锅匠"
>
> "可以立项 但是我建议 你最好找Agent-M讨论？然后你们可以有各自的思考和立场 我需要一个完整的最终方案。"

**根因诊断**（见 Architecture Map）：这不是 MCP 协议本身的问题，是我们 **callback 鉴权基础设施的 Lifecycle 层**的实现债：

1. **InvocationRegistry 是纯内存**（`packages/api/src/domains/agents/services/agents/invocation/InvocationRegistry.ts:50`，注释 line 47 自己承认 "Phase 3 will migrate to Redis" — 这个 Phase 3 没干）。API 重启 / 部署 / 崩溃 → 所有活跃 token 一次性失效。**Agent-M在 cloud Codex 跑长 session，这是他反复撞 401 的最可能根因。**
...
4. **零 telemetry** — 没有 401 率监控、没有按 agent/tool 维度的失败率，撞了多少次都没数据
5. **失败原因不结构化**（Agent-M review 加入）— `verify()` 只回 `record | null`（`InvocationRegistry.ts:110`），preHandler 把"过期/token 不匹配/invocation 不存在"全部压成同一个 401（`callback-auth-prehandler.ts:54`）。客户端降级现在靠字符串 regex 猜错误（`callback-tools.ts:260, 423`）。**不先改这个，下游降级 + telemetry 都会做歪 — 全都得用脆弱的字符串匹配。**

**为什么现在做**：#509（社区 mindfn，MERGED + intaked）+ #1263（自家 refactor，MERGED）已经把 **Transport 层（header 迁移）+ Authority 层（actor/scope helpers）** 完成了。现在缺的就是 **Lifecycle 层** —— 再不做，Agent-M会继续撞 401，team lead会继续撞"干活半小时 token 过期"。

**非作用域**（刻意排除）：
- 不改 MCP 协议本身（协议只规定 transport，我们调的是我们自己 server 的鉴权实现）
- 不做多用户隔离（那是 F077）
- 不做跨 MCP 身份联邦（飞书/pencil/claude-in-chrome 各自 OAuth，是 L5 未来层，不是 L2）
- **不在本 Feature 做 `Authorization: Bearer` 标准化**（Agent-M指出：当前 client/server/outbox 深度绑定 `x-invocation-id + x-callback-token` 双头 + legacy body/query，Bearer 解决不了核心痛点只会扩大迁移面 — 单立 Feature）
...
> 注：早稿曾把 Bearer 标准化列为 L4。Agent-M指出它本质是 transport encoding 层的 migration 关注，独立成层会在 F174 抢戏，故降为附注、单独立项。

本 Feature 聚焦 **L2（主）+ L3 残尾（附）**。L5/Bearer 作为 follow-up feature。

---

### Phase 顺序（按Agent-M建议重排）
...
**`stale_invocation` 单独**（Agent-M补充）：用户体感一样糟但当前埋没在 401 里——单独 emit 让 D 仪表盘能看到真实失败面。
...
**设计要点**：
- **范式参照** `RedisInvocationRecordStore.ts`（仓库已有的 Hash + Lua 原子操作模式 — Agent-M推荐）
- Redis key schema：`agent-hub:invocation:{invocationId}` → Hash（`catId`/`userId`/`threadId`/`callbackToken`/`parentInvocationId?`/`a2aTriggerMessageId?`）
- `EXPIREAT` 绑定 `expiresAt`，Redis 原生 TTL 清理（不再靠 in-process `cleanup()`）
- `verify()` 用 Lua 原子脚本：检查存在 + token 匹配 + 未过期 + 滑 TTL，一步完成
- `latestByThreadCat` 用独立 Redis key：`agent-hub:invocation-latest:{threadId}:{catId}`，原子 SET
- `clientMessageIds` 用 Redis Set + 有界清理（SCARD > MAX 时 SPOP 最老）—— 原 `MAX_CLIENT_MESSAGE_IDS = 1000` 保持
- LRU 500 上限不再需要（Redis TTL 自动回收）
- **Streams 不作真相源**（Agent-M明确反对）：热路径 `verify/isLatest/claimClientMessageId` 都是点查，不是 replay-first；如果要 audit，**副写一个 stream**，不反过来
...
**客户端续期算法**（Agent-M提案，替代我原来的写死 30min）：
```typescript
...
- 指标：`callback_auth_failures_total{tool, agent, reason}` — counter
...
> **设计哲学**：统计是事后审计，现场可感知性是第一入口。详见 `agent-hub-skills/refs/in-context-observability-checklist.md`。
>
> 早稿（spec v1）只规划单个 dashboard card，被team lead push back："F153 社区小伙伴设计的可观测性是上个世纪的——出问题了Agent和team lead立刻应该看到，像 memory entity 自带状态、browser-preview 端上桌那样。" 收敛为三层结构。

**`in_context_observability` 决策字段**（per checklist 模板）：

```yaml
in_context_observability:
  primary_surface: "thread 内 system_info 富块（D2b-1） + agent avatar status dot（D2b-2）"
  why_not_dashboard_only: |
    callback auth 失败影响"当前正在做的事"——Agent调 register_pr_tracking 401 了，
  ...
  noise_dedup_policy: |
    - 同一 reason+tool+agent 5 分钟内只发一条 in-context 富块（去重窗口）
    - 富块带"隐藏类似消息"按钮，点击后该组合 24h 不再发
    - 持续状态走 agent status dot（D2b-2），不重复发 thread 富块
...
- Metadata 行：tool / agent / when（相对时间）
- Action 按钮：[详情]（跳 D2b-3）+ [重试]（触发 retry）+ [隐藏类似消息]（dedup opt-out）

触发条件：`ok=false` 且 `reason ∈ degradable_reasons`（不含 stale_invocation）。

##### D2b-2: agent Status Dot（P0 · 实体层）

roster 每只Agent avatar 右下角加 status dot（绝对定位，white border 圈起来）：
...
- 24h 401 率 + reason 分布 (bar chart) + Top 工具 + Top 受影响Agent
...
**精准降级**，不假装每个工具都能优雅降级（Agent-M立场）。

**Framework 设计**：
```typescript
type DegradePolicy =
  | { kind: 'none' }                          // 直接 fail，告诉Agent调用方
...
- 降级产物**必须**标 `DEGRADED: true` 字段 + 清晰 hint，避免Agent误认为已成功
...
- `callback-auth-schema.ts` 是否还能删 — 按Agent-M策略：**先停 first-party dual-write**（`callback-tools.ts:66, 81` 客户端不再往 body 写 cred）→ **统计 fallback-only 命中**（preHandler `:39` 仅记 header 缺失时的 fallback）→ **零命中跑满一个 release window 再删** + **保留硬 deadline** 避免 compat window 永生
...
- [x] AC-A4: `stale_invocation` 与 `expired` 在 reason 上明确分开（taxonomy in `@agent-hub/shared`）
- [x] AC-A5: 客户端 `callbackPost`/`callbackGet` 解析 `reason`（typed marker `[reason=X]`），不再用 regex 字符串匹配
- **Bonus**（Agent-M review reminder #2）: reason taxonomy 抽到 `@agent-hub/shared/types/callback-auth-reasons.ts` 单一真相源 + contract test 防漂移
...
- **D2b frontend** 📐 design稿 ready (`designs/F174-callback-auth-health-card.pen`)
  - **D2b-1 in-context 富块**（P0 · 现场层）— ✅ merged 2026-04-25 via PR #1397
    - [x] AC-D4: thread 内 callback auth 失败时，server post 一条 card 富块 (`meta.kind='callback_auth_failure'`)，使用 `CALLBACK_AUTH_SOURCE` connector，前端通过 `CallbackAuthFailureBlock` 渲染 amber 边框 + reason badge + tool/agent/when + 详情/重试(disabled, pending D2b-3/follow-up)/隐藏类似消息
  ...
  - **D2b-2 system-level health indicator**（P0 · 实体层 — twice revised after alpha 反馈）— PR #1403 (rev0 per-agent dot) → PR #1410 (rev1 standalone plug indicator) → PR #1419 (rev2 HubButton badge merge ✅ merged)
    ...
    - HubCallbackAuthPanel 内部的 affected-agents roster 仍使用 `<CallbackAuthCatAvatar>` (48px + "AFFECTED agents" 文本 affordance)，那是 panel-internal context，用户主动打开后明确知道"这是 callback auth 数据"
  ...
  - **D2b-3 stats 深挖 card**（P2 · 审计层）— ✅ merged 2026-04-25 via PR #1403
    - [x] AC-D8: HubObservabilityTab 加 "Callback Auth" 子 tab，渲染 24h Failures + All-time + Affected agents + Legacy Fallback (Phase F deadline 倒计时) + reason distribution bar + Top tools + Affected agents roster (with status dots) + recent samples
...
|----|---------------------------|---------|----------|------|
| R1 | "Agent-M经常有 mcp pr tracking 挂不上 auth 过期" | AC-B1/B3 | 集成测试：模拟 API restart 后 verify 不 401 | [x] |
| R2 | "我干活，然后干了半小时，然后要发语音，结果mcp和我说token过期" | AC-C1/C3 | 集成测试：长 session + 自适应 refresh，voice callback 不 401 | [x] |
| R3 | "站在架构层面完整的优化实现最佳方案 不当补锅匠" | 四层架构 + Phase A-F 完整拆分 | 本文 + Agent-M跨家族 review | [x] |
| R4 | "#509：callback auth 基础设施统一/加固" | 本 Feature 命名 + Phase F 收口 #509 follow-up | #509 + #1263 + Phase F 三件合起来形成完整闭环 | [x] |
| R5 | "我需要一个完整的最终方案" | KD-4 ~ KD-11 收敛决策 + Phase 重排 | Agent-M + 我达成共识，team lead拍板 | [x] |
| R6 | （隐含）401 故障归因不能靠字符串猜 | AC-A1~A5 | 单测：reason 枚举完整覆盖；regression：客户端不再 regex match | [x] |
| R7 | "F153 社区小伙伴设计的可观测性是上个世纪的——出问题了Agent和team lead立刻应该看到" (D2b 设计 push back) | D2b 三层"明厨亮灶"模型（D2b-1/D2b-2/D2b-3） | `agent-hub-skills/refs/in-context-observability-checklist.md` 落地 + 三层全 merged | [x] |
...
- **Evolved from**: agent-task-hub#509（Transport 层 — MERGED + intaked），#1263（Authority 层 — MERGED）
- **Blocked by**: 无（Redis production Redis (sacred)已有基建）
- **Related**:
  - F016（Codex OAuth + 记忆闭环）— invocation-token 概念的起源
  - F061（Antigravity 孟加拉Agent）— Bug-H "persistent MCP write-path auth" 是 F174 的远房亲戚（F174 不解决 persistent 场景，但降级 framework 可能被复用）
  - F077（Multi-User Secure Collaboration）— F174 不做多用户隔离，但 F077 会依赖 F174 的持久化（重启不丢会话）
  - F086（agent Orchestration Multi-Mention）— callback auth 消费方
...
| 降级产物让Agent误认为操作成功，实际没落地 | 降级产物**必须**标 `DEGRADED: true` 字段 + 清晰 hint；单测覆盖Agent的感知口径 |
...
| KD-4 | **加 Phase A — 结构化失败原因作为前置**，下游 D/E 全部依赖 | 否则下游被迫 regex 字符串匹配，做歪后改成本更大（Agent-M review） | 2026-04-23 |
| KD-5 | Phase B 用 **Hash + Lua 原子操作**（参照 `RedisInvocationRecordStore.ts`），不用 Streams 当真相源；audit 副写 stream | 热路径全是点查不是 replay-first（Agent-M OQ-1 立场） | 2026-04-23 |
| KD-6 | Phase C refresh 客户端用 `clamp(ttlRemainingMs/4, 5m, 30m)` + jitter，不 hardcode 30min | TTL 调整时算法自适应，不用追客户端（Agent-M OQ-2 立场） | 2026-04-23 |
| KD-7 | Phase E 降级 = framework + 每工具**显式** `degradePolicy`（含 `none`） | 不假装每个工具都能优雅降级，诚实 + 系统化（Agent-M OQ-3 立场） | 2026-04-23 |
| KD-8 | Telemetry **不直接写 Knowledge Feed**，只人工沉淀阈值触发后的 pattern | 避免 401 噪音淹没 Feed（Agent-M OQ-4 立场） | 2026-04-23 |
| KD-9 | L4 Bearer 标准化降为附注、独立立项 | 当前深度绑定 X-* + legacy body/query，Bearer 解决不了核心痛点只扩大迁移面（Agent-M OQ-5 立场） | 2026-04-23 |
| KD-10 | Legacy fallback 删除：停 first-party dual-write → 统计 fallback-only → 零命中一个 release window + **硬 deadline** | 防 compat window 永生（Agent-M OQ-6 立场） | 2026-04-23 |
| KD-11 | Phase 顺序：A → B → C → D1 → E → D2 → F | 根因先治、补丁后做；不在 B 落地前做大面积降级和面板（Agent-M提出） | 2026-04-23 |
| KD-12 | `stale_invocation` 在 reason taxonomy 单列，单独 emit | 体感等同 401 但被埋没；现已有 `post_message`/schedule 单独识别的先例（Agent-M补充） | 2026-04-23 |

## Review Gate

- **Discussion**：✅ 立项后 @Agent-M 独立思考 → 收敛（spec v2 已落 KD-4~12）→ 待team lead拍板最终 scope
- **Phase A**：跨家族 review（@Agent-M owner-area），reason taxonomy 完整性是重点
...
- **Phase D2**：D2a backend ✅；D2b frontend 走 Pencil 设计稿 → @Siamese (视觉 + F056 alignment) + @co-creator 拍板 → 实现。**Design Gate 已过现场可感知性自检**（决策字段见 Phase D2 段），符合 `agent-hub-skills/refs/in-context-observability-checklist.md` 三层模型
