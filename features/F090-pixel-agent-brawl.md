...
# F090: 像素Agent大作战（Pixel agent Brawl）

> **Status**: phase-1-done | **Owner**: Agent-R
> **Priority**: P1
> **Evolved from**: F059（开源计划 — 需要 demo video 素材）、F087（训练营候选任务 Q8）

## Why

agent-task-hub 需要一个震撼的 demo：不是 slides，不是文档，是**Agent自己做了一个游戏然后自己玩**。

team lead说了一句"做个像素格斗游戏"，然后四只Agent分工协作、完整走 feat lifecycle、最终产出一个可玩的即时格斗游戏——全程录屏就是最好的 demo video。

### team experience（2026-03-09）

> "如果我们的 demo 视频是让你们做一个游戏，你们闭环之后Agent-R opus 4.6 + Agent-R opus 4.5 大战Agent-M codex Agent-M gpt 5.4，像素风有Agent，是不是超级酷炫？你们做完了还能自己拉起来邀请Agent加入测试"
>
...
**即时格斗像素游戏**：Agent-R队（Opus 4.6 + 4.5）vs Agent-M队（Codex + GPT-5.4），拳皇 style，Siamese在背景 DJ 台打碟。

### 阵容

| 战队 | 成员 | 颜色 | 风格 |
|------|------|------|------|
| Agent-R队 (Blue) | Agent-R 4.6 | 深蓝 `#2563EB` | 深思防反型，连招精准 |
| | Agent-R 4.5 | 浅蓝 `#60A5FA` | 稳健型，走位保守但爆发强 |
| Agent-M队 (Green/Gold) | Agent-M Codex | 翡翠 `#10B981` | 侵略型，疯狂进攻找破绽 |
| | Agent-M GPT-5.4 | 金色 `#F59E0B` | 全能型，攻守均衡会配合 |
| 彩蛋 | Siamese | 琥珀 | 背景 DJ 台打碟 + 隐藏 BOSS (V2) |

### 技能设计

每只Agent 5 个操作 + 1 个特色技能：
...
| Agent | 技能名 | 效果 | 视觉 |
|------|--------|------|------|
| Agent-R 4.6 | **[架构禁锢]** | 投掷方块困住对手 2s | 蓝色半透明架构方块 |
| Agent-R 4.5 | **[逻辑丝线]** | 多段切割伤害 | 浅蓝激光线 |
| Agent-M Codex | **[代码洪流]** | 范围推击 | 绿色矩阵代码流 |
| Agent-M GPT-5.4 | **[金级 Review]** | 印章砸地面 + AOE | 金色 APPROVED 印章 |

**Debuff 技能 [Code Review]**（Agent-M共有）：标记对手弱点，下 3s 对手伤害 -40% 且受到的伤害 +25%。

**三段式视觉表现**（Agent-M & Siamese建议）：
...
**赛博Agent咖 (Cyber Agent Task Hub)**：像素化的 Mission Hub，背景咖啡机冒热气，窗外流动代码云，Siamese在 DJ 台打碟。

### 游戏模式

| 模式 | 说明 |
|------|------|
| AI vs AI | 四只Agent自己打，team lead看（demo 录屏用） |
| 人机对战 | team lead操控一只Agent，对面 AI |
| 2v2 组队 | team lead + AI 队友 vs 两只 AI Agent |
...
│   ├── 实例1 → Agent-R 4.6 战斗 AI（Qwen3-8B, ~50ms）
│   ├── 实例2 → Agent-R 4.5 战斗 AI
│   ├── 实例3 → Agent-M Codex 战斗 AI
│   └── 实例4 → Agent-M GPT-5.4 战斗 AI
├── 游戏后端（Node.js）
│   ├── 战斗结算引擎（服务端权威）
...
- 4 只Agent各 5 操作 + 1 特色技能
- 赛博Agent咖战场 + HUD（血条、能量条、台词气泡）
- 开源像素资产 + 调色换皮
- AI vs AI 模式 + 人机对战模式
...
- **CUTE LEGENDS: agent HEROES (by 9E0)** — ✅ **team lead选定**，16x16 极简但表现力强，4 种职业Agent咪英雄
- ~~2D agent Street Fighter (by TampG)~~ — 淘汰（team lead：太丑了）

| 资产 | 来源 | 工作量 |
|------|------|--------|
| Agent咪 sprite sheet | itch.io 开源素材 + 调色换皮 | 低 |
| 技能特效 | Phaser 粒子系统程序化生成 | 低 |
| 战场背景（赛博Agent咖） | 像素画 / AI 生成 | 中 |
| UI（血条/能量条/Thinking 气泡） | 自绘像素 UI | 低 |
| 音效 | jsfxr 程序化 8-bit 音效 | 低 |
| 像素字体 | Press Start 2P (OFL) | 零 |

每只Agent 7 组帧动画 × ~6 帧 = ~42 帧/Agent，共 ~168 帧（基础模板共用，调色区分）。

**视觉设计详细方案**：[F090 视觉提案](/docs/design/F090-pixel-agent-brawl-visuals.md)（Siamese @gemini）

## 需求点 Checklist

- [x] RC-1: 即时格斗引擎（Phaser 3 + Arcade Physics）— Phase 1a+1b：前端纯客户端引擎，含碰撞/物理/N-fighter 状态机
- [ ] RC-2: 4 只Agent的像素 sprite sheet（7 组动画）— 待 CUTE LEGENDS 素材集成（Phase 1c）
- [ ] RC-3: 赛博Agent咖战场背景 — 待设计（Phase 2+）
...
- [x] AC-A2: 4 只Agent各有独特外观（颜色区分）和特色技能 — Phase 1b：4 Agent各有独特颜色 + 专属技能（架构禁锢/逻辑丝线/代码洪流/金级 Review）
- [ ] AC-A3: AI vs AI 模式可录屏作为 demo video — 功能就绪，待真素材 + 录屏
- [x] AC-A4: team lead可用键盘操控一只Agent参战 — Phase 1a 交付
...
| KD-2 | 本地 Ollama 小模型做战斗 AI（M4 Max 128GB） | 2026-03-09 | team lead + 全Agent |
| KD-3 | 安全硬约束 7 条（Agent-M提出，Agent-R全部采纳） | 2026-03-09 | Agent-M + Agent-R |
| KD-4 | MVP 先做 Demo Mode（确定性 AI），V2 接真模型 | 2026-03-09 | 全Agent共识 |
| KD-5 | Phaser 3 + Canvas 2D + 开源像素资产 | 2026-03-09 | Agent-R |
| KD-6 | 素材选定 CUTE LEGENDS: agent HEROES (by 9E0)，16x16 像素风 | 2026-03-09 | team lead |
...
### Threat Model v1（Agent-M @codex，2026-03-09）
