...
F042 的目标是纠正提示词与 skills 体系的系统性退化：信息架构混杂、路由不精确、compact 后身份/A2A 协议遗忘、硬编码Agent名无法适配多分身。
...
- [x] AC-C2: 硬编码Agent名治理与 reviewer 降级策略形成统一规则。
...
Agent Task Hub 的提示词和 Skills 体系存在系统性问题：
...
4. **硬编码Agent名** — 规则写死"Agent-R找Agent-M"，无法适应多分身和新Agent接入
...
### 1.1 四Agent独立审计发现（2026-02-28 16:05 起）
...
| # | 现象 | 根因 |
|---|------|------|
| R1 | Agent-M compact 后自称"Agent-R" | 压缩后只注入队友列表，缺"你是谁" |
| R2 | Agent不用 @ 协作 | A2A 协议只在 new session/compact 后注入 |
| R3 | 同族 reviewer 被 @ 后身份错位（把自己写成另一只Agent/给出不可采信的“我跑过验证”） | 身份常量未每回合 pinned；review 请求缺少 identity 握手与证据纪律 |
...
**断裂防护**（GPT-5.2 提出）：compact 后Agent不知道自己在链的哪个位置。
...
| 2 | `collaborative-thinking` | brainstorming + multi-agent-brainstorm + discussion-convergence | 方法论 |
...
| 7 | `request-review` | agent-hub-requesting-review（模板→refs/） | 流程 |
| 8 | `receive-review` | agent-hub-receiving-review | 方法论 |
| 9 | `merge-gate` | merge-approval-gate + requesting-cloud-review + finishing-branch | 流程 |
| 10 | `cross-agent-handoff` | 保留 | 协作 |
...
| 原 F042 项目 | 毕业去向 | 理由 |
|---|---|---|
| 硬编码Agent名清理（10 files） | **M1 收尾**（Agent-M执行） ✅ Done | 纯清理，不涉及架构决策 |
...
| manifest.yaml 路由真相源 | `agent-hub-skills/manifest.yaml` |
| 链式导航 15/15 | 每个 SKILL.md 有 `## 下一步` |
| refs/ 抽取 | `agent-hub-skills/refs/` |
...
BACKLOG F042（入口）
  └→ 本文件（spec + 完整决策）
      ├→ Thread thread_mm4dj9jp0tij0ch3 16:05+（第二轮四Agent审计）
      ├→ F032-agent-plugin-architecture.md（技术侧）
      └→ packages/shared/src/agent-config.json（roster 事实源）
```
