...
doc_kind: spec
created: 2026-04-22
community_pr: ["agent-task-hub#520"]
---

# F171: First Partner Onboarding — 领养第一只Agent / 配置第一位伙伴

> **Status**: done | **Owner**: Agent-M/gpt52 | **Priority**: P1 | **Completed**: 2026-04-25

## Why

我们家当前对开源小白不够友好。现有默认心智是“家里已经有Agent、你知道每只Agent是谁、也知道该先配什么账号”，但真实新用户往往并不具备这些背景知识。

team lead判断（2026-04-21）：
> “我们家现在对于开源小白不太友好所以才想接受他的产品语意。”
> “不是每个人都是你的landy这么了解你们每只Agent 每只Agent的agent 且还是正版订阅用户。”

因此需要把冷启动产品语意从“开始Agent训练营”收敛成更基础、更普适的：

1. 不假设用户已经理解所有Agent和账号体系
2. 第一次进入时，先帮助用户**领养/配置第一只Agent**
3. 先获得**第一次成功协作**
4. 再逐步引导到更多Agent、更多 guide、更多高级能力
...
- 把对外 framing 从“新手训练营”改成“领养第一只Agent / 配置第一位伙伴”
- 首次进入默认目标不是“走完整 training flow”，而是“完成第一只Agent配置并成功发出第一条有效消息”
...
- **F127** 承接账号体系与运行时建Agent：账户配置、Agent实例创建、绑定模型/accountRef
...
- 参考 `agent-task-hub#520` 的产品方向，但不照搬其 feature 编号与 bootcamp 命名
- 可吸收：
  - `0 active members` 的冷启动契约
  - 首只Agent配置优先于复杂协作概念
  - 账号配置与第一只Agent创建的一体化入口
- 不直接照搬：
  - “新手训练营”命名
...
- `agent-task-hub#520` 已合入开源仓，agent-hub 侧通过 intake PR `agent-hub#1395` 吸收可复用实现
- 吸收范围以 “First Partner Onboarding” 产品语意为准：保留 F171 的领养第一只Agent / 配置第一位伙伴 framing
...
- [x] AC-A2: 首次进入的主 CTA 与空状态文案能让开源新用户理解下一步是“领养第一只Agent”

### Phase B（冷启动流程）
- [x] AC-B1: 首次进入且无活跃成员时，用户可在一个连续流程中完成模板选择、账号配置、连通性验证、创建第一只Agent
- [x] AC-B2: 完成第一只Agent创建后，用户可立即进入可工作的对话态
- [x] AC-B3: 首次成功协作前，不要求用户先理解多Agent协作全貌
...
| R2 | “不是每个人都这么了解每只Agent、每只Agent的 agent” | AC-A2, AC-B3 | 产品流程 review | [x] |
| R3 | “我支持吸收他的产品语意，但改写成领养第一只Agent / 配置第一位伙伴” | AC-A1, AC-C2 | feature spec review | [x] |
...
- **Related**: F127（Agent管理重构 — 账户配置与运行时建Agent的技术底座）
...
| 风险 | 缓解 |
|------|------|
| 把“训练营”整套拿掉后失去家里的陪伴感 | 保留Agent人格与陪伴文案，但把主目标聚焦到“成功配置第一位伙伴” |
...
- Phase A: ✅ team lead确认冷启动产品语意（“领养第一只Agent / 第一位伙伴”）
- Phase B: ✅ Agent-M + Agent-R确认与 F127/F155/F110 的边界
