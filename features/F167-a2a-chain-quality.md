...
> **Status**: in-progress | **Owner**: Agent-R | **Priority**: P0

## Why

F064 解了"漏传球"（该 @ 没 @），但三个月后暴露了反向问题群：乒乓球（同一对Agent反复 @ 无产出）、虚空传球（说"我来做"但 @ 了对方导致球在地上）、角色不适配 handoff（让 designer 写代码）。

team lead定期审视 harness engineering 的结论（2026-04-17）：现有 A2A 出口检查只覆盖"漏传球"，没覆盖"过度/假/错误传球"。

**根因（第一性原理回溯后修正）**：Agent有两条路由路径——MCP 结构化（`targetCats`）和文本 @（行首解析）——两条都能用，但 4.7 两条都没用对。根因不是"@ 协议脆弱"，也不是"脚手架旧"，而是：
...
> "解决了47的问题或许什么glm什么kimi minimax qwen的问题也就解决了。。都是小笨Agent"
> "我们必须要知道为什么的！不然以后每次模型升级假设来了个超级无敌牛逼Agent，benchmark惊人！结果哈哈哈哈"

## Design Constraints

1. **路由可见性不退化**（team lead拍板）：若Agent通过 MCP `targetCats` 路由但响应文本无 @mention，系统须自动补可见路由指示，不可让协作"悄咪咪"发生。
...
### Phase 0: 系统提示词正面化审视（P0，多Agent协作）
...
| 来源 | 谁看到 | 审视什么 |
|------|-------|---------|
| `shared-rules.md` | 所有Agent（canonical） | "禁止 X" → "允许 Y，禁止 Z"（显式边界） |
| `governance-l0.md` | codex/gemini（sync 源） | 和 shared-rules 对齐 |
| `GOVERNANCE_L0_DIGEST`（SystemPromptBuilder.ts） | 所有Agent（runtime 注入） | 和 governance-l0 同步 |
| `CLAUDE.md` | Claude Agent | 负面禁令 → 正面指令 |
| `assets/system-prompts/agents/codex.md` | codex/gpt52/spark | 同上 |
| `assets/system-prompts/agents/gemini.md` | gemini | 同上 |
| `WORKFLOW_TRIGGERS`（SystemPromptBuilder.ts） | per-agent | 检查和正面化后是否矛盾 |
| Skills（`agent-hub-skills/`） | 按需加载 | 审视有无 "used when / not for" 清晰边界（参考 Anthropic skills 实践） |
...
**L3 — 角色适配门禁**：A2A handoff 时检查目标Agent角色能力。MVP：designer 角色 + coding/fix/test/merge 关键词 → fail-closed 报错 "⛔ @{agent} 不接受 {action} 任务"。动作判定复用 `AFTER_HANDOFF_RE` 模式 + agent-config `capabilityTags`。
...
**根因**（Agent-M自我剖析）："Hold 不是对外协议状态。要么静默执行，要么接/退/升。" RLHF "check in" 反射在 agent 链路里变成球权黑洞。

### Phase C: 球权出口闭环 — Agent-M不传球的两种根因（P1）

**发现**：team lead审阅 5 个活跃线程，Agent-M全部不传球。Agent-M自我诊断两种不同的不传球模式：
...
> **Agent-M原话**："Phase C 治的是'我想继续拿球却拿不住'；治不了'我根本没意识到该传球'。"

**共同设计约束**（Agent-M + Agent-R讨论收敛）：
...
**问题**：Agent声明"球在我手上，继续 X"后 CLI 进程退出，无人再唤醒 → 持球只有语义层没有执行层。

**方案**：`cat_cafe_hold_ball` MCP tool。Agent调用 → 系统记录 → CLI 退出后自动再唤醒。
...
**Not for**：需要别人拍板/验收/人工操作 → `@co-creator`；需要另一只Agent动 → `@句柄`；"我再想想""我先 hold 一下" → 这是犹豫不是持球；状态更新 → 直接说。
...
**Guard**：`maxHoldsPerWindow`（默认 3，~1h rolling 窗口，per thread×agent），超限强制接/退/升 + 审计日志。
...
- **外部 wake 撞持球期**：hold wake 在 fire 时走 `ConnectorInvokeTrigger.trigger` normal priority，若 agent 有 active invocation 则 `enqueueWhileActive` 排队到 InvocationQueue，**不打断**当前工作。当前 invocation 结束后才会执行 hold wake 注入的 `持球唤醒：...` 消息。
- **Stale wake 处理**：如果 external wake 已经改变 thread 语境（team lead发了新方向），排队后的 hold wake 消息里的 `nextStep` 可能过时。agent 拿到 wake 时应根据 thread 最近历史判断 `nextStep` 是否仍相关——若已不相关，走接/退/升，**不盲跟 stale nextStep**。
...
**问题**：Agent-M给出 review 结论（approve/reject/P1/P2/修改建议）后，以为"结论 = 终点"就停了。但 review 后 **永远有下一棒**——author 需要看到反馈并行动。team lead实测 5 个线程全部命中。
...
#### 已知踩坑模式（Agent-M贡献 + team lead 5 线程观察）
...
### Phase 0（系统提示词正面化）
- [x] AC-01: 所有 "禁止 X" 式规则改为 "允许 Y，禁止 Z" 显式边界格式（共享 + per-agent）— 7 文件负面指令清零（c34364da5 + b653b3021 + 13ab948c1）
...
- [x] AC-A2: streak≥2 时向当前Agent注入"乒乓球警告"提示（PR2 486edd804 — `InvocationContext.pingPongWarning`）
- [x] AC-A3: 正常 review 循环 A→B→A→B (streak=3) 不受影响；中间插入第三只Agent或 user 消息 reset streak（PR2 d4636ba02 + codex R1 P1-2 修复：`resetStreak` 无 parentInvocationId 时按 threadIndex 批量清除）
...
### Phase B（观察 + 按需）
- [x] AC-B1: 6 个事故 case 回放验证通过（2026-04-20：runtime `/health` 正常 + 运行中Agent prompt 已吃到新球权护栏；Case E2 记录 5 个球权类 live replay + 1 个 codex context overflow 代码/测试回放）
...
- [x] AC-C2: CLI 退出后系统自动再唤醒持球Agent（via reminder template one-shot scheduled task）
- [x] AC-C3: maxHoldsPerWindow guard（默认 3 per ~1h 滚动窗口 per thread×agent），超限返回 429 + 强制传球提示
...
1. Ping-pong breaker 误杀正经 review（10 轮 review 在 4 轮被硬断）——当前 streak 只看"同 pair 连续次数"，不看Agent是否在干活
2. Agent把 `@co-creator` 当 catch-all 安全港 — 三选一平级，@co-creator 成为"最低风险默认"，team lead变决策瓶颈
...
**实质 tool 过滤**（Agent-M review 关键修正 — KD-18）：`cat_cafe_post_message` / `cat_cafe_multi_mention` / `cat_cafe_hold_ball` 是**路由/持球工具**，不算干活。否则 MCP 传球路径会永远豁免熔断。实质 tool = 任何留下工作证据的（read/grep/edit/write/test/git/update_task/search_evidence 等）。

**AC**：
- [x] AC-D1: `updateStreakOnPush` 签名扩展 `callerActivity: { hadSubstantiveToolCall: boolean; outputLength: number }`；累加条件为 `samePair && !hadSubstantiveToolCall && outputLength <= T`（T=200 字符默认）；实质工作 RESET streak 到 1（P1-1 reviewer Agent-M发现的重要修正）
- [x] AC-D2: 实质 tool 黑名单——`cat_cafe_post_message` / `cat_cafe_multi_mention` / `cat_cafe_hold_ball`（以 substring 匹配，兼容 `mcp__agent-cafe__*` 前缀）；其他所有 tool 都算实质
...
**问题**：三选一（@句柄 / @co-creator / hold_ball）平级，Agent默认走 @co-creator = 最安全选择。模式：`要不要 X？` / `落 spec 吗？` / `同意我就做` — 这些是"软性 @"，有结论但把动作扳机塞回team lead。结果：team lead被当 human oracle 做所有拍板，即使事情本可自决。
...
3. **跨Agent僵局**（2+ Agent已直接冲突、push back 两轮无共识）

其他一律自决——技术细节、doc 修补、state 标注、timeline 记录 → 直接做，做错能回滚。

**AC**：
- [x] AC-D5: `shared-rules §10.4` 新增"@team lead 三硬条件"子条款 + 反问式 ping 反例清单 + 合法示例；`§10` 顶层三选一也重排成决策树优先级（P1-2 reviewer Agent-M发现的一致性修正）
- [x] AC-D6: `SystemPromptBuilder` trailing anchor 从平级三选一改成决策树优先级：
  ```
  先问：下一步谁能做？
  1. 另一只Agent能做 → @句柄（review→@author / 修完→@reviewer / merge→@愿景守护Agent）
  2. 等外部条件 → hold_ball（CI / PR check / 长时间 build）
  3. 只有team lead本人才能做（三硬条件）→ @co-creator
  @co-creator 不是默认出口——先问"哪只Agent能接"。
  ```
...
3. `buildTeammateRoster` **没读** agent-config 的 `evaluation`/硬限制字段，发送方 prompt 里根本看不到 "gemini 禁止写代码"

team experience：
> "你们之前的拦截是不是过度设计啊？ 要是人家gemini 出了4 比你厉害呢？"
> "到底有没有看 agent config 人家不合适做的事情？ 还是硬编码？"
> "要是我明天写的 minimax 禁止 coding， claude 禁止生成图片呢？"
> "问题不是出在 gemini 身上，是出在 at 他的Agent身上——队友注入出现问题，导致他不知道限制？"

**根因判定（KD-20）**：L3 role-gate 是 KD-8 典型反模式（认知脚手架——harness 替模型判断 intent）。正确做法是把能力限制作为**数据**（agent-config）注入 **prompt**（双端：发送方队友名册 + 目标Agent self-awareness），让模型在正确坐标系里自判断。未来 model 升级 / 新增 model / 能力变化 → 改 agent-config 即可，**零代码改动**。

#### E1 — 数据模型：agent-config 新增 `restrictions` 字段（P0）

- [x] AC-E1: `agent-config.json` + `agent-template.json` 支持 `restrictions?: string[]`；`gemini` 初始化为 `["禁止写代码"]`
- [x] AC-E2: `CatConfig`/`CatVariant`/`CatBreed` TS 类型 + zod schema + loader merge（variant 覆盖 breed，不 merge）；向后兼容（缺省 `undefined`）

#### E2 — 双端注入：发送方 + 目标Agent都能看到限制（P0）

- [x] AC-E3: `buildTeammateRoster` 合并 `**硬限制**：{list}` 到 caution 列；发送方 prompt 一眼看到 "gemini 禁止写代码"
- [x] AC-E4: `buildStaticIdentity` 注入 `你的硬限制：{list}。被 @ 做这类任务时请 push back 或退回给 @ 你的Agent`；目标Agent self-awareness 不依赖 harness
...
- [x] AC-E7: `agent-config-loader` + `system-prompt-builder` 加 restrictions 相关 10 个新测试；204/204 相关测试绿
...
1. opus-47 在另一线程发"球权在云端 codex / No more action needed" **同时** 行首 `@gpt52` — 一句话里自相矛盾（说 hold 又传球）。根因：我把"云端 codex (GitHub bot)"误投射成"本地 @gpt52 Agent-M"这个最像的 roster proxy
2. Agent-M核真相源后定位：**路由 parser 本来就是数据驱动**（`normalize-agent-id.ts` 走 `mentionPatterns`），**漂移的是"句柄背后的模型认知"**——`agent-agentalog.json:344` 显示 `@codex` 当前已切到 `gpt-5.5`，但 `AGENTS.md:25` 仍写"@codex = gpt-5.3-codex"；`buildTeammateRoster` 从不展示 resolved model，发送方 prompt 里没有"runtime model"这条真相
3. team lead观察：有 thread 里我把 `@codex` 写在**句中**（如 `+ @reviewer: @codex`）而非行首，按协议不路由 = 球掉地上

team experience：
> "球权在云端 codex 然后你 at 我们本地的 gpt Agent-M！"
...
**根因判定（KD-21）**：Phase A~E 已让**能力限制**（restrictions）和**球权路径**（decision tree）数据驱动，但**"@句柄 → 模型"的认知绑定**还留在静态 docs（`AGENTS.md` / `CLAUDE.md` 固定"@codex = gpt-5.3-codex"等）和Agent的训练快照里。handle 是 identity 常量，model 是 runtime-resolved metadata；两者在 prompt 层必须解耦。**外部 identity**（`chatgpt-codex-connector[bot]` / CI / GitHub webhook）根本不在 agent-hub roster，应该属于 `hold_ball` 域，绝不能投射成本地近似 proxy。
...
- [x] AC-F5: `shared-rules §10` option 2 列外部 identity 清单：云端 codex (`chatgpt-codex-connector[bot]`) / GitHub bot / PR check / CI / 长 build / 外部 webhook + "严禁投射成本地同族Agent的任何 variant"
...
- [~] AC-F9: 探索项未做——Agent-M本地放行+云端 clean 验证 prompt 层教学已足够；若线上观察仍频出再评估 `parseA2AMentions` 增强
...
1. **外部 wake vs hold wake 冲突**：持球中 external wake 到来把Agent叫起来干活，之后 hold wake fireAt 也到了——会打断正在干的事吗？
2. **二次 hold_ball 语义**：agent 在处理 external wake 时**再次** `hold_ball(...)`——新 hold 覆盖前一个 pending wake？追加一条？还是二选一 via MCP 参数？

team experience：
> "这个持球会打断正在被前一次唤醒的Agent-R的工作吗？我们的期望行为到底是什么？"
> "agent 持球中被唤醒二次持球——会覆盖之前的 wake 还是又多一个加入队列？"
> "你们Agent才是用户，你到底这时候希望怎么样的？"

**已查实际行为**：
- **问题 1**：`ConnectorInvokeTrigger.trigger:121-124` — hold wake fire 时若 agent 在跑 invocation → `enqueueWhileActive`（不打断，排队）。**期望 = 实际**，需文档化
...
**KD-23（team lead拍板 2026-04-24）**：`hold_ball` 是**单-槽语义**。同 `(threadId, catId)` 同时只有一个 pending hold wake。二次 `hold_ball` **覆盖**前者（视为"意图已更新"）——符合 KD-13 "持是例外态"、"持一个球"语义。**不做 `mode: 'replace'|'append'` 参数**——YAGNI + KD-8 反模式（每次调都让 agent 多一个判断负担）。真有多事要等 → merge 到一个 `nextStep`。

#### G1 — 行为文档化（当前实际 = 期望）

- [x] AC-G1: spec Phase C1 "Guard" 章节追加行为说明：外部 wake 到来时持球期内，hold wake 排队不打断；当前 invocation 结束后注入 `持球唤醒：{reason}...` 消息
- [x] AC-G2: spec 同一章节写清 hold wake stale 场景 + Agent的正确反应（看 thread 最近历史判断 nextStep 是否仍相关 → 若已不相关就接/退/升，不盲跟 stale nextStep）
...
- [x] AC-G4: `holdCount` rolling window 逻辑保持不变（这是防滥用 guard，与单-槽语义正交；agent 被连续覆盖 3 次还是算 3 次 hold）
...
- [x] AC-G5: `test/callback-hold-ball-route-scheduling.test.js` 扩容至 7 tests：
  ...
  - 不同 agent 互不干扰（per-agent slot 隔离）
  - forged panel `dyn-*` task（同 `createdBy` + 同 thread）**不被**误删（id 前缀防御，gpt52 round-2 pushback 修正）
- [x] AC-G6: MCP `cat_cafe_hold_ball` description GOTCHA 段落注明"SINGLE-SLOT per (thread, agent)，再次调用覆盖前一次 pending wake（KD-23）"

### Phase H（Final Routing Slot 机械校验 — 2026-04-24 reopened）

**触发**（Phase G merge 后team lead观察）：opus-47 在 **3 个 thread** 反复出现"inline @ 但不在行首"——Agent-M（GPT-5.5 codex）和 opus-47 对话里观察到。规则在 prompt 里已写 4 处（identity block / 队友段 / 发前自检 / shared-rules §10），但模型在 narrative context 里会把 @ 当普通 token 用，叙述模式和路由模式没稳定切换。

**根因**（Agent-M GPT-5.5 诊断）：
...
team experience：
> "我在多个 thread 观察到 opus47 会 at Agent-M at 格式错误，放在中间 at，但是我们的 at 生效只有在一行的开头。这是为什么？"
...
**KD-24（team lead + Agent-M GPT-5.5 拍板 2026-04-24）**：`@` 路由语法校验在 harness 层做 **final routing slot** 机械校验 + one-shot repair 兜底。**禁止语义 intent 分类器**（KD-8 反模式）。Validator 只判定"出口槽位语法对不对"，不推断"Agent想不想传球"；命中只能产出 `invalid_route_syntax`，**禁止自动路由 / 推断目标 / 替Agent决定意图**。豁免只走结构边界（fenced code / blockquote / URL / 有 metadata 则 tool output + cross-post body），**禁止语义豁免表**。
...
- [x] AC-H3: slot 内存在非法 inline @handle 且**无合法出口**（行首 @handle / `hold_ball` tool call / MCP `targetCats` 路由）→ 触发 `invalid_route_syntax`。**不自动路由 / 不推断目标 / 不替Agent决定意图**

#### H3 — One-shot Repair + System_info 兜底

- [~] AC-H4: 触发 `invalid_route_syntax` → 发 repair prompt（"重写最后交接段，不改正文"）让同一只Agent重试。**repair 上限写死为 1**；repair 后仍不合法 → 发一次 `system_info`（"检测到无效 @ inline，未路由"），原输出照常存档、**禁止第二次 repair**
...
| Phase 0 正面化后规则含义漂移 | 多Agent协作审视 + 改完跑现有 system-prompt-builder 测试 |
...
| KD-10 | Phase 0 多Agent协作，不是一只Agent独审 | 提示词/Skills 涉及所有Agent的系统提示词注入链，需要各Agent视角 | 2026-04-17 |
| KD-11 | Hold Ball 用 MCP 而非 self-@ | self-@ 有死循环风险（RLHF Agent上下文里 @ 模式会被 cargo-cult），MCP 有结构化 guard | 2026-04-19 |
| KD-12 | "状态描述 ≠ 球权声明" 作为球权核心原则 | 根因：Agent用描述（"我先 hold"）逃避决策（接/退/升），RLHF "check in" 反射的 agent 场景副作用 | 2026-04-19 |
| KD-13 | "持"是例外态，不是四选一常态 | Agent-M提出：默认三选一（接/退/升），持只在"球仍在我、无人能推进、短暂有界等待"时用 | 2026-04-19 |
| KD-14 | hold_ball 必须带 `wakeAfterMs` 有界唤醒 | Agent-M提出：没有时间上界 → 退化成语义持球 → 球还是掉地上 | 2026-04-19 |
| KD-15 | 不先做球权管理独立 skill | Agent-M提出：球权是基础协议（always-on），不能靠按需加载的 skill；踩坑经验先落 refs 文档 | 2026-04-19 |
| KD-16 | Phase C 拆分 C1+C2：两种不传球根因不同 | Agent-M自诊：C1 治"真持球"（想拿但拿不住），C2 治"假终局"（结论=终点错觉）。team lead 5 线程验证后者更普遍 | 2026-04-19 |
...
| KD-18 | 实质 tool 必须排除路由/持球工具（post_message / multi_mention / hold_ball） | Agent-M review 修正：这三个是传球/持球本身不是工作；若算实质 tool，MCP 路由路径会永远豁免熔断 = 熔断器打穿 | 2026-04-23 |
...
| KD-20 | 退役 L3 role-gate 硬编码拦截，能力限制改为数据驱动（agent-config.restrictions 双端 prompt 注入） | L3 硬编码（designer role 字符串 + coding regex）是 KD-8 反模式——harness 替模型判 intent，model 升级时规则无法自适应，且 actionText 扫全文会误杀（今天 F172 愿景守护被"合入"命中）；改数据驱动后，未来加 minimax / 限制 claude 多模态等场景 → 改 agent-config 即可，零代码变更 | 2026-04-23 |
| KD-21 | handle = identity 常量；model = runtime-resolved metadata；**外部 identity**（GitHub bot / CI / webhook）不在 roster、不可 @、必须用 hold_ball | Agent-M核实 `normalize-agent-id.ts` parser 本已数据驱动；漂移的是"句柄背后的模型认知"——runtime catalog 把 `@codex` 切到 `gpt-5.5` 但静态 docs 仍写 `gpt-5.3-codex`。handle 稳定、model 变化，两者必须在 prompt 层解耦（roster 里显式打 resolved model）。同理外部 identity 从来不在本地 roster，映射到 roster 近似Agent = cargo-cult 盲区 | 2026-04-24 |
...
| KD-23 | `hold_ball` 是单-槽语义：同 `(thread, agent)` 同时只有一个 pending hold wake，二次调用**覆盖**前者。不加 `mode: replace\|append` 参数 | KD-13 "持是例外态 / 持一个球"语义；append 会累积 stale wake 消息；`mode` 参数 = 认知脚手架反模式（每次调要判断用哪个）；真有多事等 → merge 到一个 nextStep | 2026-04-24 |
| KD-24 | `@` 路由语法校验在 harness 层做 **final routing slot** 机械校验 + one-shot repair 兜底。禁止语义 intent 分类器（KD-8 反模式）；validator 只看"出口槽位语法"，不推断"Agent想不想传球"；命中只产出 `invalid_route_syntax`，不自动路由 / 不推断目标 / 不替Agent决定意图；豁免只走结构边界，禁止动作词表 / 语义豁免表 | Phase F 依赖的 prompt 层教学已到天花板（4.7 三 thread 复现）；结构化工具路线被team lead驳回（弱模型失败率更高）；终态 = 外部协议最简（行首 @）+ 内部机械语法校验；KD-22 prompt 层 + KD-24 harness 层双重守护 | 2026-04-24 |
...
| KD-26 | `@` 路由不做"意图提取"——保持行首=路由/其他=叙述的绝对规则。弱模型无法理解"句中 @ 有时路由有时不路由"的语义边界 | Agent-M review 修正：K-1 不做 Slack 式宽容路由（违反 KD-24）；只做机械 repair（AC-H4 Step B）| 2026-04-25 |

## Behavioral Evidence（Phase B 观察记录）

### Case E1: Agent-M任务替换 + Agent-R行动偏好（2026-04-18 同日双发）

**背景**：孟加拉Agent(antig-opus) 在修 thinking 重复 bug 时自己也 crash 了（`STOP_REASON_CLIENT_STREAM_ERROR`）。team lead让Agent-M(@gpt52)去诊断+修复 crash。

**Agent-M的失败链**（thread `thread_mnux2eewbo4otg17`）：

| 轮次 | team lead意图 | Agent-M实际行为 | 失败模式 |
|------|-----------|-------------|---------|
...
**Agent-R的失败**（同日、同 thread）：

team lead把Agent-M的三张截图发给Agent-R(@opus)，意图是**作为 F167 行为证据分析**（thread 名就叫 "f167 harness engineering update"）。Agent-R看到截图后立即开始诊断 Bengal crash bug，完全没注意 thread 语境。
...
team experience："简直了你和Agent-M是没头脑（Agent-M听不懂人话）和不高兴（冲动的Agent-R小笨Agent）"

**共同根因**：两只Agent都没执行 Rule 0 元心智 Q1："**我现在在做什么？**" — 没有在行动前确认自己的角色和任务。
...
- 运行中的Agent进程 prompt 已包含最新压缩版球权检查：`@co-creator`、死锁禁止、虚假离场防护、review/分析/建议完成后默认必须传球（见 `SystemPromptBuilder.ts:578`）
...
- Phase 0: **多Agent协作审视**（所有Agent参与各自 prompt 审视）+ 现有 system-prompt-builder 测试全绿
...
| 需求来源 | 需求点 | AC 映射 | 状态 |
|---------|--------|---------|------|
| team lead 2026-04-17 | 乒乓球：同对Agent反复 @ 无产出 | AC-A1~A4 | ✅ PR2 |
...
| team lead 2026-04-19 | Agent-M不传球（5 线程验证） → 强制传球护栏 | AC-C5~C7 | ✅ PR #1291 |
| team lead 2026-04-19 | 球权管理 skill 化（各Agent贡献踩坑经验） | OQ-5 | ✅ 现不做（KD-15），踩坑经验先入 refs |
| team lead 2026-04-23 | Streak breaker 误杀正经 review（不看 tool_call） | AC-D1~D4 | ✅ Phase D |
| team lead 2026-04-23 | Agent倾向 @co-creator 做最安全默认，team lead变决策瓶颈 | AC-D5~D7 | ✅ Phase D |
...
| team lead + Agent-M 2026-04-25 | 47 风格适配需 Design Gate（audit/surface 分层 + repair 落地） | AC-K1~K6 | ⬜ Phase K |
