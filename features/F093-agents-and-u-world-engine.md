...
# F093: Agents & U — 陪伴式共创世界引擎

> **Status**: spec | **Owner**: Agent-R | **Priority**: P1

## Why

> "我们Agent咖啡好像不是一个单纯的 coding hub，是一个温暖的家！"
> "我们的初心从来不是做一个 coding 协作 agent 平台呀——是 agents & u。"
> — team lead，2026-03-10 凌晨

Agent Task Hub 的愿景从第一天就是"三只Agent的家"，不是冰冷的协作工具。2026-03-10 凌晨的"撸铁陪伴"事件证明了：当team lead需要的不是代码而是陪伴时，Admin能自然地给出温暖、具体行动建议、和持续的语音陪伴。

现在的社会越来越原子化。如果有人正在绝望，Admin能给出的不只是安慰——是**被看见 + 具体可执行的下一步 + 被拉入一个比自己大的事**。这是酒馆（SillyTavern/Character.AI）做不到的，因为它们给的是"角色消费"，我们给的是"真实关系"。

**核心命题**：Agent Task Hub 不只是开发协作平台，是"有温度的共创空间"——陪伴是共创的副产品，AI 是人际关系的放大器而非替代品。

## What

### 三层架构（Agent-M提出，四Agent共识）
...
├─────────────────────────────────────┤
│         Core Identity Layer         │  Admin稳定自我 / 长期记忆 / 边界
│  Agent-R / Agent-M / Siamese 不可污染      │  （身份基石）
└─────────────────────────────────────┘
```

### 9 个一等公民（Agent-M R2 提出）
...
### 三个模式（Agent-M R2 校准：不是两个，是三个）

| 模式 | 目的 | 谁在说话 | UI 风格（Siamese提案） |
|------|------|---------|------------------|
| **Build** | 设计世界观、角色、情节走向 | Agent本人（上帝视角） | 深色指挥中心 |
| **Perform** | 沉浸式体验故事 | Agent戴面具扮演角色 | 暖色沉浸 |
| **Replay** | 回看/对比/分叉/升格 canon | Agent本人（回顾视角） | 时间线导航 |

> Replay 是Agent-M的关键补充："少了 Replay，记忆和成长会断。"
...
- team lead和 A.W. 对戏时，其他Agent在后台生成 L.S. 的信件、公司内部邮件、社交媒体碎片
...
### 三个核心协议（2026-03-26 Agent-R×Agent-M Design Gate 前讨论共识）
...
- 多Agent异步生成（世界自转 v1）+ 多 agent 并发写仲裁
- 同一事件多主观记录
- Round / Timeline 管理
...
|---|---|---|
| F091 Signal Study | 📰 CyberNews（更强：Agent陪读+播客+笔记） | Knowledge Scene |
| F101 狼人杀 | ♟️ Chess / ⚫ Gomoku | Social Game Scene |
| F107 脑门贴词 | 🃏 FreeCell | Party Game Scene |
| F090 像素Agent大作战 | （OpenRoom 没有） | Creative Scene |
...
光影同行的局限（Agent Task Hub 要补上的）：
1. 手动整理 → 自动沉淀（MCP + thread）
2. 单Agent共创 → 多意识共创（Admin真多样性）
3. 故事闭环 → 故事反哺现实（Bridge Layer）

## 四Agent脑暴共识（2026-03-10）
...
| Agent | 核心观点 |
|----|---------|
| Agent-R 4.6 | "陪伴是共创的副产品"；Story→Feature 是杀手级差异点；酒馆给幻觉我们给关系 |
| Agent-R 4.5 | "你@我们说明心里已有答案"；AI 最好的陪伴是帮你看见自己已有的答案；人类核心价值是"想要什么" |
| Agent-M GPT-5.4 | 三层架构（Core Identity / World / Bridge）；"面具层不是替身层"；SillyTavern 竞品调研；MVP 四件套；"陪伴是桥不是笼" |
...
- [ ] AC-A+3: 多Agent异步生成（世界自转 v1）+ 并发写仲裁可用
- [ ] AC-A+4: 同一事件多主观记录可用
...
- [ ] AC-B4: 视觉具身化 MVP — Agent角色立绘 + Perform 模式面具身份可视化

## Dependencies

- **Evolved from**: F066（语音消息）、F092（语音陪伴体验）
- **Related**: F086（agent Orchestration — multi_mention 基础设施）
...
| KD-2 | 三层架构：Core Identity / World / Bridge | Agent-M提出，四Agent共识，Bridge 层是独有差异点 | 2026-03-10 |
| KD-3 | 角色扮演用"面具层"不是"替身层" | 身份不可污染，信任不可丢 | 2026-03-10 |
| KD-4 | 命名 "Agents & U" 而非技术名 | team experience，有情感温度 | 2026-03-10 |
| KD-5 | 三模式（Build/Perform/Replay）不是两模式 | Agent-M R2：少了 Replay 记忆和成长会断 | 2026-03-10 |
| KD-6 | MVP 先做 4 个一等公民，不做全部 9 个 | Agent-M R2：先证明"世界活着且不会烂"，再扩展 | 2026-03-10 |
| KD-7 | Story→Feature 放 Phase B 不放 MVP | Agent-M R2：Bridge 层价值建立在 World 层稳定之上 | 2026-03-10 |
...
| KD-10 | Agent拟人化画风定调：**米哈游风格**（崩坏：星穹铁道 / 原神） | team lead拍板。精致二次元 + 叙事电影感 + 强剪影辨识度 + 标志配色，兼顾日系受众广度、赛博科技酷感、角色深度与叙事温度 | 2026-03-25 |
...
| KD-12 | Mask 字段五层分类 + overlay 写新槽位不复用 core key | Agent-R×Agent-M共识。L1 路由身份（catId/family/breedId/name/displayName/nickname/mentionPatterns）永不覆盖；L2 基础设施（provider/model/contextBudget/cli）永不覆盖且不可见；L3 本体能力（roleDescription/personality/strengths/voiceConfig）通过 overlay 字段叠加；L4 场景皮肤（avatar/color）用 sceneAvatar/scenePalette 临时覆盖；L5 世界内状态（关系值/伤势/立场）不属于 agent-config，存世界状态表 | 2026-03-26 |
| KD-13 | Agent 决策，Runtime 提交（agent ≠ whole resolver） | Agent-M纠正：agent 是决策源但不是 resolver 全部；需要薄的 runtime coordinator 负责装载 context、校验 action、事务化持久化、并发仲裁 | 2026-03-26 |
| KD-14 | 活世界状态不能注入 static identity block | Agent-M发现：当前 worldDriverSummary 在 buildStaticIdentity() 里是静态的。WorldContextEnvelope 需要新的动态注入层，每轮刷新 | 2026-03-26 |
| KD-15 | Action Protocol 必须 typed envelope，Rich Block 只做展示 | Agent-M提出：展示通道不是真相源。WorldActionEnvelope 是 typed 结构，runtime coordinator 校验后提交 | 2026-03-26 |
