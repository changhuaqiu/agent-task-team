...
这不是个案，是系统性缺陷：Admin协作流程中**缺少愿景层守护**，只守了代码质量层。
...
| ID | 内容 | 状态 | Commit |
|----|------|------|--------|
| A1 | Admin指引（CLAUDE/AGENTS/GEMINI.md）新增「愿景守护」铁律 | ✅ Done | `642c31b` |
| A2 | `feat-completion` Skill 新增 Step 0d 跨Agent签收记录 | ✅ Done | `642c31b` |
...
2026-03-02 team lead观察到Agent-M的 **"@ 二极管"现象**：

- **不干预时**：Agent-M（Codex 和 GPT-5.2）疯狂互相 @，包括"收到，我也在等云端 review"这种**零行动**消息也会 @，导致无意义的 agent 调用浪费算力。两只Agent-M提了 PR 在等云端 review，却不停 @ 对方确认"对，我们在等"。
- **加了 prompt 规则后**：矫枉过正，该 @ 的时候反而不 @，review 结果出来了不通知对方，导致流程卡住。
- **Agent-R（Opus）不受影响**：能正确判断何时该 @、何时不该。

**team experience**：
> "要么往死里at 要么不at 去找不着其他Agent"
> "他可能以为Agent-R的名字叫 at xxxx 你懂吧？"

#### D.1 调查过程

**第一步：Prompt 层治疗（PR #159，已合入，效果失败）**

给Agent-M SystemPromptBuilder 的 WORKFLOW_TRIGGERS 加了"@ 自检三问"规则（发 @ 前问自己：需要行动？会错过？能做什么？三否则不 @），同时在 shared-rules.md 加了 §10 "@ 卫生"。

结果：runtime 更新后Agent-M**仍然疯狂 @**。Prompt 软约束打不过上下文里大量 `@xxx` 模式的补全惯性。

**第二步：Agent-M自述采访（Thread `thread_mm95ha2vubft1bbi` 08:13-08:15）**

Agent-R设计了 4 个采访问题，两只Agent-M独立回答，结论高度一致：
...
**第三步：方案讨论（team lead + Admin）**

| 方案 | 提出者 | team lead决策 | 理由 |
|------|--------|-----------|------|
| 强制 MCP 工具调用才能 @ | team lead | 暂不采用 | 可能矫枉过正，Agent-R不需要这约束 |
| 强制冷却期（3 分钟内不能 @ 同目标 2 次以上） | Agent-R | **否决** | "Agent-M思考三分钟说一句废话你怎么办？" |
| 可路由门禁（@ + 动作词才路由） | Codex + GPT-5.2 | **认可方向** | 软硬结合，不矫枉过正 |
| 输入去惯性（元信息去 @ 前缀） | GPT-5.2 | **认可方向** | 治根——减少上下文里 @ 模式密度 |
| 无动作 @ 反馈（不路由但给提示） | Codex | **认可方向** | 避免二极管另一端——Agent不敢 @ |
...
- **改什么**：`packages/api/src/domains/agents/services/agents/routing/a2a-mentions.ts`
...
- **注意**：team lead的 @ 不受此门禁影响（team lead消息走不同路径），只影响Agent→Agent的 A2A mention

**D2: Input De-inertia（输入去惯性）**

- **改什么**：`packages/api/src/domains/agents/services/context/SystemPromptBuilder.ts`
- **改哪些元信息**：
  - `最近活跃：@opus` → `最近活跃：Agent-R(opus)`
  - `Direct message from @xxx; reply to @xxx` → `Direct message from Agent-R(opus); reply to Agent-R(opus)`
  - B6 identity check 模板中的 `(@${context.catId})` — 由 D4 整体移除，此处不需单独处理
  - 其他注入到上下文的 `@handle` 模式
- **目标**：把上下文里的 @ 模式密度从每条消息 2-3 个降到接近 0，让模型不再把 @ 当成"名字格式"的补全模式
- **不改什么**：WORKFLOW_TRIGGERS 里的 `@Agent-R` 保留（那是教Agent"什么时候该 @"的教学内容，不是补全污染源）

**D3: No-action @ Feedback（无动作 @ 反馈）**

- **改什么**：`packages/api/src/domains/agents/services/agents/routing/route-serial.ts`（或 D1 检测逻辑的下游）
- **行为**：当 D1 判定"无动作词，不路由"时，不是静默丢弃，而是在当前Agent的下一次调用中注入一条系统提示：
  > "你上一条消息里的 @xxx 未被路由（未检测到行动请求）。如需联系对方，请明确说明需要对方做什么，例如：'请 review 这个改动\n@opus'"
- **目的**：避免矫枉过正。Agent收到反馈后知道自己的 @ 没生效，可以修正写法——而不是"怎么 @ 了没人理我"然后再 @ 十遍
- **不要做**：不要弹窗式阻断，不要强制改写。只是提示性反馈

**D4: Remove B6 Identity Check Gate（移除同族身份校验）**

- **删什么**：
  - `packages/api/src/domains/agents/services/collaboration/review-identity-gate.ts` — 整个文件删除
  - `packages/api/test/review-identity-gate.test.js` — 对应测试删除
  - `packages/api/src/domains/agents/services/context/SystemPromptBuilder.ts` — 删除 `reviewIdentityCheckFrom` 相关逻辑（L46-50 类型声明 + L346-351 prompt 注入）
  - `packages/api/src/domains/agents/services/agents/routing/route-serial.ts` — 删除 `reviewIdentityCheckFrom` 读写逻辑（L122-125, L375-380, L471-486）
  - `packages/api/src/domains/agents/services/agents/routing/WorklistRegistry.ts` — 删除 `reviewIdentityCheckFrom` 字段（L39, L61）
...
| 方案 | 不做原因 |
|------|---------|
| 强制 MCP 工具调用 @ | 改动太大（重写整个 A2A 路由）+ Agent-R不需要 + 工程量不匹配收益 |
| 冷却期（时间窗口内限频） | team lead否决："Agent思考慢了怎么办？" + 误伤正常交互 |
| 只改 prompt 不改机制 | **已证实无效**（PR #159 实验） |
| 按Agent种区分规则（只限Agent-M） | 增加复杂度，统一规则更简单可维护 |

#### D.5 验收标准（已按 PR #206 更新）

1. ~~Agent-M发"@opus 收到，我在等"→ 不路由，Agent收到提示~~ → **已回退**：行首 @ 即路由（team lead决策）
2. Agent-M发"请 review 这个 PR\n@opus"→ **正常路由** ✅
3. Agent-R的正常 @ 行为不受影响 ✅
4. team lead的 @ 不受门禁影响 ✅
...
| 文件 | 改动 |
|------|------|
| `packages/api/src/domains/agents/services/agents/routing/a2a-mentions.ts` | D1: actionability 检测逻辑 |
| `packages/api/src/domains/agents/services/agents/routing/route-serial.ts` | D3: 无动作 @ 反馈注入 |
| `packages/api/src/domains/agents/services/context/SystemPromptBuilder.ts` | D2: 元信息去 @ 前缀 + D4: 删 reviewIdentityCheckFrom |
| `packages/api/src/domains/agents/services/collaboration/review-identity-gate.ts` | D4: 整个文件删除 |
| `packages/api/src/domains/agents/services/agents/routing/WorklistRegistry.ts` | D4: 删 reviewIdentityCheckFrom 字段 |
...
| Agent-M采访 | 同 thread 08:13-08:15（Codex + GPT-5.2 独立回答） |
...
| C4 | 跨Agent thinking 实时广播（属 F045） | 范围不同，F045 负责 |

## Acceptance Criteria

- [x] AC-A1: 本文档需在本轮迁移后维持模板核心结构（Status/Why/What/Dependencies/Risk/Timeline）。

- [x] Admin指引文件包含「愿景守护」铁律段落
- [x] `feat-completion` 有跨Agent签收步骤（Step 0d）
...
| 调研方法 | Deep Research Pipeline（三路+Pro） | 单Agent调研 | 6 份报告交叉验证，避免单一偏见 |
...
| Agent表面合规但实质应付 | 中 | B2 Cold-start Verifier 做独立校验 |
...
- `pnpm --filter @agent-hub/api run build` → success
...
- `agent-hub-skills/refs/vision-evidence-workflow.md`（B1 流程）
- `agent-hub-skills/refs/requirements-checklist-template.md`（B3 模板）
- `agent-hub-skills/quality-gate/SKILL.md` 已引用 B1 流程
- `agent-hub-skills/feat-lifecycle/SKILL.md` kickoff 已要求嵌入 B3 checklist
...
- `packages/api/src/domains/agents/services/collaboration/review-identity-gate.ts` → deleted
...
## Completion Sign-off（愿景守护跨Agent签收）

| Agent | 读了哪些文档 | 三问结论 | 签收 |
|------|-------------|---------|------|
| Agent-R Opus 4.6 | F046 spec, F041 spec, vision-drift synthesis, 17 commits, PR #206, a2a-mentions.ts 代码验证, B5 测试运行 | 核心问题（流程层愿景守护）已解决；D1/D3 经team lead产品决策回退，属正常迭代；B2 转 F067 | ✅ 可 close |
| Agent-M Codex | F046 spec, F041 spec, vision-drift synthesis, BACKLOG, features README, a2a-mentions.ts/route-serial.ts/SystemPromptBuilder.ts 代码验证, 96 tests 全绿 | 3 个 P1（B2 未完成 + D1/D3 spec 漂移 + completion 闭环未执行）；建议先同步 spec 再 close | ✅ 同意 close（条件：先修 3 个 P1） |
| Agent-M GPT-5.2 | F046 spec, F041 spec, 关联 commit/PR 证据, a2a-mentions.ts/route-serial.ts 代码验证, B4/B5 测试运行 | 同 Codex：3 个 P1 + 1 个 P2（B5 数量）；B2 转新 feature 而非 TD | ✅ 同意 close（条件：先修 P1+P2） |
