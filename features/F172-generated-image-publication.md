...
> **Status**: done (Phase H merged 2026-04-24) | **Owner**: Agent-R/opus | **Priority**: P1
>
...
目前Agent用 built-in `image_gen` 生成图片时，文件默认落在 `~/.codex/generated_images/...`。图片本身能生成，但**没有自动晋升为 Agent Task Hub 的一等产物**：

1. 前端 rich block / message content 的真资源链路以 `/uploads/...` 为准，`.codex` 路径不在当前 runtime 服务范围内。
2. Agent如果想展示这张图，只能靠手工把本地文件搬进当前 `uploadDir`，再自己发 `media_gallery` rich block。
3. 孟加拉Agent/Antigravity 虽然本来就有图片生成能力，但其 provider 输出同样还没有接到 Agent Task Hub 的统一图片 artifact 发布链路上。
4. `image-generation` / `rich-messaging` 等家里 skill 现在仍在教Agent“手工搬图 + 手工写 `/uploads/...`”，这说明契约还停留在人工约定，没有下沉成基础设施。
...
这层是 F172 的核心，不归属某一只Agent，也不绑定某一个 provider。
...
目标不是只“让 Codex 能显示图”，而是让它生成的图片从一开始就是 Agent Task Hub 的正式 artifact。

### Phase C: Antigravity 图片输出接入

把孟加拉Agent/Antigravity 的图片生成结果，接到同一条共享发布内核上。

注意边界：
- F061 继续拥有 Antigravity provider/bridge/step taxonomy 本身
- F172 只拥有“当 Antigravity 已经生成出图片后，如何发布成 Agent Task Hub artifact 并呈现”这条后半段

### Phase D: Skill 契约与使用路径收口

把家里和图片生成/展示相关的 skill 说明收口到新契约上：

- `agent-hub-skills/image-generation`：不再把“下载到本地后手工 cp”当成终态
- `agent-hub-skills/rich-messaging` / `refs/rich-blocks.md`：从“手工搬运指南”升级为“共享发布内核的消费规则”
- 对Agent的最终使用体验是：不管走 Codex built-in、Antigravity，还是浏览器自动化生成，只要产物要进 thread，就统一晋升为 `/uploads/...` + `media_gallery`
...
- generated-image publication contract 已落地：`packages/api/src/domains/agents/services/agents/providers/generated-image-publication.ts`
- 新增测试：
...
- Codex image scanner：`packages/api/src/domains/agents/services/agents/providers/codex-image-scanner.ts`
...
- [x] AC-C2: 孟加拉Agent生成的图片与 Codex 生图在 thread 中采用同一种 `/uploads/...` + `media_gallery` 呈现方式

#### Phase C 实施证据（2026-04-23）

- Antigravity image publisher：`packages/api/src/domains/agents/services/agents/providers/antigravity/antigravity-image-publisher.ts`
...
### Phase D（Skill 契约收口）
- [x] AC-D1: `agent-hub-skills/image-generation` 明确改为消费共享发布内核，不再把手工复制文件当终态
- [x] AC-D2: `agent-hub-skills/rich-messaging` / `refs/rich-blocks.md` 更新为新的图片发布约定

#### Phase D 实施证据（2026-04-23）

- `agent-hub-skills/image-generation/SKILL.md`：manual `cp` → `publishGeneratedImage()` auto-publish
- `agent-hub-skills/rich-messaging/SKILL.md`：manual uploadDir copy → F172 contract
- `agent-hub-skills/refs/rich-blocks.md`：manual copy instructions → auto-publish explanation
...
| R4 | “这样你们生成完成之后 两只Agent都能够直接呈现给我” | AC-B1, AC-C2, AC-E1 | manual + integration test | [x] |
...
- **Related**: F061（孟加拉Agent provider 本身继续拥有图片生成能力与 bridge 主线；F172 只接它的图片发布后半段）
...
| KD-3 | F172 覆盖 Codex built-in、Antigravity、repo-local skills 三个入口，但只统一“生成完成后的图片发布链路” | team lead明确要求两只Agent都能直接呈现；真正共享的不是各自的生图方式，而是 artifact publication contract | 2026-04-22 |
