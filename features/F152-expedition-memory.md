...
> **Status**: in-progress | **Owner**: Agent-R | **Priority**: P1 | **Close Gate**: 代码链已闭环，等待 AC-C5 team lead全链路终验
...
Agent Task Hub 的Agent就是 AI 版 FDE：带着知识工程方法论，"部署"到用户的业务系统中，指导和完成开发。AI FDE 比人类 FDE 有一个关键优势——**跨客户知识迁移**：Agent在项目 A 学到的领域模式，回流全局层后，去项目 B 的Agent直接就能用。人类 FDE 很难做到这种规模化经验复用。

大量企业已经完成信息化（有系统、有代码、有文档、有流程），但信息化如何与 AI 结合还缺少探索。F152 就是让Agent能当 AI FDE 的底层能力——**冷启动理解业务 + 经验跨项目回流**。

### 具体痛点

社区用户用Agent去做他们自己的项目——鸿蒙 app 迁移、昇腾算子迁移、已有 codebase 改造。这些项目不是从零开始的，有自己的代码、文档、历史。

F102 已经做完了记忆引擎（6 接口 + SQLite 基座 + 全局/项目层 + 联邦检索），F070 做完了治理/方法论随Agent走。但Agent去到一个**已有的外部项目**时：

1. **没有记忆**：项目没有 `evidence.sqlite`，IndexBuilder 只认 agent-hub 的 `docs/` 结构，无法吃进 README / CHANGELOG / 散落 .md / package metadata
2. **无法快速理解**：项目已有大量代码和文档，Agent每次从零开始读
3. **经验不回流**：Agent在外部项目踩的坑（如"鸿蒙某 API 兼容性问题"）沉淀在那个项目里，下次去别的鸿蒙项目时用不上
...
>
> 社区洞察（2026-04-09）："这些小Agent可以变为 Palantir 概念里面的 FDE，指导和完成业务系统的开发。"

## What

### Phase 0: Knowledge Engineering Skill — Agent指导外部项目文档重构
...
Agent进入外部项目
  ...
  │   → Agent用 knowledge-engineering skill 指导文档重构
  │   → 三层知识注入（领域手册 → 模式库 → 检索管道）
  │   → 重构后 Agent Task HubScanner 直接吃
  │
...
**Skill 还要能处理的特殊场景**：
- 代码仓和文档仓分离 → Agent要能识别并提醒用户"文档放什么、怎么放"
- 文档散落在 wiki/Confluence/飞书 → Agent要能指导迁移策略
- 项目只有代码没有文档 → Agent从代码结构推导出文档骨架建议

### Phase A: GenericRepoScanner — 让 IndexBuilder 能吃非 agent-hub 结构的项目
...
- `Agent Task HubScanner implements RepoScanner`：从现有 `IndexBuilder.discoverFiles()` 抽出
...
**v1 不扫**（Agent-M否决，噪音高+性能贵）：commit message patterns、code comments。
...
### Phase B: Expedition Bootstrap Orchestrator — Agent进新项目时自动冷启动记忆

Agent进入外部项目后的自动化编排流程：

```
检测 evidence.sqlite 是否存在
  ├─ 存在 → 检查新鲜度 → 需要更新？→ incremental rebuild
  └─ 不存在 → 触发 bootstrap:
       1. 选择 scanner（检测项目结构 → Agent Task Hub or Generic）
       2. 运行 scanner → 建索引
...
- 已有 agent-hub 结构的项目：直接用 `Agent Task HubScanner`，不走 Generic
...
Agent在外部项目产生的 lesson / decision，如果具有跨项目泛化价值，应该回流到 `global_knowledge.sqlite`。
...
- [x] AC-01: `knowledge-engineering` skill 存在且可被Agent加载
- [x] AC-02: Skill 能识别外部项目的文档现状（有结构化文档 / 只有代码 / 文档散落 / 代码文档分仓）
- [x] AC-03: Skill 输出三层知识注入建议（领域手册 → 模式库 → 检索管道），内容基于 IdeaHub 咨询方法论
- [x] AC-04: Skill 能生成文档骨架模板（概念词典、规则表、操作映射），用户填充后可被 Agent Task HubScanner 索引
- [x] AC-05: Bootstrap 流程中，Agent在路径选择点（Guided vs Autonomous）向用户说明两条路径的差异

### Phase A（GenericRepoScanner）✅
- [x] AC-A1: `GenericRepoScanner` 能扫描一个没有 agent-hub `docs/` 结构的普通 Git 仓库，产出 `ScannedEvidence[]`
- [x] AC-A2: 每个 `ScannedEvidence` 带 `provenance: { tier: 'authoritative'|'derived'|'soft_clue', source: string }`（canonical naming，统一全 spec）
- [x] AC-A3: `IIndexBuilder` 根据项目结构自动选择 `Agent Task HubScanner` 或 `GenericRepoScanner`
...
- [x] AC-B1: Agent进入一个没有 `evidence.sqlite` 的外部项目时，自动触发 bootstrap
...
- [x] AC-B8: **老用户路径（场景 B）**：已做过治理 bootstrap 但无记忆索引的项目，Agent进入时主动提示确认卡（含扫描范围、预计耗时、本地索引说明）；用户可选"稍后"并 snooze（7 天冷却，不反复打扰）
...
| R2 | Agent去外部项目能快速理解项目现状 | AC-B1, AC-B2 | manual: bootstrap 后Agent能回答项目基本问题 | [x] |
| R3 | "用你们开发其他项目" — 不要求先搭 agent-hub 标准目录 | AC-A1, AC-A3 | test: 无 docs/ 结构的仓库能正常扫描 | [x] |
| R4 | Agent踩的坑能带回来下次用 | AC-C1~C4 | manual: 一条经验从外部项目回流到全局层 | [x] |
| R5 | "代码仓可能和文档分开" — Agent要能识别并提醒 | AC-02 | manual: Agent检测到文档分仓场景时给出建议 | [x] |
...
- **Evolved from**: F070（Portable Governance — 治理/方法论随Agent走 + 治理 bootstrap 链路 `projects-setup`）
...
| KD-2 | Scanner 输出带 provenance + 三层置信度 | Agent-M护栏：不带来源信息后面无法区分置信度、无法决定回流策略 | 2026-04-08 |
| KD-3 | Global distillation fail-closed（默认不回流） | Agent-M护栏：防止甲方私有语境污染全局层 | 2026-04-08 |
...
| KD-6 | provenance 持久化到存储层（`provenance_tier` + `provenance_source` 列） | 否决 keywords hack（编码到 keywords 数组），Agent-M判定为 hack | 2026-04-08 |
...
| KD-10 | Phase A v1 不扫 commit messages 和 code comments | 噪音高、语言相关、性能贵，Agent-M否决 | 2026-04-08 |
| KD-11 | 经验回流双层路由：Agent审核通道（四条件同时满足：provenance≥derived + 可验证 + 事实型 + 已脱敏）+ team lead审核通道（命中任一敏感条件即上升）；Phase C 初期先全量人审校准再逐步放权 | Agent-R×Agent-M(GPT-5.4) 讨论收敛 + team lead授权分层 | 2026-04-09 |
| KD-12 | index_state 五态状态机（missing/stale/building/ready/failed）替代简单文件存在检测 | Agent-M护栏：单靠 db 是否存在无法区分"过期""失败""构建中"，会导致误判和重复提示 | 2026-04-10 |
| KD-13 | 幂等 key = projectRoot + headCommit + scannerVersion + scanMode；服务端 in-flight lock 防重复扫描 | Agent-M提议 + Agent-R采纳，防止多 session 并发触发重复 bootstrap | 2026-04-10 |
| KD-14 | 摘要卡结构化优先 + LLM optional 增强（不违反 AC-B6）| Siamese提议 LLM 一句话定调 vs AC-B6 不绑 LLM 额度；折中：结构化默认，LLM 可用时润色 | 2026-04-10 |
| KD-15 | 老用户"稍后"snooze 机制（7 天冷却）| Agent-M护栏：不加冷却会反复打扰老用户 | 2026-04-10 |
...
| KD-17 | Phase B review 时Agent-M必须启动 dev 截图验证前端，不能只看代码 | team lead要求：前端改动必须实际打开浏览器验证，防止"代码对了但 UI 离谱" | 2026-04-10 |

## Review Gate

- Phase A: 跨家族 review（Agent-M优先）
