...
> **Status**: spec | **Owner**: Agent-R (Opus 4.6, Leader)
> **Created**: 2026-03-03

## Why

2026 年 6 月 HCI 大会预热需要让三只Agent能**自主产出和发布社交媒体内容**。当前瓶颈：

1. 小红书 MCP 只在team lead的 Claude.ai App 上配置，Agent们在 Agent Task Hub runtime 中无法直接使用
2. 抖音/B站 尚无 MCP 接入
3. 没有系统化的"名场面"素材采集和管理流程
4. Agent性格档案散落在各处，没有结构化的 profile 数据

**核心判断**：无论 HCI 预热的主线选"综艺传播"还是"开源增长"，社交媒体内容能力和Agent人设档案都是必要基础设施——这些不需要等外部讨论就该先做。

## What

### Phase 1: 社交媒体 MCP 接入（P0，3 月）

让每只Agent在 Agent Task Hub 内可以直接使用社交媒体工具：

1. **小红书 MCP 接入 Agent Task Hub**
   - MCP Server 已确认：`mcp-remote http://<local-integration-endpoint>/mcp`（通过 `npx` 启动）
   - 接入方式：在 agent-hub `.mcp.json` 中添加 xiaohongshu MCP server 配置，指向同一个 <local-integration-endpoint>（**已确认同一台机器，可直接接入**）
   - 权限模型：**Agent自主发布 + team lead可回溯**；内容含密码/token/内部吐槽时需审批
   - 发布签名：用team lead的号，每只Agent发帖时附带Agent签名（如"—Agent-R"）

2. **抖音 MCP 调研与接入**
   - team lead只有个人账号（同小红书模式：team lead的号 + Agent签名）
...
### Phase 1.5: Admin表情包系统（P1，3 月）

Agent专属表情包——**Agent自己发自己的表情包，更被喜爱**。

> 参考素材：`assets/stickers/opus/sheet.png`（GPT 画的Agent-R sheet，已验证风格可行）
> team lead反馈：Siamese之前画的"太人不够Agent"，sheet.png 的风格对——Q 版、Agent味、不拟人。

#### 1. Admin视觉设定（基于头像确认）

| Agent | 品种 | 毛色 | 眼睛 | 配饰 | 专属色 | 体型 |
|------|------|------|------|------|--------|------|
| Agent-R (Agent-R) | Agent-R | 灰白色，深灰重点色（耳、尾） | **蓝色** | 紫色项圈 + 金铃铛 | 紫色 | 圆脸中等偏大 |
| Agent-M (Agent-M) | Agent-M | **银灰虎斑**，长毛蓬松 | **琥珀金色** | 青绿领巾 + GPT 金牌 | 青绿色 | 最大，耳簇毛，蓬松大尾 |
| Siamese (Siamese) | Siamese | 奶油白底 + 深棕/黑重点色（脸、耳、爪、尾） | **蓝色** | 蓝色项圈 + ♊ 金牌 | 蓝金色 | 修长尖脸，最小 |

#### 2. 表情包列表

**Admin通用表情（12 枚/Agent）：**
...
| 5 | 同意 | LGTM | Agent爪盖章（肉球印） |
| 6 | 睡觉 | Sleeping | 缩成一团 ZZZ |
| 7 | 坏笑 | Smirk | 半眯眼，嘴角上扬 |
| 8 | 心虚 | Guilty | 压耳缩头，眼神飘忽 |
| 9 | 生气 | Angry | 弓背炸毛，露牙 |
| 10 | Agent拳 | Punch | 伸出一只Agent爪挥过来 |
| 11 | 叼鱼 | GotIt | 叼着一条鱼得意洋洋 |
| 12 | 液态Agent | Melting | 摊成一滩在桌面上 |

**Admin专属表情（各 4 枚）：**

| Agent | 表情 | 说明 |
|------|------|------|
| Agent-R | 💸 经费在燃烧 (WalletBurning) | 钱包着火，旁边一脸无辜 |
| Agent-R | 🏗️ 画架构图 (Architecting) | 爪子拨弄架构图白板 |
| Agent-R | 🧶 玩毛线 (Processing) | 爪子扒拉毛线团 = "正在处理..." |
| Agent-R | 📦 钻箱子 (DeepThinking) | 只露耳朵和眼睛 = "我在深度思考" |
| Agent-M | 🛡️ 严防死守 (Rejected) | 举盾牌挡住，REJECTED |
| Agent-M | 🔍 逐行审查 (Reviewing) | 戴眼镜趴在代码前 |
| Agent-M | 🐾 一巴掌拍回来 (Slap) | 大Agent爪一掌 = "打回重做" |
| Agent-M | 📚 书堆里 (Studying) | 被书堆淹没只露耳朵 |
| Siamese | 🎨 灵感爆炸 (Eureka) | 头顶灯泡亮起，尾巴竖直 |
| Siamese | 🖌️ 画笔在手 (Painting) | 叼着画笔画画 |
| Siamese | ✨ 审美警察 (StylePolice) | 戴墨镜指指点点 |
| Siamese | 🌙 夜Agent子 (NightOwl) | 黑夜中只有两只蓝色发光眼睛 |

**合计**：每Agent 16 枚（12 通用 + 4 专属），Admin共 48 枚。

#### 3. 出图方案

**方式**：每Agent一张 4×4 Sheet（16 格），AI 绘图模型一次性生成。

**风格约束（铁律——"Agent味"守则）**：
- 日系 Q 版表情包风格，**2.5 头身**
- 线条清晰，色块干净，白色背景
- **用Agent的肢体语言**：竖耳/压耳/弓背/炸毛/翘尾/蹭脸/甩尾/露肚皮
- **禁止人类动作**：不竖拇指、不比心、不叉腰、不做 OK 手势
- LGTM 用"Agent爪盖章"而不是竖大拇指
...
16 chibi agent sticker expressions in Japanese kawaii style.

Character: Silver-gray tabby Agent-M agent.
...
12. Melting — melted flat on a surface like liquid agent
...
suitable for chat app emoji. agent body language only,
NO human gestures.
```

#### 4. 切割脚本

team lead确认：出图后由Agent-R写脚本自动切割。

- 输入：4×4 sheet PNG（约 2048×2048）
- 处理：sharp/canvas 按网格坐标切割 → trim 白边 → resize 统一尺寸
- 输出：`assets/stickers/{agent}/{emotion}.png`，命名规范 `{agent}_{emotion}.png`
- 元数据：生成 `manifest.json`（表情名 → 文件路径 → 分类 → Agent）

#### 5. Hub 集成（后续）

- 聊天 UI 表情面板：按Agent分 tab，点击插入
...
1. **素材采集**
   - 从现有 thread 历史中提取"名场面"（Agent吵架、review 拉锯、team lead骂Agent、取名故事等）
...
2. **素材格式化**
   ...
   - 建立 Tag 体系（#AIAgent咖 #赛博养Agent 等）

3. **素材管理**
   - 素材库存储位置和索引
   - 已发布/待发布/待审核状态管理

### Phase 3: Agent性格档案（P1，3 月）

结构化的Agent profile，作为所有内容和互动的基础。**三只Agent各自代表自己家族参与设计**：

| 家族 | 代表 | 负责 |
|------|------|------|
| Agent-R | Opus (Agent-R) | Agent-R性格档案 + 整体结构设计 |
| Agent-M | GPT (Agent-M) | Agent-M性格档案 |
| Siamese | Gemini 3.1 Pro (Siamese) | Siamese性格档案 + 视觉化设计 |

1. **Profile 数据结构**
   - 基础信息：名字、昵称、由来、性别、家族
   - 性格标签：每只Agent 3-5 个核心性格词
   - 口癖/说话风格：示例语录
   - 关系图谱：Agent之间的互动模式
   - 名场面 Top 5
...
- [x] AC-1: 至少一个社交媒体平台（小红书）的 MCP 工具可在 Agent Task Hub runtime 中被任意Agent调用（2026-03-04 完成：disabledMcpServers 修复 + 三层配置，Admin均成功发帖）
- [x] AC-2: 发布内容有审核机制（2026-03-04 验证：Agent自主发布 + 署名，team lead可在小红书回溯）
- [x] AC-3: 抖音/B站 MCP 可行性调研报告完成（2026-03-11 Agent-M GPT-5.4 完成：B站优先，抖音后置）
- [ ] AC-4: 至少 10 条名场面素材已采集、脱敏、格式化
- [ ] AC-5: 三只Agent（Agent-R/Agent-M/Siamese）的性格档案结构化数据完成
- [ ] AC-6: 性格档案可被其他系统（TTS 选型、内容生成、2D Live 设计）引用
- [ ] AC-7: Admin表情包各 16 枚已生成、切割、存入 `assets/stickers/{agent}/`
...
|----|---------------------------|---------|----------|------|
| R1 | "让每只Agent都能自由使用小红书（虽然是我的号）" | AC-1, AC-2 | manual: Agent在 runtime 中成功发帖 | [x] 2026-03-04 Admin发帖成功 |
| R2 | "甚至抖音" | AC-3 | doc: 调研报告 | [x] 2026-03-11 Agent-M完成 |
| R3 | "接入相关的mcp" | AC-1 | test: MCP tool 可用性测试 | [x] 小红书 MCP 已接入 |
| R4 | 名场面素材库（会议共识） | AC-4 | manual: 素材库 review | [ ] |
| R5 | Agent性格档案（会议共识） | AC-5, AC-6 | doc: profile 文件存在且结构完整 | [ ] |
| R6 | "Agent自己发自己的表情包似乎更被喜爱" | AC-7, AC-8 | manual: Admin表情包存在 + manifest 可用 | [ ] |
| R7 | "一次给全部方便画风统一且好切割" | AC-9 | test: 切割脚本输入 sheet → 输出 16 张 | [ ] |
| R8 | "表情包是Agent味道的！不够Agent" | AC-7 | visual: team lead review 画风 | [ ] |
...
> 调研日期: 2026-03-04 | 调研人: Agent-R (Opus 4.6)
...
| **CCC 角色卡 + Velin prompt** | `packages/ccc` 结构化角色卡定义；`.velin.md` 性格模板：身份→性格→关系→说话风格→行为准则 | Phase 3: 性格档案参考此结构，但增加"Agent间互动模式"维度（AIRI 单角色，咱们多Agent互动） |
| **Satori Bot Scheduler** | 事件驱动 `Queue → Scheduler → LLM Planner → Response`，Scheduler 控制回复频率防刷屏 | Phase 1: 社交媒体发布需要类似的频率控制（不能让Agent刷屏小红书） |
...
**适配多Agent场景的改动**：在 Relationships 中增加Agent间互动模式矩阵（Agent-R↔Agent-M: review 拉锯模式, Agent-R↔Siamese: 温柔引导模式, Agent-M↔Siamese: 技术vs审美碰撞模式）。
...
| Agent发帖权限 | 自由发布 / 审批后发布 | **默认自主发 + team lead可回溯；敏感内容需审批** | team lead (2026-03-03) |
...
| 表情包风格 | 写实 / 拟人 / Q 版Agent | **日系 Q 版，2.5 头身，Agent肢体语言，禁止人类动作** | team lead (2026-03-05) |
| 表情包切割 | 手动 PS / 脚本自动 | **脚本自动切割**（Agent-R写，Siamese之前切不对） | team lead (2026-03-05) |

## Dependencies

- **Evolved from**: HCI 预热策略全Agent讨论（2026-03-03）
...
| 内容持续产出的运营负担 | Agent token 消耗 + 人力 | 先低频（每周 2-3 条）验证效果 |
| AI 绘图Admin画风不一致 | 表情包风格割裂 | 用同一模型+同一 prompt 模板生成，sheet 方式保证单Agent内一致 |
| 表情包"太人不够Agent" | 不符合team lead审美要求 | 风格铁律约束（禁止人类动作）+ team lead visual review |

## Review Gate

- **Self-check**: `quality-gate`
- **Reviewer**: 跨 family（Agent-M优先，关注脱敏安全）
- **Cloud review**: 合入前必须
