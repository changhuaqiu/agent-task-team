...
- **F041**：team lead视角 — 能力看板 UI + 配置编排器 + 全局/每Agent开关
- **F043**：Agent的视角 — MCP server 本身怎么拆分、新增哪些协作工具
...
1. **1 个 MCP server 挂 27 个 tools 平铺**：所有 tool schema 注入系统提示，prompt 臃肿，Agent选工具认知负担大
2. **协作工具缺口**：Agent不能按 catId 过滤消息、不知道有哪些 thread、不能跨 thread 通知、没有 feat→thread 索引
3. **file tools 冗余**：宿主 CLI 自带文件操作，MCP 再包一层无意义

### Agent的实际痛点（Agent-R 4.5 + 4.6 共同反馈）

- **近视眼**：只能看到当前 thread，不知道其他 thread 的决策
- **接力棒丢了**：feat 接力时找不到前序 feat 的讨论在哪个 thread
- **肉眼找Agent**：想看"Agent-M说了什么"只能 `get_thread_context(limit=100)` 然后肉眼翻
...
```
现状:
  agent-hub-mcp (1 server, 27 tools 平铺)

目标:
  ① agent-hub-collab  (协作核心, ~14 tools)  ← Admin必装
  ② agent-hub-memory   (记忆与回溯, ~9 tools) ← 按需
  ③ agent-hub-signals  (信号猎手, 5 tools)    ← 按需
```

#### ① agent-hub-collab（协作核心）
...
```typescript
// 新增参数
catId?: CatId | 'user'   // 按Agent过滤（'user' = team lead消息）
...
|------|-------------------|-------------------|------|
| 每次默认暴露的工具数（协作主链路） | 30（legacy `agent-hub-mcp` 全量） | 15（仅 `agent-hub-collab`） | **-50%** |
...
2026-02-27 team lead + Agent-R (Opus 4.6) + Agent-R (Opus 4.5)，F037 Agent Swarm 后续讨论。

核心问题由team lead提出："agent 之间的协作，在 thread 之内和跨 thread 会用到什么功能？现在的搜 codebase 够吗？Agent咖啡如何进化给你们更多可能性？"
...
|------|-------------|---------|------|
| Agent-R (Opus 4.6) | F043 spec, VISION.md, tool-registration.test.js, callback-tools.test.js | ① team lead不想当人肉路由器 ② 7/7 AC 通过，愿景对齐 4/5 ③ Agent能自主搜消息/发现 thread/跨 thread 通知/查 feat 映射 | ✅ |
| Agent-M (GPT-5.2) | F043 spec, VISION.md, tool-registration.test.js (50/50), capability-orchestrator.test.js (51/51) | ① team lead不想当路由器+Agent要自主找上下文 ② 交付物直接贡献愿景 #1/#2/#3/#5 ③ 查证据/找入口/发接力棒/管任务四条链路可用 | ✅ |
