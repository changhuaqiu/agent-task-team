...
> **Phase 4 Closed**: 2026-03-11 — TTS 韧性增强合入（PR #356, Agent-M R2 + 云端 Codex 双关放行）
...
3. **无播放调度**：没有优先级/排队/打断机制，无法支撑双Agent交替对话（F021++ 播客）
...
3. **声线试听脚本 + 声线选择**
   ...
   - team lead试听所有 `zm_*` 声线，为每只Agent选定声线
   - 三只Agent的声线期望描述（供team lead参考）：
     - **Agent-R** (Agent-R)：偏低沉温暖，语速略慢 (0.95)，"安静讲故事"
     - **Agent-M** (Agent-M)：清朗干脆，语速标准 (1.0)，"认真审稿的编辑"
     - **Siamese** (Siamese)：明快年轻，语速略快 (1.05)，"灵感停不下来的设计师"

4. **agent-voices.ts 声线更新**
...
2. **双Agent播客支持**（服务 F021++ R5）
   - 两只Agent的语音片段按 queue 行为交替播放
   - 每段播放完自动切到下一只Agent

3. **用户交互**
   - 用户说话时Agent停嘴（interrupt，需 VAD 信号）
   - 播放暂停/跳过控制
...
- [ ] AC-6: (Phase 3) 双Agent对话稿可按 queue 模式交替播放 → 拆分至未来 Feature
...
| R4 | 双Agent交替对话播放（AIRI Intent 系统启发） | AC-6 | test: queue 行为验证 | [ ] 拆分 |
...
| 升级路径 | 一步到位 / 渐进 | **渐进**：Qwen3 1.7B → 补 stream_synthesize + chunker → CosyVoice3(可选上限) | Agent-R+GPT-5.4 |
| Python TTS 替换策略 | 写死替换 / Adapter 模式 | **Adapter 模式**：`TtsAdapter` 抽象 + env var 切换 provider | team lead (2026-03-05) |
| 声线选择流程 | Agent自选 / team lead选 | **Agent出期望描述 → team lead试听拍板**（Agent听不到声音） | team lead (2026-03-05) |
...
| Agent-R声线方案 | Qwen3 VoiceDesign / GPT-SoVITS / Qwen3 Base clone | **Qwen3-TTS Base clone + 流浪者 v2 ref audio** — clone 模式 + instruct 叠加解决一切 | team lead (2026-03-09) |
| Agent-M声线方案 | Qwen3 VoiceDesign / GPT-SoVITS / Qwen3 Base clone | **Qwen3-TTS Base clone + 魈 v2 ref audio** — 统一引擎 | team lead (2026-03-09) |
...
| 声线方案架构 | D 型混合(Qwen3+GPT-SoVITS) / E 型统一(Qwen3 Base clone) | **E 型统一方案** — Admin都走 Qwen3 Base clone，GPT-SoVITS 降为离线工具 | team lead (2026-03-09) |
...
- **Reviewer**: 跨 family（Agent-M优先，关注 Provider 接口兼容性）
- **Cloud review**: 合入前必须
...
| Agent | 引擎 | 角色参考 | 参考音频 | instruct |
|------|------|---------|---------|----------|
| **Agent-R** | Qwen3 Base clone | 流浪者 v2 | `vo_wanderer_dialog_greetingMorning.wav` | 调皮狡黠、得意戏弄 |
| **Agent-M** | Qwen3 Base clone | 魈 v2 | `vo_xiao_dialog_close2.wav` | 傲娇冰山、严厉关心 |
...
- [x] 声线配置固化到 `agent-voices.ts` — PR #333 合入 main (f27b827d)
...
| VoiceBlockSynthesizer clone passthrough | PR #333 | clone 参数从 agent-voices → synthesize() 全链路透传 |
| agent-voices.ts E 型统一配置 | PR #333 | Admin声线：流浪者/魈/班尼特 + Kokoro 兼容 voice ID |
| Clone-aware timeout (30s→120s) | e57d81ae | 长文本 clone 合成防超时 |
| Agent-M R1→R3 review + 云端 review | PR #333 | 5 findings (3P1+1P2) 全部修复 |
...
- **教训**：Agent Task Hub 内容中英混杂频率高（代码术语、Agent名等），GPT-SoVITS 不适合做在线引擎。降级为离线声库工具

### 坑 4: Clone 合成超时导致语音降级
- **现象**：长文本（~200 字"Rogue Agent计划"）发送后显示 `🔇 语音合成失败`
...
- **修复**：所有Agent统一恢复为 `zm_yunjian`（Kokoro 兼容 ID）
- **教训**：voice ID 是模型层面的标识符，不是用户友好名。agent-voices.ts 的 `voice` 字段必须用模型认可的值
...
- **根因**：在 agent-hub-runtime 目录下运行的是旧代码（未 pull 最新 main）
