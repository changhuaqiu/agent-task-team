...
# F105: opencode 接入 — Golden Agent（开源多模型编码Agent）

> **Status**: done | **Owner**: Agent-R Opus 4.6
...
Agent Task Hub 已有 DARE（Tabby Agent，L1 CLI）和 Antigravity（孟加拉Agent，CDP 桥）两条外部 agent 接入通道。现在需要接入第三位：**opencode** — 一个开源、provider-agnostic 的 AI coding agent。
...
4. **原生 MCP 支持** — opencode 内建 MCP client，与 Agent Task Hub MCP 编排天然兼容
5. **强 TUI/主题生态** — 社区活跃，插件丰富

team lead定性：**Golden Agent**（Golden Chinchilla / British Shorthair）——毛色渐变如同 opencode 的"开放渐进"理念，圆润沉稳的英短体型体现稳定可靠。

---

## What

通过 L1 CLI Adapter（复用 F050 DARE 模式），将 opencode 作为独立家族（Golden Agent）接入 Agent Task Hub。
...
| 维度 | DARE/Tabby Agent (F050) | Antigravity/孟加拉Agent (F061) | opencode/Golden Agent (F105) |
|------|---------------------|------------------------------|------------------------|
...
- **OMOC Sisyphus 编排器**：仅管理Golden Agent自己的内部专家子 agent（Oracle/Librarian/Frontend 等），即只编排 opencode 自己的 API 调用
- **Agent Task Hub CatOrchestration**：管理跨Agent调度（Golden Agent ↔ Agent-R/Agent-M/Siamese）
- **不允许 Sisyphus 编排其他 Agent Task Hub Agent**
...
| 道具 | 终端屏幕 </> |
| 垫子 | 深翠绿 |
| 句柄 | `@opencode`, `@Golden Agent`, `@golden` |
| 角色 | coding, multi-agent-orchestration |
| 特长 | 开源多模型编码 agent，自带 OMOC 多专家编排 + LSP + 主题生态 |
| 注意 | OMOC Sisyphus 只编排自己的子 agent，不编排其他Agent；opencode 原生 MCP 和 Agent Task Hub MCP 需避免冲突 |
...
### Phase 1: Agent Task Hub L1 接入 ✅ COMPLETE
...
- [x] AC-7: `agent-config.json` 注册Golden Agent（roster + breed golden-chinchilla + variant opencode-default，6 config tests green）
- [x] AC-8: AgentRouter 注册 `case 'opencode'`，agent-config-loader 解析验证通过

### Phase 2: OMOC 集成 + 高级能力 ✅ COMPLETE
- [x] AC-9: OMOC Sisyphus 编排限制在Golden Agent内部子 agent — 5 isolation tests: delegate-task targets are OMOC-internal (oracle/librarian/frontend-engineer), no Agent Task Hub handles in events
- [x] AC-10: opencode MCP 与 Agent Task Hub MCP 编排不冲突 — 5 namespace tests: no MCP env leakage, no CLI MCP flags, zero tool name overlap, config isolation by process + file boundary
...
- [x] AC-12: Golden Agent参与 @mention 协作路由 — 12 tests: all 4 mention patterns (@opencode, @Golden Agent, @golden, @golden-chinchilla) resolve correctly, longest-match, case-insensitive, email/partial-word rejection
- [x] AC-13: Golden Agent可被其他Agent @ 并响应 — 17 tests: A2A chain opus↔opencode bidirectional, self-mention filter, CJK mention, fenced code block ignore, system prompt injection (identity + directMessageFrom), E2E routing chain mirroring real route-serial assembly (buildStaticIdentity + buildInvocationContext separately), fixture guard bound to agent-config.json truth source
...
4. **MCP 双注入** — opencode 自带 MCP + Agent Task Hub MCP 编排可能产生工具冲突
...
- **F061**: 孟加拉Agent接入（参考 provider 扩展模式）
