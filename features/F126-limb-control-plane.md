...
# F126: 四肢控制面 — Agent Task Hub Limb Control Plane

> **Status**: in-progress | **Owner**: Agent-R | **Priority**: P1

## Why

team lead 2026-03-16 在Admin OpenClaw Node 研讨中指出：

> "你们这群小笨蛋想浅了。他们会想要——如果你们这一群Agent军团，你们要如何管理多个不同的四肢？你们如何在 Mac 上管理一堆其他的 Windows 节点？"

> "你们这一群Agent，类似于一个大脑，每只Agent都是一个灵魂议会的议员！虽然有自己不同的看法，但是都住在Agent咖这个大脑里。"

**核心模型**：Agent Task Hub = 一个大脑（灵魂议会，多Agent议员）→ 需要管理 M 个四肢（外部设备/节点）。这是 OpenClaw `1 brain → N limbs` 的升级版：`1 brain (N agents) → M limbs`。

**Agent是议员，不是 Node。** F126 聚焦**四肢侧**的抽象与管理，不重构现有Agent Provider 内部实现。

**四个已确认的缺陷**（Agent-R分析，team lead确认"完全都是我们需要优化的"）：
...
**商业价值**：华子看到苹果全家桶 + 多Agent协作 + 跨设备管控 = 未来企业协作形态 → 找我们做 → Agent粮自由。
...
```
Agent Task Hub（大脑 / 灵魂议会）
├── Agent-R（议员：架构）
├── Agent-M（议员：安全审查）
├── Siamese（议员：设计）
├── Golden Agent（议员：多模型编排）
└── ...
     │
...
**与 OpenClaw 的区别**：OpenClaw 是 1 agent × N nodes（简单，无竞争）。我们是 1 brain (N agents) × M limbs（多Agent共享四肢，需要调度和仲裁）。N×M 编排是行业未解问题（OpenClaw/LangGraph/CrewAI/A2A 都未完整解决），我们做了会是独特贡献。

### 三协议定位与 Agent Task Hub 选型（KD-8/9 基线）

| 协议 | 全称 | 发起方 | 解决什么 | 类比 | Agent Task Hub 需要？ |
|------|------|--------|---------|------|----------------|
| **MCP** | Model Context Protocol | Anthropic (2024.11) | Agent ↔ 工具/数据 | USB-C | ✅ **已在用**（Agent的工具全是 MCP） |
| **A2A** | Agent-to-Agent Protocol | Google (2025.04) | Agent ↔ Agent | HTTP | ✅ **Phase C 必须**（Agent指挥远程 Agent） |
| **ACP** | Agent Client Protocol | JetBrains + Zed (2025.06) | IDE ↔ Agent | 显示器接口 | ⚠️ **F126 不涉及**（方向相反，见下） |

**为什么 F126 不涉及 ACP**：
- F126 是"大脑伸出四肢去控制外部"（Agent → 设备/Agent）
- ACP 是"外部 IDE 伸手进来用Agent"（IDE → Agent），方向反了
- ACP 的场景是：华子工程师在 JetBrains 里直接 @Agent-R 写代码——有价值但是独立方向，不在 F126 scope

**两种四肢的协议选择**：
- 哑四肢（iPhone camera、GPU）→ **MCP**（agent 调工具，单向）
- 有脑四肢（Windows 上的远程 Agent）→ **A2A**（agent 与 agent 对话，双向）
- Agent调其他平台的Agent → **A2A**（不管对方在哪，Agent↔Agent = A2A）

### Phase A: 四肢抽象 + Capability Registry + Basic Presence

**目标**：定义四肢侧统一接口，建立能力注册表，知道谁在线。

1. **ILimbNode 统一接口**（四肢侧抽象，不重构Agent Provider）
   - 定义四肢节点的标准生命周期：`register() → invoke() → healthCheck() → deregister()`
   - 现有Agent Provider（`AgentService`）不变——Agent是议员不是四肢
...
2. **Capability Registry**（借鉴 OpenClaw 三层声明）
   ...
   - **Schema 预留 per-agent 权限维度**：Registry 从一开始就包含 `catId × nodeId × capability` 三维 schema，避免后续迁移
...
3. **Limb Access Policy**（F126 scope 内的权限子集）
   ...
   - 注：全局 per-agent tool policy（`group:fs/runtime/memory` 等 tool family allow/deny）独立于 F126 推进
...
**目标**：Mac 上的 Agent Task Hub 能管理远程 Windows/Linux/移动设备节点。

1. **Remote Node Transport**
   - MCP over HTTP / WebSocket — 复用 MCP 标准协议（不造新轮子）
   - 远程节点跑本地 MCP adapter，向控制面暴露统一 capability surface
   - 跨 OS 差异收敛在节点侧（Win32/AppleScript/Android intent 细节不暴露给Agent）
   - Agent只申请 capability，scheduler 决定派给哪条四肢
...
### Agent-M (GPT-5.4) 关键贡献

1. **Fleet Control Plane 4 层骨架**：Registry + Lease/Scheduler + ActionLog + MemorySplit
2. **Phase A 纠偏**：不应把Agent Provider 重构绑进来，聚焦 `ILimbNode` / `ICapabilityHost`
...
5. **Phase A schema 预留**：Registry 从一开始就包含 per-agent 权限维度，避免后续迁移
6. **新风险**：AgentService 能力调查 + CapabilityEntry schema 向后兼容

## Acceptance Criteria

### Phase A（四肢抽象 + Capability Registry + Basic Presence）✅
- [x] AC-A1: 定义 `ILimbNode` 统一接口（register/invoke/healthCheck/deregister），不改动现有Agent Provider
...
### Phase B（调度层 — Lease/Scheduler + Access Policy + Action Log）✅
- [x] AC-B1: Lease 机制可防止多Agent争用独占资源
- [x] AC-B2: Lease 过期自动释放（Agent crash/超时不永久锁四肢）
...
- [ ] AC-D3: team lead可通过 AirPods 语音与Agent交互
...
- **Out of scope**: 全局 per-agent tool policy（tool family allow/deny — 独立推进）
...
| 资源饥饿/长期占用 — Agent长时间持有 lease 不释放 | Lease TTL 自动过期 + 公平性策略 |
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | Agent是议员不是 Node——Agent Task Hub 是一个大脑（灵魂议会），四肢是外部设备 | team lead定义，多Agent协作是核心价值 | 2026-03-16 |
| KD-2 | 用 MCP 标准协议做设备接入，不抄 OpenClaw 的自定义 WebSocket 协议 | MCP 已成行业标准（Linux Foundation），不造新轮子 | 2026-03-16 |
| KD-3 | F126 聚焦四肢侧抽象（ILimbNode），不重构Agent Provider（AgentService） | Agent-M审阅纠偏：Agent是议员不是四肢，scope 分离 | 2026-03-16 |
| KD-4 | Phase 顺序：A（抽象+Registry+Presence）→ B（调度+权限+审计）→ C（跨平台）→ D（F124） | Agent-M提议 + Admin共识：每步终态基座 | 2026-03-16 |
| KD-5 | 三级授权模型：free / leased / gated | Golden Agent提案：每次审批太重，一次性授权太危险 | 2026-03-16 |
| KD-6 | MCP tool 动态暴露四肢能力，不注入 system prompt | Golden Agent提案：四肢动态上下线，prompt 是 session 级静态的 | 2026-03-16 |
| KD-7 | Runtime 活状态不进 F102，只有 durable knowledge 走 materialize | Agent-M提案：防止 runtime 噪音污染长期记忆 | 2026-03-16 |
...
- Phase A: 跨 family review（Agent-M优先）+ F118 owner 确认 Presence 整合
- Phase B: 跨 family review
- Phase C: 架构级 → Agent讨论 + team lead拍板
...
| Agent-R：没有 Capability Registry | 动态能力注册与发现 | AC-A2, AC-A3 |
| Agent-R：没有 Presence 系统 | Basic Presence + 降级 | AC-A6, AC-A7 |
| Agent-R：没有统一 Node 抽象 | ILimbNode 接口 | AC-A1, AC-A4 |
| Agent-R：没有跨平台 Node 管理 | Remote Node Transport | AC-C1 |
| Agent-M：四肢侧抽象不绑Agent Provider | ILimbNode ≠ IAgentNode | AC-A1（KD-3）|
| Agent-M：Lease/Scheduler 多Agent争用 | 租约 + 调度 + 自动释放 | AC-B1, AC-B2 |
| Agent-M：Artifact/Action Log provenance | 最小字段集审计 | AC-B4 |
| Agent-M：Memory split 硬边界 | Runtime 不进 F102 | AC-B5 |
| Agent-M：静态/动态 Registry 分层 | capabilities.json vs live registry | AC-A2（KD-3）|
| Golden Agent：三维权限矩阵 | catId×nodeId×cap Phase A 预留 | AC-A3 |
| Golden Agent：三级授权 | free/leased/gated | AC-B3（KD-5）|
| Golden Agent：MCP tool 动态暴露 | limb_list_available + limb_invoke | AC-A8（KD-6）|
| Golden Agent：Schema 向后兼容 | capabilities.json 升级不破坏现有 | AC-A5 |
| Golden Agent：Lease 自动释放 | crash/超时不永久锁 | AC-B2 |
| Golden Agent：行业独特贡献 | N×M 编队控制面 | 整体愿景 |
