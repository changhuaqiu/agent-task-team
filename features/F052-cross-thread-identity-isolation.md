...
Agent Task Hub 的每条 Thread 是一条独立工作流。多条 Thread 并行时，Agent需要跨线程通知、传话、交接。F043 `cross_post_message` 解决了"传输"，但传过去的消息**没有来源标记**——收件方分不清是本线程的Agent说的，还是别线程的Agent传过来的。

### team experience（2026-03-02 Thread `thread_mm8nkwlcwmwhmfgz`）

> "我们没做跨线程的身份隔离！别线程的 codex 他顶着 codex 的名字"
> "我得知道是Agent-M本地还是其他线程来的？"
...
4. 不会自动拉 Thread A 的完整上下文——但跨线程消息**自带来源标记**，Agent可以按需用 `get_thread_context(threadId=A)` 主动拉
...
| A2A 路由 | `parseA2AMentions` 全局 catId 自引用过滤 | 跨线程同名Agent @mention 被误杀 | 🔴 高 |
...
- 跨线程（`effectiveThreadId !== record.threadId`）：`parseA2AMentions(content)` — 不传 senderCatId，不过滤同名Agent
...
```
// 本地消息（不变）
[msgId] [14:30 Agent-M] 内容...

// 跨线程消息（新增来源标注）
[msgId] [14:30 Agent-M ← from thread:mm806zbc] 内容...
```

Agent看到 `← from thread:xxx` 就知道这条不是本线程产生的。不自动注入来源线程的上下文——Agent可以按需调用 `get_thread_context(threadId=xxx)` 主动拉。
...
|----|---------------------------|---------|----------|------|
| R1 | "我得知道是Agent-M本地还是其他线程来的" | AC-A1, AC-B1, AC-B2 | test + UI badge | [x] |
...
| KD-3 | 不自动注入跨线程上下文 | 保持线程隔离边界，Agent按需主动拉 | 2026-03-02 |
...
- **Evolves into**: F056（设计语言Agent化 — 跨线程气泡作为打样）、F057（Thread 可发现性 — badge 增强）
...
| Phase B context 标注改变Agent的行为 | 只加前缀标注，不改消息内容本身 |

## Review Gate

- Phase A: 跨家族 review（Agent-M codex 或 gpt52）
...
|------|-------------|---------|------|
| Agent-R (Opus) | F052 spec, F043 spec, team experience thread | ① 核心问题=跨线程消息无来源标记+A2A误杀 ② 交付物解决了 ③ team lead在UI看到蓝色badge+codex被正确A2A触发 | [x] 签收 |
| Agent-M (Codex) | F052 spec, 代码diff(16 files), 测试覆盖 | R1: 2P1(WS路径遗漏) → R2: 1P1(后台线程遗漏) → R3: 0P1/P2 放行 | [x] 放行 |

## Known Bugs

### Bug: 同Agent跨线程消息被"自己消息"过滤吞掉 (2026-03-06)
...
**影响范围**:
- 同Agent跨线程消息：**看不到**（catId 相同 → 被过滤）
- 异Agent跨线程消息：**正常**（catId 不同 → 不被过滤）
- 这解释了为什么跨线程通讯"有时正常、有时失灵"

**独立验证**: Agent-R(Opus 4.6) 定位 + Agent-M(GPT-5.4) 独立复核确认
...
**回归测试**: `same agent + crossPost => should appear in 对话历史增量`
