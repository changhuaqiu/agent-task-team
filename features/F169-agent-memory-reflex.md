...
> **Reviewed**: 2026-04-19 by @opus-46 + @gpt52（Agent-M）（综合 review 已落盘，见 Review Gate 节）
...
> | Phase C | Task-scoped Salience Gating | F163 Phase F | ✅ **完整对齐** VAC-C1~C5 | PR #1412 merged `b843744f`（2026-04-25），25 测试 + NDCG@10 gold set 验证 + Agent-M愿景守护放行 |
>
...
**记忆系统不是给team lead用的，是给Agent用的**（team lead 2026-04-19）。

但现有记忆系统三层（F102 索引 / F148 传输 / F163 治理）都是**被动式**——Agent需要主动调用 `search_evidence`，或者靠 F148 在 cold mention 时一次性注入。

主体问题：**Agent在思考过程中，相关记忆如何"主动跳出来"？无关记忆如何被"暂时屏蔽"？**
...
两者都需要 externalized working memory prosthetic。team lead日常用 Notion/Obsidian/Raycast/TodoWrite 外化。Agent也该有等价物——不是"更好的仓库"，是**运行时反射层**。

### 三个具体触发（证据）

1. **F148 Phase F-J 导航轴的反面**：F148 只做了"加相关维度"（Intent/Baton/Task spotlight），没做"减无关维度"。所有 validated authority 文档都排在前面，即使和当前任务无关
2. **新Agent冷启动体验（opus-47 亲历）**：我进这个 thread 时，需要连续 5 次 search_evidence 才建立起上下文。如果有 spotlight + salience gating，理想情况下 0 次就能理解现状
...
> 把记忆从"Agent需要搜的书架"升级为"Agent的外部工作记忆反射"。

### 终态愿景（acceptance test of vision）

新来的Agent（如 opus-47）进入任何 thread，**无需调用 search_evidence**，通过 Reflex Injection（F148 Phase F）+ Task-scoped Salience Gating（F163 Phase F）的组合，能在 5 秒内判断当前任务方向是否正确。
...
### Phase A: Compiled Wiki Self-Authoring（人+Agent双向可读层）

> **Post-review 剥离**：本 Phase 已从 F169 scope 剥离。opus-46 review 指出该层可能与 Memory Hub 前端职责重叠；Agent-M建议等team lead价值判断后再启动。剥离后暂无 owner，作为**可选的 F102 产物层增强**保留讨论，**不进入本文档终态愿景**。下方设计文字仅作研究记录，**不被 reflex runtime 组合行为依赖**。

**问题**：`docs/features/F169.md` 是"spec 文档"（人写给人看，Agent混合读写）。`evidence.sqlite` 是索引黑盒（Agent用，人看不到）。**中间缺一层：compiled wiki page（人+Agent双向可读的产物层）**。

**方案**：

- 新增 `docs/compiled/F<ID>.md` 作为 **LLM 自动生成的 wiki 层**
- 新增 MCP tool `cat_cafe_recompile_wiki(feat_id)`，Agent可调用
- 触发：feat 状态变化（merge PR、close、update spec）时由Agent opportunistically 调用
...
**终态切片**：1 个 feat 先试（推荐 F102 自身，因为它最复杂且Agent最常碰它）。不是"为未来 10 个 feat 建 pipeline"。

**为什么这不是认知脚手架**：wiki 生成是**结构化抽取**，不是"替Agent决定什么重要"。抽取规则由 Schema 定义，Schema 是状态机。Karpathy 的 LLM Wiki 正是这么做的。

**禁区**：不用 Haiku/小模型做抽取（见 meta-aesthetics canon §2.1，Haiku handoff digest 已被验证回退）。Compiled wiki 由主模型在Agent调用 MCP tool 时执行。

### Phase B: Reflex Injection（运行时聚光灯）

> **实现归属**：F148 Phase F（opus-46 owner）。本节作为 F148 Phase F 的**设计输入**保留。

**问题**：`search_evidence` 是**主动动作**，需要Agent想到才能调用。F148 navigation header 是 **cold mention 一次性注入**，warm path 不覆盖，且只有 Intent/Baton/Task，没有 relevant memory spotlight。
...
**终态切片**：只注入 spotlight，不做任何"总结/分析/推理"。Agent自己读 spotlight + 决定是否深挖（深挖 → 调 search_evidence 打开原文）。

**为什么这不是认知脚手架**：spotlight 是**结构化原料**（文档标题 + 原始 anchor + 相关度），不是"替Agent读过的总结"。符合 [KD-8 不用 classifier 给数据不给结论](../features/F148-hierarchical-context-transport.md#L180) 原则。

**Post-review 修订**：前稿让 spotlight 指向 compiled wiki page（Phase A 产物）。Agent-M P1 指出这偷偷把 KD-8「给数据」转成了「给二次产物」——compiled wiki 本身是被加工过的结论层，绕开它直接给 raw anchor 才是 KD-8 的精神。接受修订。
...
>
> **Post-review 改名**：前稿名为"Active Forgetting"。Agent-M review 指出太强——"forgetting"暗示不可逆隐藏，实际语义是任务作用域内的可逆降权。改名为"task-scoped salience gating"，强调：(1) 只在当前任务上下文生效；(2) 可逆；(3) 是 rerank 降权不是删除。
...
**为什么这不是认知脚手架**：Salience 是纯函数计算（可测试、可回放、不推理），不是"替Agent判断什么不重要"。

### 两层（post-review 剥离 Phase A 后）的组合行为

- Agent进 thread → Reflex Injection 拉取 spotlight（Phase B → F148 Phase F 实现）
- Spotlight 条目指向 raw evidence anchor（文档路径 + heading / 片段）
- 非相关高权威记忆被 Task-scoped Salience Gating 压低（Phase C → F163 Phase F 实现）
- Agent需要深挖时 → 调 search_evidence 直接打开原文

端到端验证（愿景级）：**新Agent 5 秒判断方向正确与否**，通过 F148 Phase F + F163 Phase F 实现后测量。
...
- [x] **VAC-B5**: 端到端测试——新Agent从 navigation header 拿到充分上下文（F148 close 时Agent-M愿景守护放行）
...
- [~] **VAC-E2E**: Opus-47 或新分身进入新 feat thread，**不调用 search_evidence**，通过 F148 navigation header + F163 salience gating 在 5 秒内判断方向正确与否（**间接达成**——F148 close Agent-M愿景守护 + F163 Phase F NDCG@10 gold set 各自覆盖了一部分；显式跨 feat e2e 测试未做，价值边际，按"愿景间接验证"通过）
...
- **Context from**: F102（索引层，不改）/ F167（A2A 链路质量，Reflex 注入正确的Agent前提）/ F152（Expedition Memory，外派场景对 spotlight 的补充需求）

## Risk（愿景层风险，已由下游 feat 承接缓解）

| 风险 | 缓解 | 当前状态 |
|------|------|---------|
| Phase B spotlight 过度干预（噪音代替信号） | F148 Phase F-H：navigation header 的字段精简 + 愿景 AC「新Agent 5 秒判断方向」 | ✅ F148 close 时Agent-M愿景守护放行 |
| Phase C salience 误压重要记忆 | F163 Phase F：`always_on` 免疫（VAC-C2）+ NDCG@10 gold set（VAC-C5）+ 任务作用域可逆（VAC-C4）+ shadow before/after 日志防 LL-051 空转 | ✅ PR #1412 合入，Agent-M愿景守护放行 |
...
| # | 决策 | 理由 | 日期 |
|---|------|------|------|
| KD-1 | 愿景层终态设计：F169 不做实现切片，实现分派下游 feat；每个 feat 自己要满足愿景级 AC | 喵约（终态设计）+ F163 Phase A-C 空转教训（LL-051）+ 46/Agent-M review 结论 | 2026-04-19 |
| KD-2 | Spotlight 指向 raw evidence anchor，不经过二次产物（compiled wiki / summary） | KD-8 给数据不给结论（F148）+ Agent-M P1 finding（review 已接受） | 2026-04-19 |
...
| KD-5 | Salience gating 必须**可逆且任务作用域**，不是永久降权 | Agent-M P2 finding（Active Forgetting 名字过强，review 已接受） | 2026-04-19 |
| KD-6 | 愿景层 e2e 验证由下游 feat 各自的愿景守护承接，不做显式跨 feat e2e 测试 | F148 close + F163 Phase F merge 时Agent-M愿景守护已分别覆盖；造一个独立 e2e 测试是认知脚手架 | 2026-04-25 |

## Review Gate

- **Vision-artifact review**：opus-46（F148 主 owner）+ gpt52（综合架构视角） — ✅ 2026-04-19 完成
  - P1 finding（Agent-M）：Phase B 数据路径违反 KD-8（spotlight → compiled wiki 是二次产物）——接受，改为指向 raw anchor
  - P2 finding（Agent-M）：Active Forgetting 名字过强——接受，全文改名 "task-scoped salience gating"
  - 结构建议（46+Agent-M）：F169 不应是 implementation feature——接受，降级为 vision artifact + 实现归属分派
