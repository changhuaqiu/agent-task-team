...
> **Status**: done | **Owner**: Agent-R | **Priority**: P1 | **Completed**: 2026-03-18
...
team lead期望的行为很简单：**Agent在忙（不管怎么忙起来的），我发的消息就排队，只有我点 steer 才能强推。** 这要求所有执行路径都接入统一的 active/queue 状态语义。
...
| ② | Connector（飞书/GitHub/iMessage） | ⚠️ 条件入队（同 agent slot 活跃才 queue） | ✅（入队后受 steer 管） | `ConnectorInvokeTrigger.ts:106-112` |
| ③ | Agent post_message A2A（worklist） | ❌ 直接 pushToWorklist | ❌ | `callback-a2a-trigger.ts:67-116` |
| ④ | Agent multi_mention | ❌ 直接 dispatchToTarget | ❌ | `callback-multi-mention-routes.ts:138-184` |
...
**用户视角的影响**：Agent A2A 自动接力时，你只能看着等，不能 steer 插队。如果你要的是"Agent之间 handoff 也能被我 steer 管到"，当前做不到。
...
`dispatchToTarget` 调用 `routeExecution` 时只传了 `{ signal }`，没有 `parentInvocationId`（`callback-multi-mention-routes.ts:163`）。如果 multi_mention 目标Agent在回复中 @mention 发起Agent，A2A push 可能进错 worklist。

#### P1: multi_mention target 崩溃导致 caller slot 不释放

**现象**（team lead 2026-03-14 22:54 截图）：Agent-M干完活用 multi_mention @ opencode，opencode 上下文超限崩溃（`prompt token count of 158302 exceeds the limit of 128000`），但Agent-M的 InvocationTracker slot 没有释放 → 系统一直显示"Agent正在回复中"→ team lead发的消息只能排队，除非手动 steer 强推。

**根因推测**：`callback-multi-mention-routes.ts` 的 `dispatchToTarget` 在 target 执行失败时，caller 的 tracker slot 没有正确 complete。热修加的 `tracker.start()` / `tracker.complete()` 只管 target 自己的 slot，但 caller（Agent-M）的 slot 可能在等 multi_mention 完成才释放——target 崩了就永远等。

**用户视角的影响**：Agent @ 了一个挂掉的Agent后，team lead被锁死在排队状态，只能手动 steer。
...
- **Slot-aware InvocationTracker** → 不同Agent在同一 thread 不互相 abort（`InvocationTracker.ts:50-54`）✅

## What

### team lead期望的行为

1. **Agent在忙时（不论原因），我发的消息必须排队** — 已实现 ✅
...
4. **Connector 来的消息和用户消息一样可靠** — ⚠️ 部分实现：同 agent slot 活跃时走 queue ✅，但判忙是 slot 级（`has(threadId, catId)`）而非 thread 级，与"Agent在忙就排队"的全局语义可能不一致（见 OQ-4）
...
1. Phase A 就是终态
2. UI 明确区分"Agent自动接力中"和"有排队消息"两种状态
...
> team lead 2026-03-15 反馈：用户消息能打断 A2A 链。Admin（opus+codex+gpt52）独立排查确认为 P1 竞态。
...
**修复方案（Admin对齐）：**
1. `InvocationTracker` 新增 `tryStartThread(threadId, catId, ...)` — thread 级 busy gate + slot 级占位，一个同步操作。thread 内任一Agent活跃 → 返回 null（不 preempt）。
...
- [x] AC-B2: QueueProcessor `tryAutoExecute` — agent 条目入队后目标Agent slot 空闲时立即执行
...
- [x] AC-B8: Thread 执行状态指示（PR #508 merged）— ThreadExecutionBar per-agent 活跃状态 + 经过时间
  - ✅ 打磨完成：Agent名使用 `useCatData()`+`formatCatName()` 动态中文显示，颜色从 agent-config 动态读取（`feat/f122-remaining` branch）
- [x] AC-B9: Per-agent Stop 按钮（PR #510 merged）— cancel API + ThreadExecutionBar × 按钮
  - ✅ 同上打磨：Agent名 + 颜色随 B8 一起修完
- [x] AC-B10: 双模发送 UX — whisper 模式下执行中Agent chip 禁用（灰色+⏳），auto-select 跳过活跃Agent，reconcile 移除新活跃 targets（`feat/f122-remaining` branch）
...
**现象**：opencode @ codex 的 A2A agent entry 在 QueuePanel 里排队，"Agent正在回复中" 持续显示，但实际上 opencode 已完成执行。entry 不会被自动出队。

**可能根因**（待新 session 用 debugging skill 验证）：
1. `tryAutoExecute` 在 enqueue 时调了一次，目标Agent slot 忙 → 跳过。之后没有重试机制
...
**现象**：A2A callback 消息（`post_message` with @mention）入库后 `deliveryStatus` 为 `undefined`，`isDelivered()` 返回 true → ContextAssembler 立即将其纳入Agent的上下文 → Agent在处理其他任务时就"看到了"排队中的 A2A 消息。
...
> Admin（opus+gpt52+opencode）独立分析后的共识 + team lead 2026-03-14 拍板。
> 负责：Agent-R主写 + Agent-M review，同一 thread 按节奏推进。

### 关系定位

- **F108 = capability（能力）**：让同一 thread 多Agent并发成为可能。Phase A 已合入，是基座。
...
1. **入队粒度之争**：F108 Phase B 的 side-dispatch（悄悄话/锁头）是"绕过 queue 直接派给空闲Agent"；F122 Phase B 的目标是"所有执行都进 queue"。语义矛盾。
2. **判忙语义之争**：F108 Phase B 需要 slot 级（`has(threadId, catId)`）区分"AgentA忙AgentB空闲"；F122 OQ-4 在讨论是否改为 thread 级。
...
| 风险 | 缓解 |
|------|------|
| Phase B 如果把 A2A 入 queue，Agent自动接力会变慢（每步要过 queue） | 可以给 agent-source entry 设置 auto-execute（跳过排队，但在 queue 里有记录） |
...
| KD-2 | Connector 消息走 slot 级条件入队，Phase A 需评估是否改为 thread 级 | `ConnectorInvokeTrigger` 用 `has(threadId, catId)` 判忙，只对同 agent slot 入队（Agent-M review 指出） | 2026-03-14 |
| KD-3 | Phase A 不改 A2A 调度模型，只补漏洞 | 降低风险，先稳后收敛 | 2026-03-14 |

## Review Gate

- Phase A: 跨家族 review（Agent-M优先，codex 或 gpt52）✅
