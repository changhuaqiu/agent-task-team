...
- **部分实现**
  - `runtimeId / channel / authContextId` 已进入 payload 与 daemon，但尚未形成完整前端配置中心
  - 统一运行时目录仅部分存在，前端没有独立 `CLI Runtimes` 管理页
- **未实现**
  - 独立的 `src/app/settings/integrations/page.tsx`
  - 完整的 `Provider Profiles / Channels / Routing Policy` 数据模型与配置界面
  - 文档设计稿中描述的统一 runtime center UI

因此，本 Spec 当前应视为“进行中”，而不是“全部完成”。
...
系统 SHALL 在当前设置抽屉中提供“模型账号”和“角色卡”两类配置能力，作为当前已落地的统一配置入口。
...
### Requirement: 独立配置中心页面已经完成
**Reason**: 当前代码中不存在独立配置中心页面，继续将其描述为现状会误导实现与验收。
**Migration**: 把当前正式入口定义为设置抽屉内的“模型账号 / 角色卡”。

### Requirement: `ProviderProfile / Channel / RoutingPolicy` 已完整落地
**Reason**: 当前代码只具备参数预留和设计方向，没有完整数据模型与 UI。
**Migration**: 暂将其保留为后续阶段范围，不作为本阶段验收项。
