...
- [x] Agent 聊天先讲用户可理解的结果；不主动喷出 Invocation、WorkContract、outcome、revision、fencing 等内部术语，不在正文重复工具、命令和文件流水。
- [x] 完成、阻塞和需要用户决定三类回复都有简短模板；角色语气不会覆盖全局回复契约。
- [x] Agent 已有持久化正文或 Invocation 已结束时不再显示无限跳动的空气泡；同一 Agent 的相邻调用不会串泡或互相收尾。
