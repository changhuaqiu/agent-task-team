...
- [x] A shared runtime directory has one Conversation-owned `TASKS.md` projection; takeover uses a fail-closed claiming/owned lease shared by every server writer, safely adopts legacy unowned rows, rebuilds from the new Conversation Task Graph, stops stale watchers, rejects unknown post-claim rows, and cannot turn foreign task rows into shadow work in a new Delivery.
