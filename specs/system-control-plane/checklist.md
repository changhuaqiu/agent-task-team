...
- [x] An accepted `continue_work` checkpoint from execution, Task review, Delivery review, or acceptance verification becomes a dedicated bounded continuation, carries the exact next action into a fresh WorkContract epoch, never consumes the Invocation failure budget, and consumes the current WorkContract's only accepted exit slot; no terminal Outcome can be accepted in that same Contract. Queued continuations reserve durable capacity, while legacy unversioned rows retain retry compatibility.
- [x] WorkContract permissions exclude Task/Task Graph mutation tools; accepted Outcomes are the only Agent write seam into Task, Gate, A2A and Delivery owners.
- [x] An applied Task Graph outcome means its owner commit and runnable standalone Inbox commands already exist in the admission transaction; malformed/stale proposals and invalid assignees do not consume the terminal slot.
- [x] A standalone/A2A accepted continuation atomically queues the next command with preserved Work/Possession authority and a three-checkpoint budget; Delivery continuation remains Control Plane-owned.
- [x] A confirmed ACP CLI session is sealed before reuse when its cumulative input-token or terminated-Invocation budget is exhausted, so provider-resumed hidden history cannot bypass ContextSnapshot limits.
- [x] An active session generation with an unterminated Invocation is never reused by a concurrent dispatch; acquisition fails closed inside the same atomic boundary as Invocation creation.
...
- [x] Completion without Outcome opens exactly one fresh fenced command-only epoch, preserves the last durable output in context, exposes only the WorkContract's single-intent lifecycle tools, grants no native edit/execute or Skill tools, and fails without human escalation if that recovery turn does not submit an accepted command.
