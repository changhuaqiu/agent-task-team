...
> 依赖：[`Agent Observability 长期设计`](../../docs/technical/observability/agent-observability.md)、[`observability-drilldown`](../observability-drilldown/)、[`system-control-plane`](../system-control-plane/)、[`Platform Harness 长期设计`](../../docs/technical/execution/platform-harness-state-machine-design.md)
...
离线评估报告的应用 provenance 必须直接来自冻结 `ApplicationSnapshot` 和已经校验的 observed digest；不得在评分时重新读取可变的项目 HEAD、TeamPack、RoleCard 或 active Skill。`mode=offline` 必须将 case、root task、Harness trigger、invocation、trace 绑定到同一条运行中/评估中的 `eval_case_execution`，并绑定不可变应用快照以及与 target digest 一致的 observed digest；任一证据缺失或不匹配都 fail-closed，且不得命中历史无 provenance 的复用快照。
