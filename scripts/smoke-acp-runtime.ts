...
//   # model is optional — see DEFAULT_SMOKE_MODEL below.
//
...
/**
 * Default smoke model per runtime (used when argv[3] is omitted).
 *
 * opencode: the host's configured default (`zhipuai-coding-plan/glm-4.7`)
 * emits only `agent_thought_chunk` updates with ~1 output token and produces
 * NO `agent_message_chunk` (text) — so the text+done+completed assertion
 * cannot pass against it. DeepSeek is the verified credential on this host
 * (`~/.local/share/opencode/auth.json`) and produces a proper text response,
 * so it is the smoke default. Override with argv[3] if needed.
 *
