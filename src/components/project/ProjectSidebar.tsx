...
export function ProjectSidebar({ projects, conversations, tasks, activeSurface, selectedProjectId, onOpenActivity, onOpenAgents, onOpenProjects, onOpenSettings, onSelectProject }: {
  ...
  const workCountByProject = useMemo(() => new Map(projects.map((project) => [
    project.id, projectWorkItems(project, conversations, tasks).length,
  ])), [conversations, projects, tasks]);
  ...
  return <aside className={cn('flex h-full shrink-0 flex-col border-r border-[hsl(var(--border-subtle))] bg-[hsl(var(--bg-card))] transition-[width] duration-200', expanded ? 'w-[248px]' : 'w-14')} data-testid="project-sidebar">
    ...
    {expanded ? <>
      ...
      <div className="mt-5 flex items-center px-4 pb-1"><button type="button" onClick={onOpenProjects} className={cn('min-w-0 flex-1 text-left text-[10px] font-semibold uppercase tracking-[0.12em] text-[hsl(var(--text-tertiary))]', activeSurface === 'projects' && 'text-[hsl(var(--text-primary))]')}>项目</button></div>
