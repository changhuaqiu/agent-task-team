...
import { projectAttention, type ProjectAttentionItem } from '@/lib/project-attention';
...
export function ProjectOverviewSurface({ project, workItems, artifactCount, blockers = [], onOpenAttention, onOpenWork }: {
  ...
  blockers?: Blocker[];
  onCreate?: () => void;
  onOpenAttention?: (item: ProjectAttentionItem) => void;
}) {
  const active = workItems.filter((item) => item.status === 'in_progress');
  const attention = projectAttention(workItems, blockers);
    ...
    <div className="mx-auto w-full max-w-6xl px-5 py-6 sm:px-7">
      <section className="rounded-2xl bg-[hsl(var(--bg-card))] p-5 sm:p-6" aria-labelledby="project-overview-heading">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="text-[11px] font-medium text-[hsl(var(--text-tertiary))]">项目概览</p><h3 id="project-overview-heading" className="mt-1 text-xl font-semibold">{project.name}</h3><p className="mt-2 max-w-2xl text-xs leading-5 text-[hsl(var(--text-tertiary))]">从工作项进入具体目标、任务、交付件和讨论；项目层只汇总已确认的进度。</p></div>

        </div>
        <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <OverviewMetric icon={Clock3} label="进行中" value={active.length} />
          <OverviewMetric icon={AlertCircle} label="需要处理" value={attention.length} tone="blocked" />
      ...
      <div className="mt-5"><ProjectAttentionList items={attention} onOpen={(item) => onOpenAttention?.(item)} /></div>
      <section className="mt-5 overflow-hidden rounded-2xl bg-[hsl(var(--bg-card))]" aria-labelledby="recent-work-heading">
        <header className="flex items-center justify-between border-b border-[hsl(var(--border-subtle))] px-5 py-4"><div><h3 id="recent-work-heading" className="text-sm font-semibold">近期工作项</h3><p className="mt-1 text-[11px] text-[hsl(var(--text-tertiary))]">{workItems.length} 个工作项 · {done.length} 项完成</p></div>{workItems.length > 0 && <button type="button" onClick={() => onOpenWork()} className="text-xs font-medium text-[hsl(var(--text-secondary))] hover:underline">查看全部</button>}</header>
