...
type ProjectTab = 'overview' | 'work' | 'reviews' | 'artifacts' | 'releases' | 'activity' | 'automations' | 'evaluation' | 'diagnostics';

export function ProjectObjectWorkspace({ project, conversations, tasks, blockers, requestedWorkItem = null }: {
  project: WorkspaceProject;
  conversations: Conversation[];
  tasks: Task[];
  blockers: Record<string, Blocker[]>;
  requestedWorkItem?: { conversationId: string; taskId: string } | null;
}) {
  const [tab, setTab] = useState<ProjectTab>(() => {
    ...
    return requestedWorkItem || opensChildWorkstream ? 'work' : 'overview';
  });
  ...
  useEffect(() => {
    ...
    if (!selectedInProject) setSelectedConversationId(project.workspaceConversationId);
    setSelectedTaskId(null);
  }, [conversationIds, project.workspaceConversationId, requestedWorkItem, selectedConversationId, setSelectedConversationId, setSelectedTaskId]);
  ...
  return <div className="flex min-h-0 flex-1 bg-[hsl(var(--bg-card))]" data-testid="project-object-workspace">
    <section className="flex min-w-0 flex-1 flex-col">
      ...
      {tab !== 'work' && <A2APossessionStrip conversationId={project.workspaceConversationId} />}

      {tab === 'overview' && <ProjectOverviewSurface project={project} workItems={workItems} artifactCount={artifactCount} onCreate={() => setCreateKind('work')} onOpenWork={(item) => { if (item) setSelectedConversationId(item.conversationId); setSelectedTaskId(null); setTab('work'); }} />}
      {tab === 'work' && <ProjectWorkItemsWorkspace project={project} conversations={conversations} tasks={tasks} preferredWorkItem={requestedWorkItem} onCreate={() => setCreateKind('work')} />}
      ...
      {tab === 'activity' && <ProjectActivitySurface project={project} workItems={workItems} />}
    ...
    {createKind && <ProjectObjectCreateDialog kind={createKind} project={project} tasks={scopedTasks} onClose={() => setCreateKind(null)} onWorkCreated={({ conversationId }) => { setSelectedConversationId(conversationId); setSelectedTaskId(null); setTab('work'); }} onReviewCreated={() => { setReviewRefreshToken((value) => value + 1); setTab('reviews'); }} />}
