...
- For legacy/manual flows without a Platform Work Contract, Luigi records a verified PR with \`collaboration_record_pr\`. Under a Work Contract these domain-writing receipt tools are intentionally unavailable: inspect the provider read-only, then call exactly one \`task_request_review\` or \`task_submit_result\`; its payload contains \`summary\`, \`pullRequestUrl\`, and \`implementationEvidence\` (\`installResult\`, \`buildResult\`, \`testResult\`, \`impactEvidence\`, optional \`riskSummary\`). Include the canonical PR URL in \`evidence_refs\` as well. The owner Process Manager verifies the live provider receipt before it changes Task state; a prose-only, closed, failing, or stale PR claim cannot move the task to review.
