...
const IMPACT_ANALYSIS_ROLE_GUIDANCE = {
  mario: [
    ...
    '- 每个实现任务都记录 architecture_check=required 或 architecture_check=not_required；不得静默跳过 DK 判断。',
    '- 宣布“管道已启动”前必须核对每个目标 agent 的 dispatch receipt；缺 receipt 时要补发或说明阻塞。',
    '- 只有授权合并完成并在 main 复验后，才调用 collaboration_record_merge 闭环；不得用 task_update_status 冒充完成。',
  ].join('\n'),
  luigi: [
    IMPACT_ANALYSIS_PROTOCOL,
    PERSONALITY_AUTONOMY_PROTOCOL,
    '- 全栈实现前通过仓库搜索、调用链和相关测试确认目标组件、状态与入口；改动保持在已识别影响边界内。',
    '- commit、push、PR 和测试完成后调用 collaboration_record_pr；成功回执会自动进入 quality_gate 并唤醒 Peach。',
  ].join('\n'),
  peach: [
    IMPACT_ANALYSIS_PROTOCOL,
    PERSONALITY_AUTONOMY_PROTOCOL,
...
export const PRESET_TEAM_PACKS: LegacyTeamPackSeedInput[] = [
  {
    ...
    workflow: {
      ...
      states: [
        ...
        { name: 'quality_gate', role: 'peach', description: 'Peach 基于当前 PR head SHA 留下真实 review/comment 并执行测试；DK 仅在架构/schema/安全/跨模块风险触发时介入', transitions: [{ from: 'quality_gate', to: 'merge_verify', condition: '当前 head SHA 评审且 blocker=0' }, { from: 'quality_gate', to: 'implementing', condition: '拒绝并直接打回 Luigi' }, { from: 'quality_gate', to: 'planning', condition: '同一任务拒绝超过 2 次或需要范围决策' }] },
        { name: 'merge_verify', role: 'mario', description: '等待授权合并，并在 main 上重新执行安装、构建、测试和影响复核', transitions: [{ from: 'merge_verify', to: 'done', condition: 'provider merge receipt 与 main delivery evidence 齐全' }, { from: 'merge_verify', to: 'quality_gate', condition: 'PR head 变化，旧评审失效' }] },
        { name: 'done', role: 'mario', description: 'Mario 基于合并闭环卡完成关闭', transitions: [], terminal: true },
      ],
    },
    communicationMatrix: {
