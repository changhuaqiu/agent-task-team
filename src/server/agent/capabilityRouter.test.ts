...
// Task 10 removed the bespoke per-engine *_CAPS constants along with the
// bespoke backends. checkCapabilities is backend-agnostic, so these tests use
// synthetic CapabilitySet profiles mirroring the former codex/claude/opencode
// shapes to exercise the resume/systemPrompt/maxTurns/PTY degradation rules.
const SYNTH_CODEX_CAPS: CapabilitySet = {
  engine: 'codex',
  promptMode: 'arg',
  outputMode: 'ndjson',
...
const SYNTH_CLAUDE_CAPS: CapabilitySet = {
  engine: 'claude',
...
const SYNTH_OPENCODE_CAPS: CapabilitySet = {
  engine: 'opencode',
  promptMode: 'arg',
  outputMode: 'events',
  supportsResume: true,
...
function makeBackend(caps: CapabilitySet): AgentBackend {
...
describe('checkCapabilities — resume 降级', () => {
  it('codex 不支持 resume 时，剔除 resumeSessionId 并返回警告', () => {
    const backend = makeBackend(SYNTH_CODEX_CAPS);
  ...
  it('claude 支持 resume 时，保留 resumeSessionId 且无相关警告', () => {
    const backend = makeBackend(SYNTH_CLAUDE_CAPS);
...
describe('checkCapabilities — PTY 降级', () => {
  it('opencode requiresPty 且环境无 PTY 时，返回 best-effort 警告（不阻断执行）', () => {
    const backend = makeBackend(SYNTH_OPENCODE_CAPS);
