...
describe('evaluationLab', () => {
  ...
  it('uses weighted kappa for reviewer calibration', () => {
    const cases = Array.from({ length: 10 }, (_, index) => ({
      caseKey: `calibration-${index}`, split: 'tune' as const, input: { index },
    }));
    const dataset = evaluationLab.createDataset({
      conversationId: 'conv-lab', name: '校准集', description: '双人盲标校准', cases,
    });
    const rows = getDb().prepare('SELECT id FROM eval_case WHERE dataset_id=? ORDER BY case_key')
      .all(dataset.id) as Array<{ id: string }>;
    for (const [index, item] of rows.entries()) {
      const label = index % 3 === 0 ? 'fail' : index % 3 === 1 ? 'partial' : 'pass';
      for (const reviewerId of ['reviewer-a', 'reviewer-b']) {
        evaluationLab.annotate({
          conversationId: 'conv-lab', caseId: item.id, reviewerId,
          dimensionKey: 'correctness', label, rationale: '独立盲标',
        });
      }
    }
    expect(evaluationLab.agreement(String(dataset.id), 'conv-lab')).toMatchObject({
      compared: 10, agreementRate: 1, weightedKappa: 1, threshold: 0.7, status: 'calibrated',
    });
  });

  it('scopes global-dataset annotations to the current project and excludes unknown pairs', () => {
    const db = getDb();
    db.prepare(`INSERT INTO conversation
      (id,title,status,participants,created_at,updated_at) VALUES (?,?,?,?,?,?)`)
      .run('conv-second', 'Second project', 'active', '[]', now, now);
    db.prepare(`INSERT INTO eval_dataset
      (id,conversation_id,name,description,revision,status,created_by,created_at,updated_at)
      VALUES ('dataset-global',NULL,'Global calibration','Shared cases',1,'active','system',?,?)`).run(now, now);
    db.prepare(`INSERT INTO eval_case
      ...
      VALUES ('case-global','dataset-global','global-1','tune','manual','{}','{}','{}','hash-global','redacted',?)`)
      .run(now);
    evaluationLab.annotate({
      conversationId: 'conv-lab', caseId: 'case-global', reviewerId: 'reviewer-a',
      dimensionKey: 'correctness', label: 'unknown', rationale: 'Insufficient evidence.',
    });
    evaluationLab.annotate({
      conversationId: 'conv-lab', caseId: 'case-global', reviewerId: 'reviewer-c',
      dimensionKey: 'correctness', label: 'pass', rationale: 'Second local calibration label.',
    });
    evaluationLab.annotate({
      conversationId: 'conv-second', caseId: 'case-global', reviewerId: 'reviewer-b',
      dimensionKey: 'correctness', label: 'pass', rationale: 'Independent project label.',
    });
    expect(evaluationLab.agreement('dataset-global', 'conv-lab')).toMatchObject({
      compared: 1, usableCompared: 0, excluded: 1,
      identityVerification: 'verified', status: 'insufficient_evidence',
    });
  });

  it('does not claim calibrated kappa for a single-class label set', () => {
    const cases = Array.from({ length: 10 }, (_, index) => ({
      caseKey: `single-class-${index}`, split: 'tune' as const, input: { index },
    }));
    const dataset = evaluationLab.createDataset({
      conversationId: 'conv-lab', name: 'Single class', description: 'Degenerate kappa fixture', cases,
    });
    const rows = getDb().prepare('SELECT id FROM eval_case WHERE dataset_id=?').all(dataset.id) as Array<{ id: string }>;
    for (const item of rows) {
      for (const reviewerId of ['verified-a', 'verified-b']) {
        evaluationLab.annotate({
          conversationId: 'conv-lab', caseId: item.id, reviewerId,
          dimensionKey: 'correctness', label: 'pass', rationale: 'Same class',
        });
      }
    }
    expect(evaluationLab.agreement(String(dataset.id), 'conv-lab')).toMatchObject({
