...
describe('invocation-repo', () => {
  ...
  it('settles every non-terminal invocation after a daemon restart', () => {
    for (const [id, status] of [
      ['inv-planned', 'planned'],
      ['inv-starting', 'starting'],
      ['inv-running', 'running'],
      ['inv-terminating', 'terminating'],
    ] as const) {
      invocationRepo.create({ id, conversation_id: 'conv-1', agent_id: 'agent-a' });
      if (status !== 'planned') {
        invocationRepo.transition(id, { to: 'starting', expectedFrom: 'planned' });
      }
      if (status === 'running' || status === 'terminating') {
        invocationRepo.transition(id, { to: 'running', expectedFrom: 'starting' });
      }
      if (status === 'terminating') {
        invocationRepo.transition(id, { to: 'terminating', expectedFrom: 'running' });
      }
    }
    invocationRepo.create({
      id: 'inv-completed',
    ...
    invocationRepo.transition('inv-completed', {
      to: 'terminated',
      outcome: 'completed',
    });

    expect(invocationRepo.settleInterrupted()).toBe(4);
    expect(invocationRepo.settleInterrupted()).toBe(0);
    expect(invocationRepo.getByConversation('conv-1')).toEqual(expect.arrayContaining([
