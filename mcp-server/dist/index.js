...
function createServer() {
    ...
    server.tool('dispatch_to_agent', 'Dispatch a prompt to a specific agent in Agent Task Hub. The agent will execute the prompt using its configured CLI engine (opencode, claude, codex, or mock). Returns the run ID for tracking.', {
        ...
        return new Promise((resolve) => {
            const timeout = setTimeout(() => {
                socket.disconnect();
                resolve({
                    content: [{ type: 'text', text: `Error: Connection to daemon at ${DAEMON_URL} timed out. Ensure the Hub daemon is running.` }],
                    isError: true,
                });
            }, 15_000);
            socket.on('connect', () => {
                ...
                setTimeout(() => {
                    socket.disconnect();
                    resolve({
                        content: [{ type: 'text', text: `Dispatched to agent "${agentId}": ${prompt.slice(0, 100)}${prompt.length > 100 ? '...' : ''}` }],
                    });
                }, 500);
            });
            socket.on('connect_error', (err) => {
                clearTimeout(timeout);
                resolve({
                    content: [{ type: 'text', text: `Error: Cannot connect to daemon at ${DAEMON_URL}: ${err.message}` }],
                    isError: true,
                });
            });
            socket.connect();
    ...
    server.tool('list_agents', 'List all available agents in Agent Task Hub with their roles and configured CLI engines.', {}, async () => {
        const agents = [
    ...
    server.tool('check_daemon', 'Check if the Agent Task Hub daemon is reachable and get connection status.', {}, async () => {
        const socket = connectDaemon();
        return new Promise((resolve) => {
            const timeout = setTimeout(() => {
                socket.disconnect();
                resolve({
                    content: [{ type: 'text', text: `Daemon at ${DAEMON_URL}: unreachable (timeout)` }],
                    isError: true,
                });
            }, 5_000);
            socket.on('connect', () => {
                ...
                socket.disconnect();
                resolve({
                    content: [{ type: 'text', text: `Daemon at ${DAEMON_URL}: connected ✓` }],
                });
            });
            socket.on('connect_error', (err) => {
                clearTimeout(timeout);
                resolve({
                    content: [{ type: 'text', text: `Daemon at ${DAEMON_URL}: error — ${err.message}` }],
                    isError: true,
                });
            });
            socket.connect();
