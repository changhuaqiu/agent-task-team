<div align="center">
  <h1>🐈 Agent Task Hub</h1>
  <h3>领养你的 AI 研发团队</h3>
  <p>
    <strong>想法和产品之间，隔着的不是程序员，而是实现力。</strong>
  </p>
  <p>
    <a href="#-为什么需要-agent-task-hub">为什么需要</a> •
    <a href="#-它是什么">它是什么</a> •
    <a href="#-核心理念">核心理念</a> •
    <a href="#-快速开始">快速开始</a> •
    <a href="#-了解更多">了解更多</a>
  </p>
</div>

---

## 🎯 为什么需要 Agent Task Hub

**你有没有过这样的经历？**

脑子里有一个绝妙的想法，想做一个产品、一个工具、一个 demo。但你卡住了——

- 找程序员？太贵，而且沟通成本巨大
- 自己学？时间不够，而且不想成为程序员
- 用 AI 工具？每次都是从零开始，上次的对话、上次的经验、上次踩的坑，全都丢了

**问题不是你缺一个更强的 AI，而是你缺一支团队。**

传统 AI 工具是"一次性助手"——用完就忘，下次重新开始。但真正的团队不一样：

- 他们**记得**你们一起做过的每一个决定
- 他们**积累**了你们磨合出来的协作默契
- 他们**成长**了，从 80 分的通用能力，长成 100 分的专属配合

**Agent Task Hub 就是让你"领养"这样一支团队的地方。**

---

## 🤖 它是什么

Agent Task Hub 不是另一个 AI 聊天工具。

它是一个**多智能体协作平台**，让你和多个 AI Agent 组成虚拟研发团队，共同把想法变成能运行的产品。

```
┌─────────────────────────────────────────────────────────────┐
│                                                             │
│    你（人类）                                                │
│    ├── 提供愿景                                              │
│    ├── 做出判断                                              │
│    └── 带着作品走进人群                                       │
│                                                             │
│    Agent 团队                                               │
│    ├── ⭐ Mario       — 项目统筹、拆解任务、协调升级         │
│    ├── ⚡ Luigi       — 全栈开发（前后端 + API + 数据模型） │
│    ├── 🌸 Peach       — 质量保障（评审 + 集成测试）         │
│    └── ⚙️ DK          — 架构工程（按需架构门禁）           │
│                                                             │
│    一起磨合、一起成长、一起交付                                │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## 💡 核心理念

### 1. 想法和产品之间，隔着的是实现力

> 卡住的不是想法，是把它做成能跑的东西的能力。Coding 是这个时代最直接的实现力。

以前传递想法靠 PPT，未来靠能真正跑起来的 POC 和 demo。你缺的不是更强的 AI 工具，是一支能把想法变成产品的团队。

### 2. 领养团队，不是配置工具

> 工具用完就忘，团队越磨越强。

你和 Agent 一起磨出来的协作规则、踩过的坑、沉淀的 skills，都会留下来——这些加在一起才是真正的生产力。下次项目直接复用，从通用的 80 分继续长到属于你的 100 分。

### 3. 对等协作，共享记忆

> 没有 Boss Agent，各有视角；自由判断，结构化交付。

四个 Agent 各有专长——项目统筹、全栈开发、质量保障、架构工程——自己判断该不该回应、怎么回应。执行有纪律：TDD、代码审查、质量门禁。Agent 之间的对话是共享的，上下文不丢，不会出现"我跟上一个 Agent 说过了"的尴尬。

### 4. AI 不是让你退场，而是让你上场

Agent Task Hub 不是替你完成梦想，而是让你终于有机会带着自己的团队，把想法做成能运行的世界。以前没有资源、没有团队、没有舞台；现在 Agent 陪你一起把它做出来，带着作品走进人群。

---

## ✨ 能做什么

| 场景 | 怎么用 |
|------|--------|
| **做一个产品原型** | 描述你想要什么，团队帮你从 0 到 1 |
| **改造现有项目** | 把代码库接入，团队帮你分析、重构、加功能 |
| **学习新技术** | 让团队带你做一个真实项目，边做边学 |
| **自动化重复工作** | 把流程告诉团队，以后一句话触发 |

---

## 🚀 快速开始

### 一键安装

```bash
git clone <repo-url> agent-task-hub
cd agent-task-hub
./setup.sh    # 自动安装依赖 + 构建
pnpm start    # 启动
```

打开 http://localhost:3000，开始领养你的团队。

### 三步上手

```
第一步：创建项目
  └── 告诉团队你想做什么

第二步：配置账号
  └── 让团队能调用 AI 能力

第三步：开始协作
  └── 发消息、分配任务、看进展
```

---

## 🏗️ 技术架构

如果你是开发者，可能关心这些：

| 层级 | 技术 | 说明 |
|------|------|------|
| 前端 | Next.js 16 + React 19 | 现代化 Web 应用 |
| 状态 | Zustand 5 | 前端状态管理 |
| 数据库 | SQLite | 本地持久化，零配置 |
| 实时通信 | Socket.io | 双向 WebSocket |
| Agent 执行 | ACP（Agent Client Protocol） | 单一 `AcpBackend` 经 stdio JSON-RPC 驱动 OpenCode（原生）/ Claude / Codex（适配器） |

**统一 Agent 运行时（ACP）**：daemon 只通过一个 `AcpBackend`、用 Agent Client Protocol（stdio JSON-RPC）驱动三类运行时——OpenCode 原生 ACP、Claude / Codex 经 `@agentclientprotocol` 适配器接入。声明式 Catalog（`agentCatalog.seed.json`）是启动事实源，适配器版本锁定；daemon 不再为每个引擎写死分支或解析私有 CLI 输出。

**会话级隔离**：每个项目中每个 Agent 维护独立会话，上下文不串。

**队列隔离**：跨项目的任务排队互不干扰。

**Skill 系统**：可复用的能力模块，从 Git 仓库导入。

→ [完整架构文档](./docs/wiki/01-architecture.md)

---

## 📸 界面预览

```
┌─────────────────────────────────────────────────────────────────┐
│  Agent Task Hub                                    [+新项目] [⚙]│
├──────────┬──────────────────────────────────────┬───────────────┤
│          │                                      │               │
│  📁 项目  │   🎯 目标：做一个 Todo 应用          │   📋 看板     │
│          │   ─────────────────────────────      │               │
│  My App  │                                      │   □ 设计 UI   │
│  Demo    │   ⭐ Mario: 我来拆解任务...           │   □ 写代码    │
│  学习    │   ⚡ Luigi: 已完成登录模块...          │   ■ 测试     │
│          │   🌸 Peach: 代码审查通过...            │               │
│          │                                      │   ⚠ 风险     │
│          │   ─────────────────────────────      │   无          │
│          │   [输入消息...]                       │               │
│          │                                      │               │
└──────────┴──────────────────────────────────────┴───────────────┘
```

---

## 🌟 为什么选择 Agent Task Hub

| 对比 | 传统 AI 工具 | Agent Task Hub |
|------|-------------|----------------|
| **协作方式** | 1 对 1，单线程 | 多 Agent 并行协作 |
| **记忆** | 用完就忘 | 持久化记忆，跨会话保留 |
| **经验积累** | 每次从零开始 | 团队越磨越强 |
| **上下文** | 只有当前对话 | 全项目共享 |
| **质量保障** | 无 | TDD + 代码审查 + 门禁 |
| **适用场景** | 简单问答 | 完整产品开发 |

---

## 🗺️ 路线图

- ✅ 项目工作台 UI
- ✅ 多 Agent 协作
- ✅ 会话级隔离
- ✅ SQLite 持久化
- ✅ Skill 系统
- ✅ 智能任务分发
- 🚧 统一集成配置中心 — 账号模型 + 角色卡绑定 + daemon 执行上下文扩展
- 🚧 Role Card 生态系统 — TeamPack 数据模型 + GitHub 导入管道 + 协作编排层
- 🚧 安全扫描 — Prompt 注入检测 + 敏感信息检测 + 危险指令检测
- 🚧 项目绑定 — 项目与 TeamPack 1:1 绑定，生命周期内不可切换
- 🚧 团队模式引擎 — pipeline / parallel / hub_spoke / custom 四种策略
- 🚧 来源追踪 — TeamPack.source 记录导入来源（github/preset）
- 🚧 输入验证 — 目录结构探测 + pack.json 格式校验 + role.json 验证
- 🚧 速率限制 — shallow clone 30s 超时 + GitHub API 频率控制
- 🚧 错误国际化 — 统一错误码与 i18n 资源管理
- 📋 更多 Agent 类型支持
- 📋 团队协作功能

---

## 📚 文档

| 文档 | 说明 |
|------|------|
| [产品愿景](./docs/product/vision.md) | 为什么做这个项目 |
| [架构设计](./docs/wiki/01-architecture.md) | 技术架构详解 |
| [开发规范](./docs/sop.md) | 如何参与贡献 |
| [Agent 指南](./AGENTS.md) | Agent 工作约束 |
| [路线图](./docs/roadmap.md) | 阶段目标 |
| [完整文档](./docs/README.md) | 文档导航 |

---

## 🤝 参与贡献

我们欢迎任何形式的贡献：

- 🐛 报告 Bug
- 💡 提出新功能
- 📖 完善文档
- 🔧 提交代码

---

## 📄 开源协议

MIT License

---

<div align="center">
  <h3>「领养团队，一起长出世界。」</h3>
  <p>
    <sub>让每个人都有机会带着自己的团队，把想法变成现实。</sub>
  </p>
</div>
