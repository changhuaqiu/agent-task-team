...
Task Graph / Harness 是任务状态推进和 Agent 派发的唯一服务端边界。Web 客户端只消费任务投影，不得根据遗留 socket 事件自行把 `pending` 改为 `in_progress`，也不得重复触发 Agent dispatch。daemon 解析出唯一 runtime task path 后把它持久化到 task；Harness 接受 owner dispatch 并推进 `pending → in_progress` 时，在通知页面前同步更新该 runtime `TASKS.md`。每个规范化 runtime path 同时只能有一个 watcher；watcher 显式接收 Conversation identity，新 Conversation 接管同一路径时必须关闭旧 watcher、清除旧 debounce，并通过 `TASKS.owner.json` 的 claiming/owned 租约重建投影，不能从目录 basename 猜测任务域。`startTaskWatcher()` 返回幂等 cleanup，由实际创建 watcher 的调用持有，重复 start 不取得已有 watcher 的关闭权。`TASKS.md` watcher 在完成兼容文件解析、质量门禁和数据库更新后，必须从 Task Graph 重新读取权威任务状态与依赖再发布 `project:view(type=task.sync)`；原始文件中的过期字段即使被主动 invocation 或 receipt gate 拒绝，也不能通过投影回流并覆盖页面。
