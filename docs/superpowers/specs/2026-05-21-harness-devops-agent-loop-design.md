...
### 2.1 单角色 Agent Loop（角色内迭代）

每个角色内部有独立的 Agent Loop 循环：

```
① 👤 人下达任务 → ② 🤖 AI Skill 处理 → ③ 👤 人审阅 → ◇ OK?
   ├── YES → 输出产物，进入下一阶段
   └── NO  → 反馈修正，回到 ①（1-N 次迭代）
```
...
每个交接点由一个黄色菱形（◇）标记，表示：
- 交接人完成 Agent Loop，产物经过自身审阅确认
- 接收人收到产物后，开始自己的 Agent Loop
- 如发现问题，接收人可发起跨角色反馈（测试→开发、测试→架构师、测试→BA）
...
### 2.3 Sprint 知识闭环（Sprint 回顾 → 知识库）

每个 Sprint 结束时，全员通过 AI 辅助提取规则：
...
| Agent Loop | 角色内部的迭代循环：人下达任务→AI处理→人审阅→确认/修正 |
