...
**状态**：Accepted（2026-08-31 修订 Project 归属）
**日期**：2026-07-20
...
- 平台已有 Project 表达长期仓库、目录与团队边界；Issue 在该 Project 下获得独立工作项协作空间。
- DeliveryRun 表达一次从目标到最终结果的持续交付承诺。
- Task Graph 表达 Agent 拆解后的任务、依赖、负责人和质量门。

维护者创建符合策略的 Issue 后，系统按仓库目录复用或幂等创建 Project，并在其下创建独立工作项后开始交付。
主流程不再要求用户复制 Issue 内容或指定下一个 Agent；同一仓库的后续 Issue 不重复创建 Project。

## 用户可见行为

- 工作项标题保留 `#Issue编号 + Issue标题`，可以从平台定位回 GitHub。
- Issue 正文和验收清单作为规划上下文，不把 webhook、delivery ID、receipt 等内部概念暴露在主体验。
- 重复事件只显示同一个工作项；同一仓库只显示一个长期 Project。
...
- 同一 Issue 始终映射到同一个 Project 下的工作项和交付运行。
