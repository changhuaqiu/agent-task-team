...
→ Harness Habitat 主海报
→ 端到端实机演示
→ 30 秒内讲清用户问题、工作路径和产品对象
→ 角色协作、当前能力与明确边界
→ 与普通 Coding Agent 的差异
→ 快速开始、技术架构与仓库地图
→ 项目状态与文档入口
```
...
README 是产品入口，不是愿景文档、架构全文或路线图副本。首页文案遵守四条约束：

1. 首屏先回答“它是什么、为谁解决什么、如何看到真实产品”，不先铺内部术语；
2. Project → Work Item → Task → Deliverable / Evidence 的层级必须直接可见，不能让访客把产品理解成共享群聊；
3. 已实现能力与仍在强化的边界分开表达，不把目标架构或本地开发 EXE 写成正式发行能力；
4. 架构只保留一张主路径图和仓库地图，深入细节统一链接到 `docs/` 与 `specs/`。
