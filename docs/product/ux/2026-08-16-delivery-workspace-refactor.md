...
- Agent 聊天正文面向用户，不面向控制面。第一句直接说明“完成了什么、结论是什么或需要用户做什么”；普通完成回复控制在一至五句，确有必要时最多补三项证据、风险或下一步。文件路径、测试命令、工具名和内部状态默认进入 Trace/调试，用户追问时再展开。
- 角色人格只改变语气与专业视角，不改变可读性底线。不得把 Invocation、WorkContract、outcome、revision、fencing、Gate ID 等实现词直接当作用户解释；专业词不可避免时，必须紧跟一句白话说明。
...
- 用统一的用户可见回复契约约束所有角色：先给语义摘要，再按需展开结构化细节；角色人格不能覆盖产品级沟通规则。
...
- 输入提示不要求用户必须理解 `@Agent`；无显式指定时，服务端按当前交付 Team Runtime 选择可用接手人。需要定向并行时，输入区以次级提示展示可选的行首 `@成员` 语法，无接手人或成员引用无效时页面显示明确错误。
