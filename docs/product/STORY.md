...
## 2026-09-06：从“看见很多状态”到“知道下一步做什么”

### 原来的用户处境

项目已经分了工作项，但侧栏把执行任务也计入工作数量；子任务卡住时概览没有提示。创建目标后不知道是否已经启动，旧工作容易跳到同空间里的另一项工作；完成页面又把项目分支评审和单项验收混在一起，只能复制文件路径，无法直接查收内容。

### 优化后的变化

日常路径收敛为“创建目标 → 交给团队安排 → 处理当前阻塞 → 查收成果与验收”。统筹者及账号来源可见；创建、提交和运行不再混为一谈。概览里的阻塞可以直接打开对应子任务，高级操作默认收起。旧工作保持独立定位，成果按贡献角色和类别组织，能安全读取文件正文，并显示这项工作的验收人、版本、证据和缺失信息。

### 验证证据

固定样本包含 4 个工作项、13 条任务记录：侧栏数量由 13 修正为 4，真正进行中由误计 3 修正为 1，之前漏掉的子任务阻塞现可见。真实浏览器验证了接入目录、创建目标、一键直达阻塞、旧工作 B 查收及四节 Markdown 预览；自动化回归和两轮独立评审覆盖精确导航、提交幂等、证据归属、通知去重与预览安全。完整命令、结果和集成状态见[体验评测](../technical/evaluation/2026-09-06-ux-journey-evaluation.md)。

### 保留边界

这些改进不要求重建旧项目，也不替历史缺失记录制造验收结论。预览是当前磁盘内容，不是冻结版本；重试不会放宽权限。浏览器样本没有调用真实模型，不能据此声称 Agent 整体完成率或执行耗时已改善。

设计依据：[简明工作旅程](ux/2026-09-06-simple-work-journey.md)、[验收与安全预览](../technical/execution/work-result-reading.md)。

---

## 2026-09-05：GitHub 首页重排为面向首次访客的产品入口

### 原来的用户处境

README 虽然解释了 Agent OS 愿景，但“为什么需要、它是什么、核心理念、为什么选择”多次重复同一观点。真实演示之外仍保留过时的 ASCII 界面，Project / Work Item / Task / 交付件层级、默认角色分工和当前桌面发布边界都不够直接；面试官或首次访问者需要读很久，仍难快速判断产品做什么、怎么运行、哪些能力已经验证。

### 优化后的变化

- 首屏用一句产品定义、用户承诺、开发者预览状态和播放入口建立预期，端到端演示成为第一内容区；
- 主体按“问题 → 从目标到证据的路径 → 产品对象层级 → 角色协作 → 当前能力与边界 → 快速开始 → 架构”组织，不再重复六段理念；
- 明确 Project 下每个 Work Item 独立持有 Task、活动、交付件与证据，项目概览只做跨工作项汇总；
- 删除旧 ASCII 截图和冗长路线图副本，用真实 GIF / MP4、Mermaid 主路径和仓库地图承载可验证信息；
- 中英文 README 保持同一结构，并明确当前只有开发者预览、本地桌面构建不等于正式签名发行版。

### 结构验证

中英文首页的六个主导航、两张 Mermaid 图、全部仓库内链接和三项演示资产均通过确定性检查；GitHub Markdown 渲染结果可定位首屏播放入口、第一内容区演示、产品层级、快速开始和桌面发布边界。README 不再包含旧 ASCII 界面或重复的详细路线图，独立审查确认信息层级、事实边界和双语结构一致后才合入。这些检查只证明信息架构和呈现完整，不替代首次访客的计时理解测试。

### 仍然保留的边界

README 优化改善的是首次理解、面试展示和开发者上手路径，不证明 Agent 总体任务完成率已经提高。正式安装包、签名、自动更新、跨平台发布矩阵，以及固定任务集上的完成率和路径效率，仍按技术设计、活动规格和评估文档推进。

### 设计与实现依据

- [品牌视觉与 README 叙事规范](brand/agent-task-hub-brand-visual-system.md)
- [产品愿景](vision.md)
- [文档架构](../README.md)
...
确定性服务端用例证明同一 Project 中两个新 Work 获得两个不同执行作用域，Project workspace 中的新 Task 为 0。本地真实页面显示 3 个独立工作项，项目活动有 0 个输入器，所选工作项活动有 1 个输入器；从 A 的活动页真实创建 B 后，标题、详情、聊天和 A2A 作用域同步切换。全量回归 271 个文件、1980 项通过，2 项按既有配置跳过；TypeScript、production build 和 Rust release build 通过，独立复审最终为 0 Critical / 0 Important。最终桌面 build 为 `desktop-build-03f33ae76d76cbfda2be76e0c9812268`。
...
在真实桌面 SQLite 的一致备份上，候选版本将 1 个过期 Invocation 结束，关闭 22 个残留 active Authority，终态 A2A 占用从 20 降为 0，退役 handler 的 queued/running 从 2 降为 0，并回填 2 个历史完成时间；数据库完整性与外键检查均通过。Task 仍为 2/4 完成，没有把失败伪增成成功。全量回归 1981 项通过、2 项跳过，生产构建通过。
