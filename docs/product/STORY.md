...
## 2026-08-19：Agent 已经回答时，聊天区不再只剩一个跳动的空气泡

### 原来的处境

用户看到 Agent 气泡一直跳动，以为任务仍在执行或 Agent 没有回复。实际运行事实却显示调用已经正常结束，最终评审正文也已写入数据库；临时流式气泡因为完成事件迟到或遗漏，反而遮住了真正答案。已有答案还常混入 Gate、revision、grep、平台接纳等内部汇报词，用户需要先理解系统实现才能找到结论。

### 优化后的变化

- 流式气泡按具体 Invocation 对账；同一 Agent 的下一次调用不会复用上一轮气泡，迟到的旧完成事件也不会关闭新回复；
- 空临时气泡与持久化正文并存时，聊天区直接展示已经落库的正文；
- 所有角色共用“先说结果、必要说明最多三项、技术细节进入工具记录”的用户可见回复契约，角色性格不能覆盖它。

### 已验证的效果

- 真实 `/api/state` 事实确认 `inv-0001787097781164-046127-faf7b015` 已正常终止且最终正文存在，排除“Agent 仍在运行”；
- 回归覆盖空临时气泡遮挡正文、同 Agent 连续 Invocation、旧完成事件迟到以及首次/后续/wakeup 提示词注入；
- 全量回归 1680 项通过、2 项跳过，Next.js 生产构建通过。

### 仍然保留的边界

提示词和角色卡的确定性契约已经落地，但本轮没有向用户正在运行的交付注入合成消息，因此不把“真实模型回复质量已经提高”写成已验证结论。下一次干净任务仍需做 baseline/candidate ApplicationSnapshot paired experiment；详见对应评测记录。

### 设计与实现依据

- [交付工作区 UX 决策](ux/2026-08-16-delivery-workspace-refactor.md)
- [Human-readable Agent Chat Evaluation](../technical/evaluation/2026-08-19-human-readable-agent-chat-evaluation.md)
