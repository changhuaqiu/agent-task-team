...
> Archive status: implemented (2026-07-14). The active 4-person migration is defined by `specs/team-simplification/`.
...
- Moving a task into `in_review` requires `implementation_evidence` with applicable install, build, test, and impact-boundary evidence.
- Moving a task into `done` requires `delivery_evidence` with target-branch, install, build, test, and residual-risk evidence.
...
- Default-team prompts require verifiable implementation, review, architecture, testing, and handoff evidence.
