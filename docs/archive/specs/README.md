...
| 规格 | 归档状态 | 原因或替代规格 |
| --- | --- | --- |
| [`ux-journey-completion/`](ux-journey-completion/) | implemented | 九项全流程体验修正、确定性比较、独立评审与本地桌面更新完成；长期事实见工作旅程、结果阅读和 2026-09-06 体验评测 |
...
| `architecture-subtraction-37/` | implemented | 第三十七轮删除 Team Runtime 重复账号候选 interface 与 Provider 别名，账号 readiness schema 只由 `account-auth` 拥有 |
...
| `architecture-subtraction-55/` | implemented | 第五十五轮删除无人读取的 filesystem session metadata，让会话恢复只由 SQLite `sessionRepo` 持久化，并收窄 GC sidecar |
