...
带 legacy proposal 标记的派发，且不得把这种预期抑制显示为内部错误。该标记必须持久化到 Agent Inbox command，并由 Inbox Scheduler 传入 Invocation activation；Invocation Planner 必须再次依据持久化 DeliveryRun 拒绝，不能只依赖浏览器内存或 socket 接入层。
