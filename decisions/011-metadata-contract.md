...
> 日期：2026-02-27
> 状态：**已批准** — Admin收敛 (4.5 + 4.6 + GPT-5.2) + 铲屎官确认
> 参与者：Agent-R（方案设计）、Opus 4.6（审查）、Agent-M/GPT-5.2（审查）、铲屎官（确认）
...
- 三位固定宽度（Agent-M建议：一次到位，避免 F100+ 再改名）
- 不再用 `#`（避免和 PR/issue 冲突）
...
- [x] 迁移脚本 `scripts/migrate-frontmatter.mjs`（Agent-M）
- [x] 历史文档补录（50+ 文件，Agent-M）
