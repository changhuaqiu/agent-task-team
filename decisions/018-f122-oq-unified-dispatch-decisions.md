...
> 决策日期：2026-03-15 | 决策人：铲屎官 | 提案人：Agent-R
...
**效果**：
- A2A 任务在 QueuePanel 可见（用户知道Agent在干嘛）
- 用户可以 steer 插队纠正方向（Agent聊歪了能拉回来）
...
**理由**：multi_mention 本质是"一次 @ 多只Agent"，产生的每个子调用和 A2A handoff 语义相同。统一入 queue 消除独立分发平面。
...
**效果**：
- AgentA 在忙时，发给AgentB 的消息直接执行（by the way 场景）
- 用户可以同时和多只Agent交互，只要目标Agent空闲
- 前端提示"XXAgent正在回复中"，用户知道谁在忙

**放弃的方案**：thread 级判忙（`has(threadId)`）。理由：太粗，AgentA忙时AgentB也被锁，牺牲并行性。

**安全保障**：`tryStartThread()` 在执行层面仍是 thread 级 busy gate（同一 thread 不会并发两个 invocation），slot 级只影响入队决策。

## 对 F108 Phase B 的影响

F108 Phase B 的 side-dispatch（悄悄话/锁头 → 绕过 queue 派给空闲Agent）需要与 F122 Phase B 协调：
- side-dispatch 可以设计为 queue 里的特殊 entry type（`source: 'side-dispatch'`）
- slot 级判忙支持 side-dispatch 的"AgentA忙发给AgentB"语义，方向一致
