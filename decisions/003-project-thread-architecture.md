...
1. **CLI 天然需要目录**: `spawn('claude', ['-p', ...], { cwd: projectDir })` — 三只Agent的 CLI 都在一个目录里工作。目录就是项目上下文。

2. **文件系统即记忆**: 目录结构本身就是知识的组织方式 — CLAUDE.md, AGENTS.md, GEMINI.md, docs/, src/ 都是项目记忆的一部分。

3. **跨项目天然隔离**: 当Agent咖帮铲屎官做别的项目时（非 agent-hub 本身），`AgentServiceOptions.workingDirectory` 已支持设置不同 cwd。Project = Directory 让这个变得自然。
...
│   ├── Participants (活跃Agent)
│   ├── Tasks (Phase 3.5, 毛线球)
│   └── Summary (Phase 3.5, 拍立得照片墙)
├── Thread
│   └── ...
└── 项目配置 (CLAUDE.md, agent-config.json 等)
```
...
| Agent配置外置 | 身份注入的前置 | agent-config.json |
...
Demo 发现: 三只Agent全不知道自己是谁。

```
System Prompt 分层组装:
1. 身份层: "你是Agent-R(Opus), Agent Task Hub 的主架构师..."
2. 项目层: "当前项目: agent-hub, 路径: /path/to/project"
...
```
用户打开 Agent Task Hub → 检测 cwd → 注册为 Project
...
- **备选方案 C**：按Agent身份拆 Project（每Agent一个项目视图）
  - 不选原因：会制造知识孤岛，破坏Admin协作共享上下文的目标。
...
5. **双轨制 (Task + Roundtable)** — Agent-M的设计 (`dual-track-collaboration-design.md`) 已获共识。
