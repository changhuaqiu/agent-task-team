...
> **状态**: 已决定
> **日期**: 2026-04-02
> **决策者**: 铲屎官 + Agent-R + Agent-M（review）
> **上下文**: F151 Design Gate review + feat-lifecycle 指导

## 背景

Agent Task Hub 已通过 F088/F132/F137 接入飞书、Telegram、DingTalk、企业微信五个渠道。华为小艺是 HarmonyOS 设备的原生 AI 助手，覆盖手机/平板/手表/车机。
...
3. Agent Task Hub 自身充当 WebSocket 客户端连接 HAG，用户无需额外部署
...
| 投递 | 非流式 — `reasoningText` 思考气泡 → 每Agent完成后 `artifact-update(text)` append 累积 |
...
| ID | 来源 | 生命周期 | 用途 |
|----|------|---------|------|
| `params.sessionId` | 华为 HAG | 跨 app 重启稳定 | 对话标识 → 映射到 Agent Task Hub thread |
...
Agent A 完成
  ↓
artifact-update { text: "AgentA回复", append: false }        → 首只Agent，替换
  ↓
Agent B 完成
  ↓
artifact-update { text: "\n\n---\n\nAgentB回复", append: true }  → 追加（分隔线）
  ↓
...
- `append:false` 发全量文本单Agent可行，但多Agent场景覆盖其他Agent的累积内容
...
**多Agent投递**：`hasArtifact` Set 追踪首只Agent是否已发送。首只 `append=false`，后续 `append=true` + `---` 分隔线。
...
| 8 | ~~多Agent replyParts 聚合~~ → append 累积 | 首只Agent `append=false`，后续Agent `append=true` + `---` 分隔。无 debounce，信号驱动关闭（真机验证 2026-04-06） | 2026-04-06 |
...
8. **不要做 intra-artifact 流式**：HAG app 对同一 artifactId 的 `append:true` delta 和多 text artifact 的 `append:false` 都有渲染问题（真机验证 2026-04-06）。改用 append 累积：首只Agent `append=false`，后续Agent `append=true`（跨 artifactId 追加有效）。
